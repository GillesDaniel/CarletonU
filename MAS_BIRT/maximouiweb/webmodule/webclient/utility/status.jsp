...
* IBM Corp.
--%><%@ page import="java.util.*,psdi.webclient.system.runtime.*,psdi.webclient.system.session.*,psdi.util.*"                 
%>
<%
  ...
  if (impsuccess != null)
  {
  	impsuccess = HTML.securitySafeWithHTMLEncoding(impsuccess);
  }
	String servletBase = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath();
  	...
  	if (filename != null)
  		filename = HTML.securitySafeWithHTMLEncoding(filename);
	...
	if(success)
	{
		...
		if (applicationId != null)
			applicationId = HTML.securitySafeWithHTMLEncoding(applicationId);
		applicationId = (applicationId == null) ? "<unknown>" : applicationId;
		
		String sessionId = request.getParameter(WebClientSessionManager.UISESSIONID);
		if (sessionId != null)
			sessionId = HTML.securitySafeWithHTMLEncoding(sessionId);
		msgtext = "Import of application " + applicationId + " was successful<br><br><a href='../ui/maximo.jsp?event=loadapp&value="+applicationId+"&"+WebClientSessionManager.UISESSIONID+"="+sessionId+"&" + WebClientSessionManager.CSRFTOKEN + "=" + wcs.getCSRFToken() + "'>Loadapp "+applicationId+"</a>";
    	getSqlTxt = "<a href='../ui/"+applicationId+".ora?event=exportsql&targetid="+applicationId+"&"+WebClientSessionManager.UISESSIONID+"="+sessionId+"&" + WebClientSessionManager.CSRFTOKEN + "=" + wcs.getCSRFToken() + "'>Save SQL</a> for presentation";
        getSqlTxtAll = "<a href='../ui/"+applicationId+".ora?event=exportsqlall&"+WebClientSessionManager.UISESSIONID+"="+sessionId+"&" + WebClientSessionManager.CSRFTOKEN + "=" + wcs.getCSRFToken() + "'>Save SQL</a> for presentation (also writes all presentations to disk)";
	}
%>
