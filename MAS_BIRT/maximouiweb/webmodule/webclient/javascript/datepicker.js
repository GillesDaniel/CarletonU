 ...
 */
	 ...
	 */
	updateDPParent = function(locale)
	{
		...
		else
		{
			...
			var tmpLocale = dojo.locale;
			dojo.locale = locale;
			//dojo is resolving wrong the DatePattern 'a' so we need to replace it apar: IJ07466
	...
	showDatePicker = function(fld) 
	{
		...
		let readonly = false;
		if(fld) {
			readonly = fld.getAttribute('readonly') === 'readonly'
		}
		Array.from(datePickerDialog.domNode.querySelectorAll('button')).forEach(button => {
		...
		if((pop.x + datePickerDialog.domNode.parentNode.offsetWidth) > window.innerWidth){
			datePickerDialog.domNode.parentElement.style.left = '0px';
		}
		if(first)
		{
			try
