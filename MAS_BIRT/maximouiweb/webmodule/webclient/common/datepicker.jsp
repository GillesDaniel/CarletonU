...
--%><%
	...
	String calType = s.getUserInfo().getCaltype().toLowerCase();
...
%><%!
	...
	String renderDateField(String id, String type, int size, String value, String up, String down, int increment, String pattern)
	{
		String htmlOut ="				<td class='fld_td'>"+
										"<table role='presentation'>"+
											"<tr>"+
												"<td>"+
													"<button type=\"button\" onclick='javascript: adjustDate(\""+type+"\", "+increment+")' class='cal_btn tiv_btn_top'>"+up+"</button>"+
												"</td>"+
											"</tr>"+
											"<tr>"+
												"<td>"+
											...
											"<tr>"+
												"<td>"+
													"<button type=\"button\" onclick='javascript: adjustDate(\""+type+"\", -"+increment+")' class='cal_btn tiv_btn_bottom'>"+down+"</button>"+
												"</td>"+
											"</tr>"+
										"</table>"+
									"</td>";
	...
	String renderAMPMButton(String id, String timePattern)
	{
		...
		String htmlOut ="			 <td class='fld_td'>"+
										"<div id='AMPM'><button type=\"button\" id='"+id+"_AMPMbutton' pattern='a' onclick='javascript: adjustDate(\"AMPM\",null)' class='cal_btn tiv_btn'></button></div>"+
									"</td>";
		return htmlOut;
	}
%>
					...
					<tr>
						<td class="date_footer" colspan="2" style="padding-left: 10px; padding-right:10px;">
							<table role="presentation" style="width:100%">
								<tr>
									<td>
										<button type="button" onclick="javascript: setDate()" class="tiv_btn foot_btn"><%=resources[0]%></button>
									</td>
									<td style="width:100%">
									</td>
									<td style="white-space: nowrap">
										<button type="button" onclick="javascript: clearParent()" class="tiv_btn foot_btn"><%=resources[1]%></button>
										<button type="button" onclick="javascript: updateDPParent('<%=userLocale%>')" class="tiv_btn foot_btn"><%=resources[2]%></button>
										<button type="button" onclick="javascript: closeDatePicker()" data-cancel="true" class="tiv_btn foot_btn"><%=resources[3]%></button>
									</td>
								</tr>
							</table>
						</td>
		...
		<script>
			...
			dojo.addOnLoad(function() {	
				...
				if(dojo.locale.match(/^ar(?:-.+)?$/) && <%=!calType.equalsIgnoreCase("gregorian")%>)
				{
