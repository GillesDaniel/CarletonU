...
<%
	...
	synchronized (wcs)
	{
		...
		{
			tableControl=(Table)wcs.getControlInstance(tableId,true);
		}
	...
	<%
						...
						if (tblRemote != null)
						{
							...
							dnldRemote.setDownloadSet(true);
	...
	<%
								try
								{
									MboValueData[] row = null;
									if(dnldRemote.toBeSaved()){
										row = mbo.getMboValueData(arrDataAttributes);
									}
									else{
										row = mbo.getMboValueDataForDownload(arrDataAttributes);
									}
									String strData = null;
									int maxType = -1;
									for(int k = 0; k < row.length; k++)
									{
										...
										if (arrDataAttributes[k].equalsIgnoreCase(DynamicAttributeComponent.DYNAMICATTRIBUTE))
										{
											for (String attibuteName : DynamicAttributeComponent.CLASSSPECVALUEATTRIBUTES)
											{
												mvd = mbo.getMboValueData(attibuteName);
												if (mvd.getData().length() > 0)
												{
													break;
												}
											}
	...
	<%												}
													...
													{
	%>
														<td align="right" x:num><%=mvd.getDataAsDouble()%></td>
	<%
													}
												}
	...
	<%												}
													...
													{
	%>
														<td align="right" style='mso-number-format:"\[$-409\]m\/d\/yy\\ h\:mm\\ AM\/PM\;\@";' x:num><%=mvd.getData()%></td>
	<%
													}
												}
	...
	<%
												...
												else
												{
													...
													// IV96409 - Fix formula injection vulnerability
													if (strData.indexOf("=")==0 || strData.indexOf("+")==0 || strData.indexOf("-")==0 || strData.indexOf("@")==0)
													{
														strData = "'" + strData;
													}
													
													if (WebClientRuntime.getWebClientSystemProperty("webclient.downloadpreservewhitespace","1").equals("1"))
													{
														strData = WebClientRuntime.replaceString(strData," ", "&nbsp;");
													}
	%>
	...
	<%
		...
		<%
							...
							dnldRemote.setDownloadSet(false);
