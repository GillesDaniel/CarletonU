...
else
{
	...
	boolean hideHeadings  = !Boolean.valueOf(bbControl.isFilterOpen()); 
	if(!inbbportlet)
	{
...
%>
	<% 	if(accessibilityMode) {	%>
		<table role="presentation" border="0">
			<tr id="filter_msg_row_<%=layoutId%>">
				<td width="1" valign="top">
					<img alt="" class="info_image" border="0" src="icon_details.gif" role="presentation"/>
				</td>
				<td id="filter_msg_<%=layoutId%>" class="text help" style="cursor:default <%if(hideHeadings){%>display: none<%}%>"><%= wcs.getMessage("ui","filterMsg")%></td>
			</tr>		
		</table>
	<% } %>
	<table data-bulletin-board="true" aria-label="<%=HTML.encodeTolerant(component.getProperty("arialabel"))%>" summary="<%=HTML.encodeTolerant(component.getProperty("arialabel"))%>" id="<%=id%>_table" <%=tableAutomationId%> <%if(inbbportlet){%>class="plinner"<%}%> width="<%=width%>" border="0" selectedMsgIndex="-1">
...
<%				//display all column headings
				for(int i = 0;i < attributes.size(); i++)
				{
					...
					headersString += "col_"+i+" ";%>
					<th id="col_<%=i%>" class="<%=textcss%> khtc" nowrap>
						&nbsp;
...
<%			String inputAutoId="";
				...
				{
					arrColumn = (String[])attributes.elementAt(i);	%>
					<td headers="col_<%=i%>" id="filter_cells_<%=layoutId%>_<%=i%>" headers="col_<%=i%>" <%=filterCellsAutoId%> class="rsfco">
...
<% 		}//end if <!--Venkyg : End -- It is TEXT box for searching field text box -->
		...
		if(inbbportlet)
		{
%>			<tr ><td width="2px" colspan="5" headers="<%=headersString%>" height="2px"></td></tr><%
		}
		if(data != null)
		{
			for(int i = 0 ;i < data.getMboDataCount(); i++)
...
<%					for(int j = 0;j < attributes.size(); j++)
					{
						...
						if(inbbportlet && arrColumn[0].equals("viewstatus"))
						{
							...
							if(viewedMsgIdList.contains(bbUniqueId))
								...
								isViewed = noLabel;	%>
							<td headers="col_<%=j%>" width="5%" align="<%=defaultAlign%>" nowrap valign="top">
								<span class="bbm" style="<%= style %>"><%=isViewed%></span>
							 </td>
<%						}
						else if(arrColumn[0].equals("subject"))
						{
							String subject = HTML.sanitize(displayData.getData());
							...
							<td headers="col_<%=j%>" width="30%" align="<%=defaultAlign%>" nowrap>
								<a tabindex="0" title="<%= linkTitle %>" id="<%=id%>_bbmessagelink_<%=i%>" <%=bbmsglinkAutoId%> class="<%=textcss%> bbmlc" <%=otherEvents %> onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) {bboard_displayMessage('<%=id%>',<%=i%>,<%=expandonlyone%>,<%=bbUniqueId%>);}" href="javascript:bboard_displayMessage('<%=id%>',<%=i%>,<%=expandonlyone%>,<%=bbUniqueId%>)">
									<span id="<%=id%>_bbshortsub_<%=bbUniqueId%>" <%=bbshortsubAutoId%> aria-label="<%=HTML.encodeTolerant(shortsub)%> <%= wcs.getMessage("ui","collapsed")%>" style="display:flex;flex-direction:row;align-items:center;padding-<%=defaultAlign%>:5px;padding-<%=reverseAlign%>:3px"><img src="img_row_unselect.gif"><%=shortsub %></img></span>
									<span id="<%=id%>_bbfullsub_<%=bbUniqueId%>" <%=bbfullsubAutoId%> style="display:none;flex-direction:row;align-items:center;padding-<%=defaultAlign%>:5px;padding-<%=reverseAlign%>:3px"><img src="img_row_unselect.gif" style="transform: rotate(270deg)";><%=subject %></span>
							 </td>
<%						}
...
<%						}
						else
						{
							if((!inbbportlet && arrColumn[0].equals("postdate")) || inbbportlet)
							{
								...
								<td headers="col_<%=j%>" width="18%" align="<%=defaultAlign%>" nowrap>
...
%>				</tr>
				<tr>
					<td <%if(inbbportlet) out.println("colspan=\"5\""); else out.println("colspan=\"3\"");%>  headers="<%=headersString%>">
...
<%				/* End message details */
				...
				{
%>					<tr><td colspan="5" class="bbmsd" headers="<%=headersString%>"></td></tr><%
				}
		...
		if(data == null || count==0)
		{
			if(inbbportlet)
			{	%>
				<tr class="<%=textcss%> pnc"><td colspan="6" headers="<%=headersString%>" title="<%=labels[0]%>" style="text-align: center"><%=labels[0] %></td></tr>
<%			}
			else
			{
%>				<tr class="<%=textcss%> pnc"><td colspan="3" headers="<%=headersString%>" title="<%=labels[0]%>" style="text-align: center"><%=labels[0] %></td></tr><%
			}
		}	%>
		</table>
<%		if(inbbportlet) 
...
%>	<component id="<%=id%>_holder"<%=compType%>><%="<![CDATA["%>
		<script>
			...
			hideShowElement("filter_msg_row_<%=layoutId%>",(dojo.byId("filter_msg_row_<%=layoutId%>").style.display=='none'));
		</script>
	<%="]]>"%></component>
