...
height = WebClientRuntime.isNullCheck(height) ? "500" : height;
...
String graphTitle = HTML.encode(kpiGraph.setGraphInfo(type));
...
%>	<div aria-hidden="true" id="<%=id%>" <%if(automation){%>automationid="<%=realId%>_div"<%}%> style="width:100%;height:100%;display:inline">
<%	if (graphTitle != null) 
	{
...
%>		<table aria-hidden="true" cellspacing="0" cellpadding="0" border="0" width="100%">
			<tr colspan="3">
				<td style="position:relative;display:block">
					<div id="kpichart_<%= id %>_holder" style="width: 100%; height: <%=height%>px" dir="ltr"<%/*This dir is a hack but without it labels aren't aligned correctly in rtl*/%>></div>
...
%>
			...
			chart.addAxis("x", {
				...
				"labelFunc": function(def, axisNumericPosition)
				{
					...
					return dojo.date.locale.format(date, {formatLength: 'short', selector:'date', locale: DOJOLOCALEFORDATEFORMAT});
				}
			});
			chart.addAxis("y", {
				"vertical": true,
...
%>				"leftBottom": false,
<%				}
%>				"labels": <%=yLabels.serialize()%>
			});

			var markers = dojox.charting.SimpleTheme.defaultMarkers;
...
%>			chart.addSeries("<%=WebClientRuntime.makesafevalue(lineChartData.lines[k].description.replace("\"", "%22"))%>", <%=points.serialize()%>, {"marker":markers.<%=markers[k % markers.length]%>, "stroke":{"color": "<%=lineChartData.lines[k].color%>"}, "plot":"other" });
<%			}
