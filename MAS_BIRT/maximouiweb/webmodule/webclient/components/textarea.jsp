...
<%	}
	...
	if(selectonfocus.equals("false"))
	  cssclass += " ns";
	String maxlength = length.length() > 0 ? "maxlength=\"" + length + "\"":"";
    ...
    if(exceptionType == ComponentInstance.EXCEPTION_NONE && component.isHoversActive() && !control.isOnTableFilterRow())
    {
        %><%@ include file="../common/extendedtooltip.jsp" %><%
    }
    if (wcs.getSkinName().equals("iot18") && isReadOnly)
    {
    	cssclass = cssclass + " readonly ";
    }
	%><textarea id="<%=id%>" <%=ariaString%> <%=automationId%> class="fld <%=cssclass%> <%=exceptionClass%>" <%=exc%><%
		...
		%> style="overflow:auto;width:<%=cols%>px;height:<%=rows%>px;<%=align%>;white-space:pre-wrap;"<%
