...
--%><%@ include file="../common/componentheader.jsp" %><%
{
	...
	String DESIGNER_IMAGE_PATH = "designer/";
					...
					while (y < sectionColCount)
					{
						...
						String itemdesc = md[x].getMboValueData(cols[2]).getData();	%>
							...
							<div  id="designer_ctrl_<%= itemctrl %>" class="tpnosel" controltype="<%= itemctrl %>" controlid="<%= itemctrl %>" tabindex="0"
								...
								<span><img src="<%= DESIGNER_IMAGE_PATH + itemctrl.toLowerCase() + ".gif" %>" align="middle" alt="<%= itemctrl %>"/><br><span name="controllabel" id="controllabel" style="display:<% if (isPalleteLabelsOn) out.print("inline"); else out.print("none"); %>"><%=itemdesc%></span></span>
							</div>
							</td>
<%						y++;
						x++;
