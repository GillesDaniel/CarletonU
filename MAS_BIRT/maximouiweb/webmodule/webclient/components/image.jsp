...
%><%
	...
	String iconMenu = component.getProperty("iconmenu");
	boolean defaultRender = component.isDefaultRender();
	if(defaultRender)
	{
		%><%@ include file="../common/componentholder_begin.jsp" %><%
	}
	if(iconMenu.length()>0 && component.needsRender()){
		%><div id="<%=id%>_holder" class="bc"><%
	}
	...
	if(!WebClientRuntime.isNull(dataattribute) && (control.getType().equals("image") || control.getType().equals("table-image")) && WebClientRuntime.isNull(control.getProperty("sigoption")))
	{
		if(control.getType().equals("image"))
		{
			src = ((BoundComponentInstance)component).getString();
		}
		else if(control.getType().equals("table-image") && WebClientRuntime.isNull(src))
		{
			String compId = ((ComponentInstance)component).getId();
			String rowStr = compId.substring(compId.indexOf("R:") +2);
			String rowNum = rowStr.substring(0, rowStr.indexOf("]"));
			int row = Integer.parseInt(rowNum.toString());
			src = ((ComponentInstance)component).getDataBean().getMbo(row).getString(dataattribute);
		}
	}
	...
	{
		if(align.equals("center") && !UAGENT.equals("IE") && comboBox)
		{
			align = "top";
		}
		align = "align=\""+align+"\"";
	...
	if(!lookup.equalsIgnoreCase("NONE"))
	{
				...
				&& (isLdOwner && (WebClientRuntime.isNull(longdesc) || (longdesc != null && longdesc.equalsIgnoreCase("true")))))
		{
			...
			else
			{
				...
				{
					msg = component.getProperty("arialabel") + " " + msg;
				}
				else
				{
					tabindex = "-1";
			...
			if(!lookup.equals("") || !menutype.equals(""))
			{
				...
				if(menutype.equals(""))
				{
					...
					if(msgKey != null)
					{
						...
						{
							if(accessibilityMode){
								String prependString = control.getProperty("prepend");
								...
								msg = component.getProperty("title") + " " + prependString + " " + message.getMessage();
							}
							else {
				...
				else
				{
					...
					if(accessibilityMode && src.equals("img_menu"))
					{
						msg = component.getProperty("title") + " " + msg;
					}
	...
	if(!show.equals("false"))
	{
		...
		{
			if(control.getType().equals("multilinetextbox") && control.getProperty("rows").equals("1"))
			{
				cssclass = cssclass.replaceAll("mltblu","");
			}
		...
		if(WebClientRuntime.isNull(src) || (src.indexOf(".gif")==-1 && src.indexOf(".jpg")==-1 && src.indexOf(".png")==-1)  && src.indexOf(".jpeg")==-1)
		{
			imageType = ".gif";
		}
		//IBMBIDI Start
		...
		//IBMBIDI End
		if(ariaHidden.equals("true")&&msg==""){
			ariaHidden = "role=\"presentation\"";
		}
		else
			ariaHidden = "";
		boolean hideDateLookup = Boolean.valueOf(WebClientRuntime.getWebClientSystemProperty(WebClientConstants.WEBCLIENT_SCREENREADER_HIDE_CALENDAR, "false")).booleanValue();
		if(!isDate || (!accessibilityMode || !hideDateLookup)){
			if(accessibilityMode && clickable)
			{
				tabindex="NONE";
			%><a id="<%=id%>" class="dbutton_anchor" <%
					if(comboBox)
					{
						%> tabindex="-1"<%
					}
					%> title="<%=msg%>" alt="<%=msg%>" <%=linkedcomponent%> onfocus="input_onfocus(null,this);"<%
					%> onkeyup="cancelEvent(event)"<%
					%> onkeydown="if(event.key === ' '){event.target.click()}"<%
                    %> onkeypress="if(event.key === ' '){event.target.click()}"<%
	        ...
	        String noBgHover = "";
	        if(wcs.getSkinName().indexOf("iot18")>-1){
	        	noBgHover = "no-bg-hover";
	        }
			%><img id="<%=id%><%if(accessibilityMode){%>_img<%}%>" <%=sf%> <%=ariaHidden%> <%=active%> ctype="image" alt="<%=msg%>"<%
					...
					%> <%=ariaRole%> <%=cssclass%> <%=align%> <%=width%> <%=height%> <%=longDesc%> <%=noBgHover%> <% 
					
					if(!WebClientRuntime.isNull(mxEventJSHandler))
					{ 
						%> mxejse="<%=mxEventJSHandler%>" <% 
					}
					if(!WebClientRuntime.isNull(extendedTooltip))
					{ 
			...
			if(accessibilityMode && clickable)
			{
			%></a><span class="dbutton_spacer" role="presentation" aria-hidden="true"></span><%
			}
			else if(src.equals("blank")){%>
	    ...
	    <% }   if(component instanceof Image && component.getProperty("iconmenu").length()>0) {
	            ...
	            if(iconCount > 0){
	                ...
	                %><a title="<%=icon.get("label")%>" href="Javascript: sendEvent('<%=icon.get("event")%>','<%=target%>','<%=icon.get("app")%>')" class="fieldIcon"  tabindex="<%=tabindex%>" onmouseover="appendClass(this,'fieldIcon_hover')" onmouseout="removeClass(this,'fieldIcon_hover')" ><img id="<%=id%>_<%=index%>" aria-hidden="true" src="<%=IMAGE_PATH%><%=imageName%>" border="0" /></a><%
	                }
	            }
	        }   
		}
