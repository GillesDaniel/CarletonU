...
else
{
	...
	<%="<![CDATA["%><table role="presentation" cellspacing="0" cellpadding="0" align="<%=defaultAlign%>" class="plinner">
			...
			{
			String portletControlId = portletControl.getId();
				if(allActions!=null)
				{
						...
						<tr>
							...
							<td  nowrap  align="<%=defaultAlign%>"  valign="top" style="vertical-align:top;">
								<div nowrap class="bc" style='display:inline;text-align:<%=rtl?reverseAlign:defaultAlign%>'>
			        				 <a ctype="label" id="QuickInsert_<%=appId%>" tabindex="0" href="Javascript: sendEvent('quickinsert','<%=portletControlId%>','<%=sb.toString()%>',3);" onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) { sendEvent('quickinsert','<%=portletControlId%>','<%=sb.toString()%>',3);}" class="text qil" title="<%= HTML.encodeTolerant(customDesc) %>" >
			        				 <img src='<%=IMAGE_PATH%><%=imgSb.toString() %>' <%=altString%> class="qim" onerror="this.src='action_insert_generic.gif'"/><%=customDesc%></a>
								</div>
							</td>
							<td class="prs"></td>
						</tr>
