...
String maskString = WebClientRuntime.getWebClientProperty("webclient.mask") ;
try {
	...
	else
	{
		...
		boolean errorCondition = portletControl.getErrorCondition();
		...
		boolean hideNonPersistPortletFilter=Boolean.valueOf(WebClientRuntime.getWebClientProperty("webclient.hideNonPersistPortletFilter","false")).booleanValue();
		...
		if((viewChanged || filterToggled || stateChanged) && !errorCondition)
			portletControl.setChangedFlag(false);
		if (errorCondition)
			portletControl.setErrorCondition(false);
				...
				|| (portletStateManager.isPortletLoaded() && component.hasChanged() && !stateChanged))
		{
			...
			else
			{
				...
				if( (precedeGraph.equals("true") || component.getProperty("showchart").equals("true") || portletStateManager.wasPortletLoadCalled() ) && groupByAttribute!=null)
				{
	...
	%>	<component vis="true"  ignrdispstyle="true" id="<%=id%>_holder" holder="<%=holderId%>" comptype="<%=component.getType()%>"><%="<![CDATA["%> 
				<div id='rsportletgraph_<%= layoutId %>' style='display:<%= precedeGraph.equals("true")?"block":"none" %>; position:relative; top:0px;'>
	<%					if(graphData != null)
					{
	...
	%>					<%--  portletinternalheader --%>
					<table role="presentation" width="100%" cellspacing="0" border="0" cellpadding="0" align="<%=defaultAlign%>" class="plinner">
						<tr>
							<td colspan=2>
	<%							if(portletControl.getProperty("readonlymode").equals("false") || portletControl.getDataBean().hasSigOptionAccess("SHOWPORTLETDETAILS"))
								{
	%>								<table role="presentation" width="100%"  cellspacing="0" cellpadding="0" align="center">
									<tr>
										<td class="pih" width="45%" nowrap="true" valign="middle" align="<%=defaultAlign%>">
										...
										<td class="pih" width="45%" align="<%=reverseAlign%>" valign="middle">
											...
											<%=HTML.encode(graphDetails.getProperty("attributename"))%></a>   
										</td>
									</tr>
								</table>
	<%								}
	...
	%>
						<%--  rsgraph --%>
						<tr>
							<td aria-hidden="true" <%if(column.equals("1")){%><%="width=\"60%\""%><%}%> valign="top" class="pgt"><%=HTML.encode(rsGraph.getTitle())%>
							<div id="rchart_<%=id%>" style="display:block;<%=chartstyle%>" dir="ltr"<%/*This is a hack but without it labels aren't aligned correctly in rtl*/%>></div>
							<script type="text/javascript">
								//dojo.require('layers.mbs.startcenter');
								dojo.require('ibm.tivoli.mbs.dojox.charting.Chart');
	...
	<%							if(chartData instanceof PieChartData)
							{
								...
								for(int i = start; i != end; )
								{
									...
									slice.put("tooltip", HTML.encode(item.tooltip));
									sliceData.add(slice);
									urls.add(item.url);
	...
	%>									var urls = <%=urls.serialize()%>;
									...
									chart.addSeries("Series A", <%=sliceData.serialize()%>);
									//APAR IV68095 - If height and width are not specified, graph might get cut off
									<% if (portletStateManager.isMaximised()) {%>
	...
	<%							}
							else if(chartData instanceof BarChartData)
							{
								...
								for(int i = 0; i < length; i++)
								{
									...
									bar.put("tooltip", HTML.encode(item.tooltip));
									barData.add(bar);
									urls.add(item.url);
									JSONObject xlabel = new JSONObject();
									xlabel.put("value", i + 1);
	...
	%>									var urls = <%=urls.serialize()%>;
									...
									chart.addAxis("x", {
										natural: true, labels: <%=xlabels.serialize()%>, majorLabels:true, minorLabels:true, 
									...
									chart.addSeries("Series 1" , <%=barData.serialize()%>);
									new dojox.charting.action2d.Tooltip(chart, "default");
	<%							}
	...
	%>						<br><span class="<%=textcss%> pnc" align="center" valign="top"><%=graphErrorMsg %></span><br>
						<span style="font-style:normal;text-align:left;padding-left:10px"><a tabindex="0" title="<%=labels[4] %>" class="<%=textcss%> pl" style="font-style:normal" onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) { sendEvent('changeview','<%= id %>','LIST');}"   href="javascript:sendEvent('changeview','<%= id %>','LIST')" <%=otherEvents %>><%=labels[4] %></a></span>
	<%					}
	%>				</div>
				<%--  end rsgraph --%>
	
				<%-- begin rsdata --%>
				<div id='rsportletdata_<%= layoutId %>' style='display:<%=(precedeGraph.equals("false"))?"inline":"none" %>; top:0px;'>
					<table width="100%" cellspacing="0" cellpadding="0" border="0" align="<%=defaultAlign%>" class="plinner">
	<%				String[] headings = new String[attributes.size()];	
	          if(data != null)
						{
	...
	<%								//display all column headings
								for(int i = 0;i < attributes.size(); i++)
								{
									...
									headings[i] = id+"_"+i;
	%>							<th role="columnheader" id="<%=headings[i]%>" class="<%=textcss%> khtc" nowrap="true"  <%= bAlign %>>
								&nbsp;
								<span>
	...
	<%									}
									...
									{
	%>										<a tabindex="0" title="<%=sortLabel%>" class="<%=textcss%> pl ibhc" onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) { sendEvent('sort','<%= id %>','<%=sortIndex%>');}"  href="javascript:sendEvent('sort','<%= id %>','<%=sortIndex%>')" <%=otherEvents %>>
	<%									}
									...
									out.print(WebClientRuntime.makesafevalue(finalData));
									%></a>
	...
	<%							//display all column headings
							ArrayList nonPersistentColumns = portletControl.getNonPersistentColumns();
	...
	%>								<td id="filter_cells_<%=layoutId%>_<%=i%>" class="rsfco" <%= bAlign %> headers="<%=id%>_<%=i%>">
									&nbsp;
									<span id="filter_dataspan_<%=layoutId%>_<%=i%>">
	<%									boolean hideFilterForNP = nonPersistentColumns!=null && nonPersistentColumns.contains(arrColumn[0]) && hideNonPersistPortletFilter;
										if(/*arrColumn[0].indexOf(".") == -1 &&*/ !hideFilterForNP)
										{
	...
	%>										<input
											...
											value="<%=WebClientRuntime.makesafevalue(portletControl.getQbeSetting(arrColumn[0])) %>"
											<%= bidiAttrs %>
											tabindex="0"
											title="<%= WebClientRuntime.makesafevalue(arrColumn[1]) %>"
											size="5"
											maxlength="254"/>
	<%										}
										else
	...
	%>							<td style="padding-<%=defaultAlign%>:5px" <%= bAlign %> headers="<%=id%>_<%=i%>">
								<a tabindex="0" title="<%=openRecordlbl %>" class="<%=textcss%> rsd <% if(keyValue != null) { out.println("pkr");%>" onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) {sendEvent('openrecord','<%= id %>','<%= appToGo%>-<%= keyValue.getDataAsLong() %>');}"  href="javascript:sendEvent('openrecord','<%=id %>','<%= appToGo%>-<%= keyValue.getDataAsLong() %>')" <% }%>  <%=otherEvents %>>
									...
									if(displayData!=null)
									{
										...
										BidiUtils.applyBidiAttributes(rInfo,arrColumn[0],displayData.getData(),langcode);
				 						int type = displayData.getTypeAsInt();
										String calType = control.getWebClientSession().getUserInfo().getCaltype();
	               		   	      		if (finalData!=null && finalData!="" && calType.equalsIgnoreCase("islamic"))
										{
											if(type==MXFormat.DATE){
											Date pDate = psdi.util.MXFormat.stringToDate(finalData,control.getWebClientSession().getUserInfo().getLocale());
											finalData = psdi.util.MXNFormat.dateToString(pDate,control.getWebClientSession().getUserInfo().getICULocale());
											}
	
											if(type==MXFormat.DATETIME){
											Date pDate = psdi.util.MXFormat.stringToDateTime(finalData,control.getWebClientSession().getUserInfo().getLocale());
											finalData = psdi.util.MXNFormat.dateTimeToString(pDate,control.getWebClientSession().getUserInfo().getICULocale());
											}
										}
										finalData = WebClientRuntime.makesafevalue(finalData);
										boolean shortenText = Boolean.valueOf(WebClientRuntime.getWebClientProperty("webclient.shortenportlettext","true")).booleanValue();
										if (shortenText || ismobile)
										{
										...
										{
											finalData=HTML.decode(finalData);
											finalData=WebClientRuntime.wrapText(finalData,"<br/>",wraplength);
											finalData=HTML.encodeTolerant(finalData);
										}
										if (displayData.isHidden())
										{
											finalData = maskString;
										}		
										%><%=finalData%><%
									}
									%></span>
