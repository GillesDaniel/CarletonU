...
--%><%@ include file="../common/simpleheader.jsp" %><%
	...
	String cssClass = component.getProperty("cssclass");
	boolean alignbottom = component.getProperty("alignbottom").equalsIgnoreCase("true");
	...
	if(component.needsRender())
	{
		...
		<td id="<%=id%>" aria-live="polite" <%=visible?"":"aria-hidden=\"true\""%> style="<%=width%>display:<%=display%>;<%=designerSelected%>;padding:0px 2px;vertical-align: top" sc="true" <%=componentEvents%> height="100%" class="<%if(designmode){%><%=designClass%> <%}%>sectioncol <%=cssClass%>">
			<div id="<%=id%>_div" <%if(!wcs.isPostTivoli09Skin()){%>style="height: 100%;<%if(alignbottom){%>display: inline;<%}else{%>  vertical-align: top;<%}%>"<%}%>>
