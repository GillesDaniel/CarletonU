...
<%			}
			if(exceptionType != ComponentInstance.EXCEPTION_NONE)
			{
      		...
      		<td valign="middle" class="cbtd"><%
					%>
              <div class="maximo-toggle-button-outer">
                <div id="<%=id%>" <%=ariaLabel%> <%=exc%>
               	<% 
                 if(mxEventJSHandler.length() != 0)
                  ...
                  if(component.isAsyncEnabled())
                    ...
                    ctype="togglebutton" role="checkbox" class='maximo-toggle-button' tabindex="0" aria-checked="<%=checked.equals("checked")%>" aria-readonly="<%=isReadOnly%>">
                  <div></div>
                </div>
              </div>
              <%
