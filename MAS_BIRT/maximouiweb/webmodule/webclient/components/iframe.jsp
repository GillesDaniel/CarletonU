...
<%	}
	else 
	{
		...
		{ 	
			...
			String servExt = ((servPath.indexOf("?")==-1)?"?":"&")+"controlId="+control.getId()+"&componentId="+component.getId()+"&"+wcs.getUISessionUrlParameter();
		...
		<div id="<%=id%>_iframe_div" <%=divAutoId%> style="display:inline" align="<%=defaultAlign%>" valign="top">	
			<iframe id="upload_iframe" title="<%=title%>" allowtransparency="true" <%=iframeAutoId%> class="<%=cssclass%>" role="presentation" tabindex="0" src="<%=servPath%><%=servExt%>" height="<%=height%>" width="<%=width%>" scrolling="<%=scroll%>" frameBorder="0" marginHeight="0" marginWidth="0" <%=componentEvents%>  style="<%=style%>" > </iframe>
	     </div>
<%		}
