...
//Alts and Messages
String browser = wcs.getUserAgentName();
...
String tabIndex = "-1";
boolean currentlySelected = (selectedNode!=null && selectedNode.getRenderId().equals(control.getRenderId()));
...
{		
	if(!parentNode.needsRender() && (parentNode instanceof TreeNode))
	{
        ...
        {        	
        	holderId = parentNode.getRenderId()+"-trn_children";	%>
<%@ include file="../common/componentholder_start.jsp" %>
	...
	if(treeControl.getId().contains("associateClassification_tree") || treeControl.getId().contains("searchClassification_tree"))
	{
	...
	String topULId = treeControl.getTopULId();
%><li role="presentation" id="<%=controlId%>_node" <%=automationId%> class="<%= nodeClass%>" style="background:transparent;" nodeinfo='{"selected": <%=currentlySelected%>, "expanded":<%=expanded%>, "toggleevent": {"eventtype":"<%=toggleEvent%>","targetid":"<%=id%>","value":"<%=unformattedKeyValue%>"}, "primaryevent" : <%=primaryEvent.serialize()%>, "secondaryevent" : <%=secondaryEvent.serialize()%>}' async="0" ae="togglechildren" fldInfo="{eventpriority:2}">
		<div id="<%=id%>" tabindex="<%=tabIndex%>" style="display:inline;border:1px solid transparent;" onblur="this.style.border='1px solid transparent';" onfocus="this.tabindex='0';setFocusId(event,this); focusTreeNode('<%=id%>', '<%=topULId%>'); this.style.border='1px solid #6699cc';"  <%if(nodeHasChildren){%>aria-expanded="<%=expanded%>"<%}%> aria-labelledby="<%=controlId%>_nodename_link" role="treeitem"><span role="presentation" style="vertical-align:middle;background:transparent"><%
		...
		if(!isfakenode)
		{
			if(displayNodeImage)
			{
	        %><a role="presentation" tabindex="-1" id="<%=controlId%>_displayimage_link" class="ni" <%=nodeEvents%> onkeypress='if(hasKeyCode(event,"KEYCODE_ENTER")) {processEvent(<%=nodeImageEventJson.serialize()%>);}' href='javascript: focusTreeNode("<%=id%>", "<%=topULId%>"); processEvent(<%=nodeImageEventJson.serialize()%>)'><img alt="" role="presentation" id="<%=controlId%>_displayimage_image" align="absmiddle" src="<%=IMAGE_PATH+nodeImage%>" /></a><%
			}
			if(enableReturn && handleAction)
			{
			%><a role="presentation" tabindex="-1" id="<%=controlId%>_nodeimage_link" class="ni" <%=nodeEvents%> href='javascript: focusTreeNode("<%=id%>", "<%=topULId%>"); processEvent(<%=nodeEventJson.serialize()%>)' onkeypress='if(hasKeyCode(event,"KEYCODE_ENTER")) {processEvent(<%=primaryEvent.serialize()%>);}'><img border="0" role="presentation" alt="<%=selectRecordlbl +" "+ HTML.encodeTolerant(nodeLabel)%>" align="absmiddle" id="<%=id%>_select_image" <%=automationIdSelectImage%> cell_uid="<%=objectname+":"+keyValue%>" src="<%=IMAGE_PATH+selectImage%>" /></a><%
			}
		}
		if(enableSelectNode)
		{
			if(!isfakenode)
			{
				%><a  role="presentation" tabindex="-1" class="<%=nodeLabelClass %>" id="<%=controlId%>_nodename_link" <%=nodeLinkTextDecoration %> <%=nodeEvents%> href='javascript: focusTreeNode("<%=id%>", "<%=topULId%>"); processEvent(<%=primaryEvent.serialize()%>)'><%=HTML.encodeTolerant(nodeLabel)%></a><%
			}
			else
			{
		%><span tabindex="-1" class="<%=nodeLabelClass %>" id="<%=controlId%>_nodename_link" style="text-decoration:none"><%=HTML.encodeTolerant(nodeLabel)%></span><%
			}
		}
		else
		{
		%><span tabindex="-1" class="<%=nodeLabelClass %>" id="<%=controlId%>_nodename_link" <%=nodeLinkTextDecoration %>><%=HTML.encodeTolerant(nodeLabel)%></span><%	
		}	
		%></div>
	<%	if(nodeHasChildren) 
		{
			//	Chrome - in chrome the "display" attribute causes issues when expanding a treenode. Use margin-left instead.
			//	For later processing below and in Async.js, attach a maxtype="treenode" attribute to this element to identify it a Chrome treenode.
			if(browser.equals("CHROME")) {
				%><ul role="group" maxtype="treenode" aria-live="polite" id="<%=controlId%>_children" class="nc" takesfocus=true  style="list-style:none;<% if(expanded) out.print("display:inline"); else out.print("margin-left: -30px"); %>"><%
			}
			else {
...
%>			
		...
		<component id="<%=id%>_holder"<%=compType%>><![CDATA[<script>
			...
			var otn = getElement("<%=parentNode.getRenderId()%>-trn_children");
			scrollToMiddle(treeDiv,otn);
	
			var nodeImage = getElement("<%=parentNode.getRenderId()%>-trn_state_image");
...
<component id="<%=id%>_holder"<%=compType%>><![CDATA[<script>
			...
			{			
				...
				//	If maxtype=treenode, then we're in Chrome and we won't do the display=none, but
				//	use margin-left, which was set above
				var elementtype = outer.getAttribute("maxtype");
				if(undef(elementtype) || elementtype != "treenode") {
					hideShowElement(outer,false);
				}
			}
			var nodeImage = getElement("<%=control.getRenderId()%>-trn_state_image");
			if(!undef(nodeImage))
			{				
...
<%
		...
		if(((enableReturn && handleAction) || enableSelectNode) && selectedNode!=null && selectedNode.getRenderId().equals(control.getRenderId()))
		{			
...
%>
<component id="<%=id%>_holder"<%=compType%>><![CDATA[<script>
				var previousNode = getElement("<%=lastSelectedNodeId+"-trn_nodename_link"%>");
