...
--%><%@ include file="../common/simpleheader.jsp"%><%
	...
	if( !WebClientRuntime.isNull(popme) && popme.equals("true") )
	{
		...
		%><%@ include file="../common/componentholder_start.jsp" %>
			...
			if(dialogToRemove)
			{
				dojo.publish("hidedialog",[{"id":"<%=id%>_inner"}]);
...
<%		String innerdialogwaitAutoId="";
		...
		String dialogLabel = control.getProperty("label");
...
<%		}	
				...
				if (children!=null)
				{
%>				<table id="<%=id%>_inner" <%=innerAutoId%> aria-labelledby="<%=id%>_inner_content_<%=ctr%>" role="dialog" tabindex="-1" aria-label="<%=dialogLabel%>" positionAtTop="<%=positionAtTop%>" positionX="<%=positionX%>" positionY="<%=positionY%>" modal="<%=modal%>" width="<%=width%>" height="<%=height%>" class="dsc1 mc<%if(modal){%> modal<%}%>" border="0" defaultbutton="<%=defaultbutton%>" control="true" <%=control_events%> style="visibility:hidden;" cellpadding="0px" cellspacing="0px">
...
%>			<tr id="<%= id + DIALOG_CONTENT_SUFFIX + ctr%>" aria-live="polite" <%=collapsed?"aria-hidden=\"true\"":""%> <%=dialogcontentsuffixAutoId%> style="<%= dialogState%>" valign="top">
				<th id="<%=id%>_inner_content_<%=ctr%>" <%=innercontentAutoId%> valign="BOTTOM"  style="padding:0px;<%if(accessibilityMode){%>background:#ffffff;border:3px solid #fff;<%}%>">
<%					if(ctr==0)
					{	//don't output whitespace around the following image
						%><img role="presentation" tabindex="0" alt="" id="<%=control.getId()%>_first" alt="" onfocus="moveable_onfocus(this);" src="<%=IMAGE_PATH%>blank.gif" width="1" height="1" style="display:block;margin:0px" firstbuttonid="first"/><%
					}
					child.render();
					if(ctr>0)
					{
						...
						%><img role="presentation" tabindex="0" labelid="<%=labelid%>" alt="" id="<%=control.getId()%>_last" onfocus="moveable_onfocus(this);" src="<%=IMAGE_PATH%>blank.gif" width="1" height="1" lastbuttonid="<%=lastbutton%>" style="position:absolute; margin: 0px; height: 1px; width: 0px"><%
					}
				%></td>
<%				ctr++;
%>			</tr>
...
%>			updateAutoFill("<%=control.getPage().getPageAutoFillId()%>",<%=autoFillInfo.serialize()%>);
<%			}
%>			addLoadMethod("_initDialog('<%=id%>','<%=control.getId()%>',<%=max%>,<%=modal%>)");
				<%if(!accessibilityMode){%>
        addLoadMethod("window.setTimeout(\"focusItem('<%=(String)app.getCurrentPage().getFocusRenderId()%>',true)\", 200);");
        <%}%>
<%			if(serverMessage != null && serverMessageType != "")
			{
%>			showMessage(<%=serverMessageType%>, "<%=serverMessage%>", <%=dialogMessage%>);
