...
String USERAGENTVERSION = request.getHeader("User-Agent");
String position = "position:initial";
if(USERAGENTVERSION.toUpperCase().indexOf("MSIE 8")>-1)
...
<%								}
%>							</td>
							<td width="40%" align="<%=reverseAlign%>" valign="middle" class="pih" style="text-align: <%=reverseAlign%>">
...
%>								<td  align="center">
									<a tabindex="0" title="<%=labels[11] %>" class="text" onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) {sendEvent('openrecord','<%=id %>',<%= kpi.get("kpiuid") %>);}"  href="javascript:sendEvent('openrecord','<%=id %>',<%= kpi.get("kpiuid") %>)">
										<img alt="<%= HTML.encodeTolerant(kpi.get("kpiname").toString())+" - "+labels[9]+":"+kpi.get("lastupdated") %><%= ", "+labels[10]+":"+portletControl.formatNumber(kpi.get("lastkpivalue")) %>" style="background-color:#<%=statusColor%>" src="<%=IMAGE_PATH + trend.image()%>" border="0" />
									</a>
								</td>
...
<%				} //!displayList
				...
				if(portletStateManager.wasPortletLoadCalled() || (portletStateManager.isPortletLoaded() && component.hasChanged()))
				{
					...
					if (noOfKpis == 1)
					{
						String kpititle = kpiGraph.getTitle();
						String whitechar = "";
						
						if (wraplength == -1) {
							whitechar = "style=\"white-space : normal !important;\"";
						}
%>				<tr>	
					<td id="kpigraphtitle_<%=layoutId %>" <%=whitechar%> valign="middle" colspan="2" class="kgt">
<%
						if (shortenText)
						{
							kpititle = WebClientRuntime.shortenString(kpititle,35);
						}
						
						if (wraplength > 0) {
							 kpititle = HTML.decode(kpititle);
							 kpititle = WebClientRuntime.wrapText(kpititle,"<br/>",wraplength);
						}
%>					<%= HTML.encodeTolerant(kpititle)%>
<%					}
				}
%>				</td>
			</tr>
			<tr>
				<td style="<%=position%>" valign="middle" width="40%" aria-hidden="true">
...
<%							}
							else if(chartData instanceof BarChartData)
							{
								...
								for(int i = 0; i < length; i++)
								{
									...
									bar.put("tooltip", WebClientRuntime.makesafevalue(item.tooltip));
									barData.add(bar);
									urls.add(item.url);
...
%>
								document.<%=methodName%> = function()
								{
									...
									chart.addAxis("x", {natural: true, labels: <%=xlabels.serialize()%>});
									...
									{
										if(evt.type == "onclick" && barurls[evt.index] != "")
										{
											warnExit = false;
...
<%				}
				...
				if(displayList)
				{
%>					<td nowrap style="padding-<%=defaultAlign%>:1px" valign="top">
					<table border="0" width="100%" cellspacing="0" cellpadding="0" <%if(accessibilityMode){%>title="<%=control.getProperty("label")%>" summary="<%=control.getProperty("label")%>"<%}%>>
						<tr>
<%							if(portletControl.isLinkedToReport())
							{
%>							<th id="<%=id%>_col1" class="text khtc" style="padding-<%=defaultAlign%>:2px;padding-<%=reverseAlign%>:2px;text-align:center"><%= labels[12] %></th>
<%							}
							if(portletControl.isLinkedToKPI())
							{
%>							<th id="<%=id%>_col2" class="text khtc" style="padding-<%=defaultAlign%>:2px;padding-<%=reverseAlign%>:2px;text-align:center"><%= labels[13] %></th>
<%							}
%>							<th id="<%=id%>_col3" class="text khtc" style="padding-<%=defaultAlign%>:2px;padding-<%=reverseAlign%>:2px;text-align:center" nowrap><%=labels[3]%></th>
							<th id="<%=id%>_col4" class="text khtc" style="padding-<%=defaultAlign%>:2px;padding-<%=reverseAlign%>:2px" nowrap><%=labels[8]%></th>
							<th id="<%=id%>_col5" class="text khtnc" style="padding-<%=defaultAlign%>:2px;padding-<%=reverseAlign%>:2px" nowrap><%=labels[5]%></th>
							<th id="<%=id%>_col6" class="text khtnc" style="padding-<%=defaultAlign%>:2px;padding-<%=reverseAlign%>:2px" nowrap><%=labels[6]%></th>
							<th id="<%=id%>_col7" class="text khtnc" style="padding-<%=defaultAlign%>:2px;padding-<%=reverseAlign%>:2px" nowrap><%=labels[7]%></th>
						</tr>
<%
					for(int i = 0 ;i < noOfKpis; i++)
					{
...
<%							if(portletControl.isLinkedToReport())
							{
%>							<td headers="<%=id%>_col1" align="center">
<%								if(kpi.get("reportnum") !=null && !kpi.get("reportnum").toString().equals("") )
								{
%>								<a tabindex="0" title="<%=labels[0] +": "+   kpi.get("reportdesc") %>" class="text" onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) {sendEvent('openreport','<%=id%>','<%= kpi.get("reportnum") %>');}" href="javascript:sendEvent('openreport','<%=id%>','<%= kpi.get("reportnum") %>')">
									<img alt="<%=labels[0] +": "+  kpi.get("reportdesc") %>" src="<%= IMAGE_PATH %>btn_report_link.gif" border="0"/>
...
<%							}
							if(portletControl.isLinkedToKPI())
							{
%>							<td headers="<%=id%>_col2" align="center">
<%								if(kpi.get("linkto")!=null && !kpi.get("linkto").toString().equals(""))
								{
%>								<a tabindex="0" title="<%=labels[1] +": "+  kpi.get("linktodesc") %>" class="text" onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) {sendEvent('openrecord','<%=id%>','<%= kpi.get("linkto").toString() %>');}" href="javascript:sendEvent('openrecord','<%=id%>','<%= kpi.get("linkto").toString() %>')">
									<img alt="<%=labels[1] +": "+  kpi.get("linktodesc") %>" src="<%= IMAGE_PATH %>btn_kpi_link.gif" border="0"/>
...
%>							<td headers="<%=id%>_col3" align="center">
...
<%							String kpiname = !isBidiEnabled? kpi.get("kpiname").toString() : kpi.get("kpinamebidi").toString();  //bidi-hcg-SC
							...
							if (wraplength == -1) {
								whitespace = "style=\"white-space : normal !important;\"";
							}
							
%>							<td headers="<%=id%>_col4" <%=whitespace%> class="text <%=tdClass%> kdb">
<%
								if (shortenText)
								{
									kpiname = WebClientRuntime.shortenString(kpiname,35);
								}
								
								if (wraplength > 0) {
									 kpiname = HTML.decode(kpiname);
									 kpiname = WebClientRuntime.wrapText(kpiname,"<br/>",wraplength);
								}
%>
								<%= rtlPercentMark %><%= HTML.encodeTolerant(kpiname) %><%= sPercentMark %>
							</td>
							<td headers="<%=id%>_col5" class="text <%= tdClass %>" <%= dir %>>
								<%= portletControl.formatNumber(kpi.get("kpivalue")) %>
							</td>
							<td headers="<%=id%>_col6" class="text <%= tdClass %>" <%= dir %>>
								<%= portletControl.formatNumber(kpi.get("target")) %>
							</td>
							<td headers="<%=id%>_col7" class="text <%= tdClass %>" <%= dir %>>
								<%= portletControl.formatNumber(kpi.get("variance")) %>
							</td>
						</tr>
<%					} //end for
