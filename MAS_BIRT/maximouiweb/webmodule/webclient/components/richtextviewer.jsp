...
--%><%@ include file="../common/simpleheader.jsp" %><%
		...
		if (control.getBoolean("ontabledetails")) {
		...
		if (!designmode)
		{
			...
			int updateDelay = wcs.getOriginalEvent().getType().equalsIgnoreCase("loadapp")?800:100;
...
%>				<span class="<%=textcss%>">
					...
					<script>
						setTimeout(updateRichTextViewer, <%=updateDelay%>, '<%=id%>', '<%=value%>', true, <%=rtl%>);
					</script>
				</span>
...
%>					<script>
						...
						cssLink.href = "<%=servletBase%>/<%=skin%>css/rte.css"; 
						cssLink .rel = "stylesheet"; 
						cssLink .type = "text/css"; 
						document.getElementById("<%=id%>").contentDocument.head.appendChild(cssLink);
					</script>
<% 				} 
			}
			else
			{
%>			<script>
				setTimeout(updateRichTextViewer, <%=updateDelay%>, '<%=id%>', '<%=value%>', false, <%=rtl%>);
			</script>
<%			}
		}
		else
