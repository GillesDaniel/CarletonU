...
String topULId = id+"_tree";
if(component.needsRender())
{
	treeControl.setTopULId(topULId);
	//Attributes specified for this control (in xml)
	String treeheight = component.getProperty("height");
	treeheight = component.getWebClientSession().attachUOM(treeheight);
	String treewidth = component.getProperty("width");
	treewidth = component.getWebClientSession().attachUOM(treewidth);
	...
	<%}%>
			<div control="true" align="<%=defaultAlign%>"  class="tcont <%=cssclass%>" <%if(!ismobile){%>style="<%=treeheight%><%=treewidth%>overflow:auto;"<%}%>>
				<div id="<%=treeControl.getId()+"_treecontainer"%>">
					<ul onkeydown="treeKey(event, this)" role="tree" id="<%=topULId%>" <%=treeAutomationId%> nowrap="true" align="<%=defaultAlign%>" class="tcontr" style="margin:2px;padding:2px;list-style:none;">		
...
else
{
	...
	%><%@ include file="../common/componentholder_start.jsp" %>
			<ul onkeydown="treeKey(event, this)" role="tree" id="<%=topULId%>" <%=treeAutomationId%> nowrap="true" align="<%=defaultAlign%>" class="tcontr <%=cssclass%>" style="margin:2px;padding:2px;list-style:none;">			
<%		
			component.renderChildrenControls();
%>
			</ul>	
