...
--%><%@ include file="../common/simpleheader.jsp" %><%
	...
	boolean IsLabelledByIdExists = true;
	...
	while (i.hasNext())
	{
		...
		if(component.needsRender())
		{
			...
			<script>
				elLabel = document.getElementById("<%=labelledById%>");
				if(!elLabel)
				{ 
					<% IsLabelledByIdExists = false; %>
				}
			</script>

	<td id="<%=id%>" aria-live="polite" <%if(!visible){%>aria-hidden="true"<%}%> <%if(!WebClientRuntime.isNull(labelledById) && IsLabelledByIdExists){%> aria-labelledby="<%=labelledById%>"<%}%> <%=automationId%> <%=componentEvents%> <%=align%> style="<%=padding%>display:<%=display%>;white-space:nowrap;padding-bottom: 2px;" class="<%=cssclass%> <%if(sectionBorder){%>sectionb<%}%>" valign="top" <%=height%> ><%
		}
...
<%		}
		control.setFocusable(Boolean.valueOf(expanded) && ((ControlInstance)control.getParentInstance()).isFocusable() );
		child.render();
		if(component.needsRender())
		{
	%></td><%
				...
				try
				{
					...
					if(so && "<%=display%>" == "none")
					{
						...
						if(par.getAttribute("sc") == "true")
							par.style.width = par.clientWidth + "px";
					}
				}
				catch(error)
				{
...
<%	}
	...
	{
%>			
		<script>addLoadMethod("hideShowElement('<%=id%>','<%=display%>')");</script>
<%	}
