...
if(component.getProperty("showheader").equals("true"))
{
	...
	%>
	<table role="presentation" id="<%=id%>" <%=graphhdrAutoId%> class="<%=headerclass%>" width="100%" style="background-color: #EDEEED" cellspacing="0" cellpadding="0" align="center">
		<tr>
			<%if(!sReportName.equals("") || !sLinkedKPI.equals("")){%>
				<td width="10%" valign="bottom" nowrap="true">
					<%if(!sReportName.equals("")){%>
						<a title="<%=labels[0] +": "+ htKPIData.get("reportdesc") %>" class="text" tabindex="0" onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) {sendEvent('openreport','kpi','<%=sReportName %>');}" href="javascript:sendEvent('openreport','kpi','<%=sReportName %>')">
							<img alt="<%=labels[0] +": "+htKPIData.get("reportdesc") %>" src="<%= IMAGE_PATH + reportImage %>" border="0" />
						</a>
					<%}%>
					<%if(!sLinkedKPI.equals("")){%>
						<a title="<%=labels[1] +": "+htKPIData.get("linktodesc") %>" class="text" tabindex="0" onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) {sendEvent('openlinkedkpi','kpi','<%=sLinkedKPI %>');}" href="javascript:sendEvent('openlinkedkpi','kpi','<%=sLinkedKPI %>')">
							<img alt="<%=labels[1] +": "+htKPIData.get("linktodesc") %>" src="<%= IMAGE_PATH + kpiImage %>" border="0" />
						</a>
					<%}%>	
			    </td>
			<%}%>
			<td  id="<%=id%>_lsKPIDate" width="45%" class="<%=textcss%> kub" <%=dojoTypeInput%>>
	  		...
	  		<td width="45%" class="<%=textcss%> kub" align="<%=reverseAlign%>">
  				<a tabindex="0" class="pl" onclick="setCurrentfocusFromId(null,'<%=id%>')" onkeypress="if(hasKeyCode(event,'KEYCODE_ENTER')) {sendEvent('updatekpis','kpi','kpi');}" href="javascript:sendEvent('updatekpis','kpi','kpi');"><%= labels[3] %></a>
	  		</td>
		</tr> 
		<tr>
			<td colspan="3">
