...
if(component.needsRender())
{
	...
	if (mboName == null || mboName.isEmpty())
	{
		...
		{
			itemHotKey = "hotkey='"+itemActionKey.charAt(itemActionKey.length() - 1)+"'";
...
%>			<li id="<%=id%>" <%=itemHotKey%> role="presentation"  <%if(disabled){%> disabled="true" aria-disabled="true"<%}%>><%
				%><a id="<%=id%>_text" role="button" onkeydown="itemAKey(event,this)" <%=automationId%><%
						%><%=itemHotKey%>  onfocus="appendClass(this,'onhover')" onblur="removeClass(this,'onhover')" <%
						%><%if(disabled){%> disabled="true"<%}%> class="<%=textcss%> <%=cssclass%> <%if(!disabled){%>on<%}else{%>off<%}%>"<%
							if(!designmode)
							{
				...
				if(type.equalsIgnoreCase("HEADER"))
				{
					...
					%><a id="<%=id%>_dropdown" onkeydown="itemAKey(event,this)" lc="<%=id%>"<%
							%> onfocus="appendClass(this,'onhover')" onblur="removeClass(this,'onhover')" <%
							...
							%>><%
						%><img id="<%=id%>_dropdown_menuarrow" <%=menuAutomationId%> alt="<%=itemTitle%> <%=wcs.getMessage("ui","menu")%>" src="<%=IMAGE_PATH%>atb_popoutmenu.gif" /><%
					%></a><%
				}
			%></li>
<%		}
		...
		else if((buttonType.equalsIgnoreCase("toolbar") || buttonType.equalsIgnoreCase("dyntoolbarmenu") || buttonType.equalsIgnoreCase("dyntoolbar")) && (text != null && !text.isEmpty()))
		{
			...
			String tabIndex = "tabindex=\"-1\"";
			if(!disabled && ((ToolbarButton)control).isFirstButton()) {
				tabIndex = "";
			}
...
%>			<li role="presentation"<%=className%> id="<%=id%>" <%=itemHotKey%> ctype="toolbarbutton" <%if(disabled){%>disabled="disabled"<%}%>><%
				%><a role="button" id="<%=id%>_anchor" href='javascript:<%=href%>' <%=automationId%> onkeydown="itemAKey(event,this)" title="<%=HTML.encode(itemTitle)%>"<% 
						%><%=itemHotKey%> onfocus="appendClass(this,'onhover')" <%=tabIndex%> onblur="removeClass(this,'onhover')" <%
						%><%if(disabled){%> disabled="true" <%}%> eventtype="<%=mxevent%>" class="<%if(!disabled){%>on<%}else{%>off<%}%>" ><%
					%><img role="button" tabindex="-1" id="<%=id%>_image" src="<%=IMAGE_PATH%><%=image%>" alt="<%=HTML.encode(itemTitle)%>"/><%
				%></a><%
			%></li>
		<% 	if(parentControl instanceof psdi.webclient.controls.ToolbarActions){
				((ToolbarActions)parentControl).addrenderScript("fixToolbarButtons(\"li[eventtype='"+mxevent+"']\", \""+id+"\");"); 
			}
		}
	}
