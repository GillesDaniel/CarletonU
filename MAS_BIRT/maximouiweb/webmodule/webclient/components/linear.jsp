...
//width = component.getWebClientSession().removeUOM(width);
String styleWidth = component.getWebClientSession().attachUOM(width);
String height = component.getProperty("height");
//height = component.getWebClientSession().removeUOM(height);
String styleHeight = component.getWebClientSession().attachUOM(height);

if(designmode)
{	
	if(false) //remove this if we want to show the control in designer
	{
%>	<div id="<%=id%>" class="sdm" style="width:<%=styleWidth%>;height:<%=styleHeight%>;"><%=control.getLocalizedType()%></div>
<%	}
}
else
{
