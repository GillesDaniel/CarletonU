...
define([
    ...
    return declare([GanttWidget, _MaximoIO, Handlers, WorkView], {
        ...
        OnFilterEnd: function(/*TGrid*/ grid, /*int*/ type) {
            ...
            if (clearFilters) {
                ...
                this.visitVisibleChiden(state, function (row, state) {
                    if (row && row['_OBJECTNAME'] === 'ASSIGNMENT') {
                        state.count++;
        ...
        OnShowMaximoDialog: function (/*TMenuItem*/item) {
        	this.grid.ActionBlur();
        	this._focusGanttObject = null;
        	var element = document.getElementById("SystemNavAppContent-sc");
        	if (element != null) {
        		element.click();
        	}
        	this.inherited(arguments);
        ...
        OnRenderFinish: function (/*TGrid*/ grid) {
            ...
            if (this.warningMessage != null) {
            	//"Disable color usage by turning off the system property: shift.enable.colors or define more colors for the other system property: skd.shift.colors."
            	this.grid.ShowMessageTime(this.warningMessage, 0, null, [this.buttonLabel]);
            }
        }
    });
