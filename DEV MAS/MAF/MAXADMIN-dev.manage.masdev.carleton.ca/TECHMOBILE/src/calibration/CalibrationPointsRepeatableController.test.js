...
describe("CalibrationPointsRepeatableController", () => {
  ...
  describe("pageResumed", () => {
    it("Should set page state variables when page is resumed", async () => {
      ...
      // Assert
      expect(page.state.padding).toEqual(PageConstants.PADDING.SMALL);
      ...
      expect(page.state.colwidth).toEqual(PageConstants.COLUMN_WIDTH.LARGE);
    });

    it("Should load state data when page is resumed", async () => {
      // Arrange
      const app = await baseSetup();
      const page = app.findPage("calpointrepeatable");
      const controller = page.controllers[0];

      controller.loadStateData = jest.fn();

      // Act
      controller.pageResumed(page, app);

      // Assert
      expect(controller.loadStateData).toHaveBeenCalled();
    });
    ...
    it("Should instatiate helpers when page is resumed", async () => {
      ...
      controller.loadStateData = jest.fn();
      controller.getAssetFunctionDS = jest.fn(() => ({ currentItem: true }));
      controller.getCalibrationPointsDS = jest.fn();
      controller.loadDsConfig = jest.fn(() => ({ currentItem: true }));
      controller.createGroupedCalpointsDS = jest.fn(() => ({
        currentItem: true,
      }));
      ...
      // Assert
      expect(controller.datasheetCalculation).toBeInstanceOf(DatasheetCalculation);
    });
    ...
    it("Should group calibration points when page is resumed", async () => {
      ...
      // Assert
      expect(controller.createGroupedCalpointsDS).toHaveBeenCalled();
    });
  });
  ...
  describe("setLabelColor", () => {
    it("shouldReturnRedWhenStatusIsFail", async () => {
      ...
      const app = await baseSetup();
      const page = app.findPage("calpointrepeatable");
      const controller = page.findController((c) => c instanceof CalibrationPointsRepeatableController);
  ...
  describe("getAssetFunction", () => {
    it("shouldReturnAssetFunction", async () =>{
      // Arrange
      const app = await baseSetup();
      const page = app.findPage("calpointrepeatable");
      const controller = page.findController((c) => c instanceof CalibrationPointsRepeatableController);

      page.state.assetfunction = "something"

      // Act
      const assetfunction = controller.getAssetFunction(DatasheetConstants.STATUS_BROKEN);

      // Assert
      expect(assetfunction).toEqual(page.state.assetfunction);
    })
  });
  ...
  describe("createGroupedCalpointsDS", () => {
    it("Should return grouped calibration points when calibration point is passed", async () => {
      ...
      controller.datasheetCalculation = new DatasheetCalculation(dsconfig.currentItem);
      controller.pageInitialized(page, app);
      controller.getCondition = jest.fn(() => CalibrationPointConstants.CONDITION_ASFOUND);
  ...
  describe("initializeGroupedCalpointsDS", () => {
    it("Should return datasource with calibration points grouped by attribute 'point'", () => {
      ...
      const hashSet = {
        ...
        3: [createCalpoint({ pluscwodspointid: 1013, point: "3", pointdescription: 'calpoint3' })],
      };
      ...
      groupedCalpointsDS.items.forEach((group, index) => {
        ...
        expect(group.pointdescription).toEqual(
          hashSet[index][0].pluscwodspointid + ` ${hashSet[index][0].pointdescription || ''}`
        );
  ...
  describe("findPointDescription", () => {
    it("shouldReturnPointDescriptionOfFirstItemInHashset", async () => {
      ...
      const calpointsHashSet = {
        "1": [
          createCalpoint({ point: "112", pointdescription: "Calibration point" }),
          createCalpoint({ point: "113", pointdescription: "Something something" }),
        ]
      }
      ...
      // Act
      const description = controller.findPointDescription(calpointsHashSet, key);
  ...
  describe("changeCalpoint", () => {
    it("shouldNotPerformCalculationWhenFormErrorExist", async () => {
      ...
      const assetfunction = createAssetfunction({ pluscwodsinstrid: "11", plantype: CalibrationPointConstants.PLANTYPE.ANALOG });
      const calpoint = createCalpoint({ pluscwodspointid: "112", asfoundinput: "100.60", asfoundoutput: "3500.100", isaverage: false, _group_index: 0});
        ...
        items: [
          {
            calpointsDS: {
              items: [
                calpoint,
                createCalpoint({ pluscwodspointid: "113", asfoundinput: "XXXXXX", asfoundoutput: "3500.100", isaverage: false }),
                createCalpoint({ pluscwodspointid: "114", asfoundinput: "100.06", asfoundoutput: "3500.102", isaverage: false }),
                createCalpoint({ pluscwodspointid: "115", asfoundinput: "100.66", asfoundoutput: "3500.103", isaverage: true }),
              ]
            }
          }
        ]
    ...
    it("shouldPerformCalculationsWhenFormIsValid", async () => {
      ...
      // Arrange target objects
      const assetfunction = createAssetfunction({ pluscwodsinstrid: "11", plantype: CalibrationPointConstants.PLANTYPE.ANALOG });
      const calpoint = createCalpoint({ pluscwodspointid: "112", asfoundinput: "100.123", asfoundoutput: "3500.100", isaverage: false, plantype: CalibrationPointConstants.PLANTYPE.ANALOG, _group_index: 0});

      // Arrange state variables
      page.state.assetfunction = assetfunction;
      page.state.condition = CalibrationPointConstants.CONDITION_ASFOUND;
      page.state.dsconfig = { currentItem: createDsConfig({ pluscdsconfigid: "601", stddev: DatasheetConstants.DEVIATION_N }) };
      page.state.groupedCalpointsDS = {
        items: [
          {
            avgpoint: {},
            calpointsDS: {
              items: [
                calpoint,
                createCalpoint({ pluscwodspointid: "113", asfoundinput: "100.231", asfoundoutput: "3500.100", plantype: CalibrationPointConstants.PLANTYPE.ANALOG, isaverage: false }),
                createCalpoint({ pluscwodspointid: "114", asfoundinput: "100.312", asfoundoutput: "3500.102", plantype: CalibrationPointConstants.PLANTYPE.ANALOG, isaverage: false }),
                createCalpoint({ pluscwodspointid: "115", asfoundinput: "100.123", asfoundoutput: "3500.103", plantype: CalibrationPointConstants.PLANTYPE.ANALOG, isaverage: true }),
              ]
            }
          }
        ]
  ...
  describe("validateCalpoint", () => {
    it("shouldDisplayFieldErrorsWhenCalpointIsInvalid", async () => {
      ...
      // Arrange target objects
      const assetfunction = createAssetfunction({ pluscwodsinstrid: "11", plantype: CalibrationPointConstants.PLANTYPE.ANALOG });
      const calpoint = createCalpoint({ pluscwodspointid: "112", asfoundinput: "XXXXXX", asfoundoutput: "3500.100", isaverage: false, _group_index: 0});
        ...
        items: [
          {
            calpointsDS: {
              ...
              items: [
                calpoint,
                createCalpoint({ pluscwodspointid: "113", asfoundinput: "100.60", asfoundoutput: "3500.100", isaverage: false }),
                createCalpoint({ pluscwodspointid: "114", asfoundinput: "100.06", asfoundoutput: "3500.102", isaverage: false }),
                createCalpoint({ pluscwodspointid: "115", asfoundinput: "100.66", asfoundoutput: "3500.103", isaverage: true }),
              ]
            }
    ...
    it("shouldNotDisplayFieldErrorsWhenCalpointIsValid", async () => {
      ...
      // Arrange target objects
      const assetfunction = createAssetfunction({ pluscwodsinstrid: "11", plantype: CalibrationPointConstants.PLANTYPE.ANALOG, inputprecision: 5 });
      ...
      expect(page.state.groupedCalpointsDS.items[0].calpointsDS.items[calpoint._index].asfoundinput).toEqual("100.00000");
    });
    ...
    it("shouldNotFormatEmptyValue", async () => {
      ...
      // Arrange target objects
      const assetfunction = createAssetfunction({ pluscwodsinstrid: "11", plantype: CalibrationPointConstants.PLANTYPE.ANALOG, inputprecision: 5 });
      const calpoint = createCalpoint({ pluscwodspointid: "112", asfoundinput: "", asfoundoutput: "3500.100", isaverage: false, _group_index: 0, _index: 0 });
      ...
      // Assert
      expect(page.state.groupedCalpointsDS.items[0].calpointsDS.items[calpoint._index].asfoundinput).toEqual("");
    });
  });
  ...
  describe("save", () => {
    it("Should save calibration point", async () => {
      ...
      datasheetDSMock.safeSave = jest.fn();

      // Mock controller methods
      controller.calibrationHelper = { updateCalpointsIntoAssetFunction: jest.fn() }
      ...
      // Mock app resources
      controller.app.navigateBack = jest.fn();
      controller.app.state.calpointsds = [
      ...
      controller.page.state.groupedCalpointsDS = {
        items: [
          {
            avgpoint: createCalpoint({ pluscwodspointid: 1014, _group_index: 0, _index: 3, isaverage: true }),
            calpointsDS: {
              items: [
                createCalpoint({ pluscwodspointid: 1011, _group_index: 0, _index: 0 }),
                createCalpoint({ pluscwodspointid: 1012, _group_index: 0, _index: 1 }),
                createCalpoint({ pluscwodspointid: 1013, _group_index: 0, _index: 2 }),
              ],
            },
          },
        ],
      ...
      const assetfunction = createAssetfunction({ dsplannum: "DS101", outputprecision: 3, inputprecision: 3 });
      ...
      expect(datasheetDSMock.safeSave).toHaveBeenCalled();
  ...
  describe("mapAllCalpointsToArray", () => {
    ...
    it("shouldKeepVirtualPropertiesWhenCreatingArray", async () => {
      ...
      // Act
      const calpoints = controller.mapAllCalpointsToArray(groupedCalpointsDS, CalibrationPointConstants.KEEP_VIRTUAL_ATTRS);
  ...
  describe("formatCalpointBeforeSave", () => {
    it("shouldFormatAnalogPoints", async () => {
      ...
      // Arrange target objects
      const assetfunction = createAssetfunction({ pluscwodsinstrid: "11", plantype: CalibrationPointConstants.PLANTYPE.ANALOG, inputprecision: 5, outputprecision: 5 });
      const calpoint = createCalpoint({ pluscwodspointid: "112", asfoundinput: "1,000.0", asfoundoutput: "3,500.0", plantype: CalibrationPointConstants.PLANTYPE.ANALOG });
    ...
    it("shouldFormatDiscretePoints", async () => {
      ...
      // Arrange target objects
      const assetfunction = createAssetfunction({ pluscwodsinstrid: "11", plantype: CalibrationPointConstants.PLANTYPE.DISCRETE, inputprecision: 5, outputprecision: 5 });
      const calpoint = createCalpoint({ pluscwodspointid: "112", asfoundsetpoint: "1,000.0", plantype: CalibrationPointConstants.PLANTYPE.DISCRETE });
    ...
    it("shouldNotFormatEmptyNumbers", async () => {
      ...
      // Arrange target objects
      const assetfunction = createAssetfunction({ pluscwodsinstrid: "11", plantype: CalibrationPointConstants.PLANTYPE.DISCRETE, inputprecision: 5, outputprecision: 5 });
      const calpoint = createCalpoint({ pluscwodspointid: "112", asfoundsetpoint: "", plantype: CalibrationPointConstants.PLANTYPE.DISCRETE });
  ...
  describe("updateCalpointsIntoDS", () => {
    it("Should find calibration point in datasource and update it with updatedCalpoints", async () => {
      ...
      const calpointsDS = {
        // Datasource
        items: [
          createCalpoint({ pluscwodspointid: 1011, asfoundinput: 1.15, asfoundoutput: 100.66 }),
          createCalpoint({ pluscwodspointid: 1012, asfoundinput: 1.20, asfoundoutput: 50.66 }),
          createCalpoint({ pluscwodspointid: 1013, asfoundinput: 3.15, asfoundoutput: 75.12 }),
          createCalpoint({ pluscwodspointid: 1014, asfoundinput: 10.15, asfoundoutput: 88.10 }),
        ]
      }
      ...
      expect(calpointsDS.items[3].asfoundinput).toEqual(300.00)
      expect(calpointsDS.items[3].asfoundoutput).toEqual(400.00)
    });
  });
  ...
  describe("displayFieldErrors", () => {
    it("shouldMarkAsFoundAsFail", async () => {
      ...
      const calpoint = createCalpoint({ pluscwodspointid: "1011", asfoundfail: false, _group_index: 0, _index: 0 });
    ...
    it("shouldMarkAsLeftAsFail", async () => {
      ...
      const calpoint = createCalpoint({ pluscwodspointid: "1011", asleftfail: false, _group_index: 0, _index: 0 });
    ...
    it("shouldNotMarkOutputExceededWhenTypeIsNotWarning", async () => {
      ...
      const calpoint = createCalpoint({ pluscwodspointid: "1011", asfoundfail: false, _output_exceeded: false, _group_index: 0, _index: 0 });
  ...
  const createDsConfig = (props) => defaults(props, {
