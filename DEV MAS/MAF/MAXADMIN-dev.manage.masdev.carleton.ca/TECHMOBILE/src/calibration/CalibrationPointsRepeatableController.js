...
class CalibrationPointsRepeatableController extends CalibrationPointsController {
  ...
  async pageResumed(page, app) {
    ...
    page.state.asfoundcallabel = this.setLabelColor(this.page.state.asfoundcalstatus);
    page.state.asleftcalstatus = page.params.asleftcalstatus;
    page.state.asleftcallabel = this.setLabelColor(this.page.state.asleftcalstatus);
    ...
    calpointsDS?.addIgnoreField('showWarningOnInputChange');
    calpointsDS?.addIgnoreField('showWarningOnOutputChange');
    ...
    // Instantiate helpers
    this.datasheetCalculation = new DatasheetCalculation(page.state.dsconfig.currentItem, this.getLocale());
  ...
  setGroupError(calpoint, hasError) {
    this.getGroupedCalpointsDS().items[calpoint._group_index].haserror = hasError;
  }
  ...
  findAvgPoint(calpointsHashSet, key) {
    return calpointsHashSet[key].find(calpoint => calpoint.isaverage === true);
  }
  ...
  async changeCalpoint(context) {
    ...
    // Load dependent datasources
    const dsconfig = this.getDsConfig();
    const calpointsDS = groupedCalpointsDS.items[calpoint._group_index].calpointsDS;
    ...
    const calpointsExceptAvg = calpointsDS.items.filter(calpoint => !calpoint.isaverage);
    ...
    const formErrorCount = CalibrationPointValidation.countErrors(formErrorsAllowEmpty);
    ...
    const avgpointMirror = groupedCalpointsDS.items[avgpoint._group_index].avgpoint;
  ...
  async validateCalpoint(context, event) {
    ...
    if (errors.length > 0) {
      ...
      const errorsOnly = errors.filter(CalibrationPointValidation.filterErrorsOnly);
    ...
    const errorCount = CalibrationPointValidation.countErrors(formErrorsAllowEmpty);
  ...
  async save() {
    ...
    // with new values.
    /* const calpoints = */ this
      .mapAllCalpointsToArray(groupedCalpointsDS)
    ...
    this.page.state[`${condition}calstatus`] = this.page.state.assetfunction[`${condition}calstatus`];
  ...
  displayFieldErrors(groupedCalpointsDS, fieldErrors) {
    ...
    if (fieldName.endsWith("output") && errorType === CalibrationPointConstants.FIELD_WARNING) {
