...
describe("CalibrationPointsController", () => {
  ...
  describe("goBack", () => {
    ...
    it("Should navigate back when not text was changed", async () => {
      // Arrange
      const app = await baseSetup();
      const page = app.findPage("calibrationpoints");
      const controller = page.controllers[0];
  ...
  describe("saveChanges", () => {
    it("Should reset text flag to false and change status", async () => {
      ...
      const pluscWoDsMock = { item: testCalibrationData.member[0], safeSave: jest.fn() };

      controller.app.findDatasource = jest.fn(() => pluscWoDsMock);
      controller.app.navigateBack = jest.fn();
      page.state.isdirty = true;

      controller.getDatasheetDS = jest.fn(() => pluscWoDsMock);
      controller.getLengthError = jest.fn(() => false);
      controller.getNumberError = jest.fn(() => false);
      controller.saveUpdate = jest.fn();
      ...
      expect(controller.saveUpdate).toHaveBeenCalled();
      expect(controller.app.navigateBack).toHaveBeenCalled();
    });
  });

  describe("discardChanges", () => {
    it("Should return false when asset function ds is empty", async () => {
      // Arrange
      const app = await baseSetup();
      const page = app.findPage("calibrationpoints");
      const controller = page.controllers[0];
      ...
      ds.getChildDatasource = () => ({ load: jest.fn(() => []) });

      // Act
      const result = await controller.discardChanges();
    ...
    it("Should redirect to asset functions page", async () => {
      ...
      expect(app.setCurrentPage).toHaveBeenCalled();
    });
  });

  describe("loadStateData", () => {
    it("Should check if we have asset function and calibration points", async () => {
      // Arrange
      const app = await baseSetup();
      const page = app.findPage("calibrationpoints");
      const controller = page.controllers[0];
      ...
      const result = await controller.loadStateData(app, page);

      expect(page.state.loading).toBeFalsy();
      expect(result).toEqual(false);
    });
    it("Should load all data when assetFunctionsDetailsDS and calpointsds does not exist", async () => {
      const app = await baseSetup();
      const page = app.findPage("calibrationpoints");
      const controller = page.controllers[0];
      ...
      app.state.calpointsds = null;
      
      const filter = 'wonum=1206';
      const filter1 = 'wodsnum=1011';

      const workorderDS = app.findDatasource("woDetailCalibration");
      await workorderDS.load({where: filter});

      const datasheetDS = app.findDatasource("pluscWoDs");
      await datasheetDS.load({where: filter1});

      await controller.loadStateData(app, page);

      expect(page.state.loading).toBeFalsy();
      expect(workorderDS.items.length).toBeGreaterThan(0);
      expect(datasheetDS.items.length).toBeGreaterThan(0);
  ...
  describe("calculateCalPointTolerances", () => {
    ...
    it("Should NOT calculate tolerance error when input or output are missing", async () => {
      ...
      const calpointsDS = new Datasource(
        new JSONDataAdapter({
          idAttribute: "pluscwodspointid",
          src: testDatasheetENData.member[0].pluscwodsinstr[0].pluscwodspoint,
        }),
        {
          autoSave: false,
          idAttribute: "pluscwodspointid",
          name: "testCalpointsDS",
        }
      );
      ...
      controller.getAssetFunctionDS = jest.fn(() => ({
        currentItem: pluscWoDs.member[0].pluscwodsinstr[0],
      }));
    ...
    it("Should calculate tolerance error when changing status", async () => {
      ...
      // Arrange datasources
      const domainCalStatusDS = await loadDomainCalStatusDS();

      const datasheetDS = new Datasource(
        new JSONDataAdapter({
          idAttribute: "pluscwodsid",
          src: testDatasheetENData.member,
        }),
        {
          autoSave: false,
          idAttribute: "pluscwodsid",
          name: "testDatasheetDS",
        }
      );

      const assetFunctionDS = new Datasource(
        new JSONDataAdapter({
          idAttribute: "pluscwodsinstrid",
          src: testDatasheetENData.member[0].pluscwodsinstr,
        }),
        {
          autoSave: false,
          idAttribute: "pluscwodsinstrid",
          name: "testAssetFunctionDS",
        }
      );
      ...
      app.state.assetFunctionsDetailsDS = assetFunctionDS;
      datasheetDS.getApplication = jest.fn(() => app);

      // Arrange controller mocks
      const controller = page.controllers[0];
      controller.getLocale = jest.fn(() => LocaleConstants.EN_US);
      controller.getDomainCalStatusDS = jest.fn(() => domainCalStatusDS);
      controller.getDatasheetDS = jest.fn(() => datasheetDS);
      controller.getCalibrationPointsDS = jest.fn(() => calpointsDS);
      controller.getAssetFunctionDS = jest.fn(() => assetFunctionDS);
      controller.getConditionPrefix = jest.fn(
        () => CalibrationPointConstants.CONDITION_ASFOUND
      );
      controller.loadDsConfig = jest.fn(loadDsConfig);
      controller.getMaxVars = jest.fn(() => []);
      ...
      for (let i = 0; i < 3; i++) {
        const calpoint = {
          ...testDatasheetENData.member[0].pluscwodsinstr[0]
            .pluscwodspoint[0],
          ...entries[i],
        };
  ...
  describe('validateCalPointAndCalculateTolerances', () => {
    ...
    it('verifies validateNumerical has been called.', async () => {
      ...
      controller.validateNumerical = jest.fn(() => true).mockName('validateNumerical');
      controller.validateLength = jest.fn(() => false).mockName('validateLength');
      ...
      // Act
      controller.validateCalPointAndCalculateTolerances({ item: calpoint, whichFieldTypeChanged });

      // Assert
      expect(controller.resetValidationFlags).toHaveBeenCalledWith(calpoint, whichFieldTypeChanged);
      expect(controller.validateNumerical).toHaveBeenCalledWith(calpoint, whichFieldTypeChanged);
      expect(controller.standardizeDecimal).not.toHaveBeenCalled();
      expect(controller.checkBounds).not.toHaveBeenCalled();
    });
    ...
    it('verifies validateLength has been called.', async () => {
      ...
      const controller = page.controllers[0];
      controller.resetValidationFlags = jest.fn();
      controller.validateNumerical = jest.fn(() => false).mockName('validateNumerical');
      controller.validateLength = jest.fn(() => true).mockName('validateLength');
      controller.standardizeDecimal = jest.fn();
      controller.checkBounds = jest.fn();
      controller.calculateCalPointTolerances = jest.fn();
      ...
      // Assert
      expect(controller.validateLength).toHaveBeenCalledWith(calpoint, whichFieldTypeChanged);
      expect(controller.standardizeDecimal).not.toHaveBeenCalled();
      expect(controller.checkBounds).not.toHaveBeenCalled();
    });
    ...
    it('verifies standardizeDecimal and checkBounds have been called.', async () => {
      ...
      // Assert
      expect(controller.standardizeDecimal).toHaveBeenCalledWith(calpoint, whichFieldTypeChanged);
      expect(controller.checkBounds).toHaveBeenCalledWith(calpoint, whichFieldTypeChanged);
    });
  });
  ...
  describe('validateNumerical', () => {
    it('should evaluate a valid number.', async () => {
      ...
      const page = app.findPage("calibrationpoints");
      const controller = page.controllers[0];
      controller.getConditionPrefix = jest.fn().mockReturnValue(CalibrationPointConstants.CONDITION_ASFOUND);
      const whichFieldTypeChanged = CalibrationPointConstants.FIELD_TYPE.INPUT;
      const fieldName = `${CalibrationPointConstants.CONDITION_ASFOUND}${whichFieldTypeChanged}`
      const calpoint = { plantype: CalibrationPointConstants.PLANTYPE.ANALOG, [fieldName]: '500.00' };
      
      // Act
      const isValidationFailed = controller.validateNumerical(calpoint, whichFieldTypeChanged);
    ...
    it('should evaluate a invalid number.', async () => {
      ...
      const calpoint = { plantype: CalibrationPointConstants.PLANTYPE.DISCRETE, [fieldName]: 'abcd' };
      ...
      expect(controller.setCalPointDsWarning).toHaveBeenCalledWith(calpoint, message, whichFieldTypeChanged);
      expect(controller.setCalPointDsWarning).toHaveBeenCalledWith(calpoint, message, whichFieldTypeChanged);
    });
  });
  ...
  describe('validateLength', () => {
    it('verifies input number should be of valid length.', async () => {
      ...
      const whichFieldTypeChanged = CalibrationPointConstants.FIELD_TYPE.INPUT;
      const fieldName = `${CalibrationPointConstants.CONDITION_ASFOUND}${whichFieldTypeChanged}`
      const calpoint = { plantype: CalibrationPointConstants.PLANTYPE.ANALOG, [fieldName]: '123456' };
      
      // Act
      const isValidationFailed = controller.validateLength(calpoint, whichFieldTypeChanged);
    ...
    it('verifies negative input number should be of valid length.', async () => {
      ...
      const whichFieldTypeChanged = CalibrationPointConstants.FIELD_TYPE.INPUT;
      const fieldName = `${CalibrationPointConstants.CONDITION_ASFOUND}${whichFieldTypeChanged}`
      const calpoint = { plantype: CalibrationPointConstants.PLANTYPE.ANALOG, [fieldName]: '-123456789012345' };
    ...
    it('should raise an error that Entered number is too long.', async () => {
      ...
      const whichFieldTypeChanged = CalibrationPointConstants.FIELD_TYPE.INPUT;
      const fieldName = `${CalibrationPointConstants.CONDITION_ASFOUND}setpoint`
      const calpoint = { plantype: CalibrationPointConstants.PLANTYPE.DISCRETE, [fieldName]: '1234567890123456' };
  ...
  describe('standardizeDecimal', () => {
    ...
    it('should remove commas from formatted number.', async () => {
      ...
      const fieldName = `${CalibrationPointConstants.CONDITION_ASFOUND}${whichFieldTypeChanged}`
      controller.getModifiedFieldName = jest.fn().mockReturnValue(fieldName);
      const calpoint = { plantype: CalibrationPointConstants.PLANTYPE.ANALOG, [fieldName]: '11,23,456' };
  ...
  describe('checkBounds', () => {
    it('should check user input is within the upper and lower bounds of the instrument.', async () => {
      ...
      const whichFieldTypeChanged = CalibrationPointConstants.FIELD_TYPE.INPUT;
      const controller = page.controllers[0];
      const selectedAssetFn = { cliplimitsin: true, instrcalrangefrom: 25, instrcalrangeto: 100, instroutrangefrom: 2,  instroutrangeto: 20 };
      ...
      const fieldName = `${CalibrationPointConstants.CONDITION_ASFOUND}${whichFieldTypeChanged}`
      controller.getModifiedFieldName = jest.fn().mockReturnValue(fieldName);
      const calpoint = { plantype: CalibrationPointConstants.PLANTYPE.ANALOG, [fieldName]: '45', pluscwodspointid: 125 };
    ...
    it('should check user input is outside the upper and lower bounds of the instrument.', async () => {
      ...
      controller.setCalPointDsWarning = jest.fn();
      controller.setInputError = jest.fn();
      const message = 'The As Found input 126 is outside the range of 25 to 100.';
      const fieldName = `${CalibrationPointConstants.CONDITION_ASFOUND}${whichFieldTypeChanged}`
      controller.getModifiedFieldName = jest.fn().mockReturnValue(fieldName);
      const calpoint = { plantype: CalibrationPointConstants.PLANTYPE.ANALOG, [fieldName]: '126', pluscwodspointid: 125 };
      ...
      expect(controller.setCalPointDsWarning).toHaveBeenCalledWith(calpoint, message, whichFieldTypeChanged, );
      expect(controller.setInputError).toHaveBeenCalledWith(calpoint.pluscwodspointid, whichFieldTypeChanged, true);
    });
    ...
    it('should check user input is outside the upper and lower bounds of the instrument with cliplmits instruction.', async () => {
      ...
      const whichFieldTypeChanged = CalibrationPointConstants.FIELD_TYPE.INPUT;
      const controller = page.controllers[0];
      const selectedAssetFn = { cliplimitsin: false, instrcalrangefrom: 25, instrcalrangeto: 100, instroutrangefrom: 2,  instroutrangeto: 20 };
      controller.getAssetFunctionDS = jest.fn().mockReturnValue({ currentItem: selectedAssetFn});
      controller.getConditionPrefix = jest.fn().mockReturnValue(CalibrationPointConstants.CONDITION_ASFOUND);
      controller.setCalPointDsWarning = jest.fn();
      controller.setInputError = jest.fn();
      const message = 'The As Found input 102 is outside the range of 24 to 101.';
      const fieldName = `${CalibrationPointConstants.CONDITION_ASFOUND}${whichFieldTypeChanged}`
      controller.getModifiedFieldName = jest.fn().mockReturnValue(fieldName);
      const calpoint = { plantype: CalibrationPointConstants.PLANTYPE.ANALOG, [fieldName]: '102', ron1lower: 24,  ron1upper: 101, pluscwodspointid: 125 };
    ...
    it('should swap ronupper and ronlower value if range is not defined properly.', async () => {
      ...
      controller.setCalPointDsWarning = jest.fn();
      controller.setInputError = jest.fn();
      const message = 'The As Found input 23 is outside the range of 101 to 24.';
      const fieldName = `${CalibrationPointConstants.CONDITION_ASFOUND}${whichFieldTypeChanged}`
      controller.getModifiedFieldName = jest.fn().mockReturnValue(fieldName);
      const calpoint = { plantype: CalibrationPointConstants.PLANTYPE.ANALOG, [fieldName]: '23', ron1lower: 101,  ron1upper: 24, pluscwodspointid: 125 };
  ...
  describe("updateStatus", () => {
    it("Should update status to PASS when condition is valid", async () => {
      CalibrationPointConstants.PREFIXES.forEach(async (condition) => {
        ...
        const event = {
          ...
          status: DatasheetConstants.STATUS_PASS,
        };

        // Act
        const result = controller.updateStatus(event);

        // Assert
        expect(result).toEqual(true);
        expect(event.item[`${condition}pass`]).toEqual(true);
        expect(event.item[`${condition}fail`]).toEqual(false);
      });
    });
    ...
    it("Should update status to FAIL when condition is valid", async () => {
      CalibrationPointConstants.PREFIXES.forEach(async (condition) => {
        ...
        const event = {
          ...
          status: DatasheetConstants.STATUS_FAIL,
        };
        ...
        // Assert
        expect(result).toEqual(true);
        expect(event.item[`${condition}pass`]).toEqual(false);
        expect(event.item[`${condition}fail`]).toEqual(true);
      });
    });
    ...
    it("Should return false when condition is not valid", async () => {
      ...
      // Assert
      expect(result).toEqual(false);
      expect(event.item.asfoundpass).toEqual(true);
      expect(event.item.asfoundfail).toEqual(false);
    });
  });
  ...
  describe("getModifiedFieldName", () => {
    it("Should correctly return field name when plantype is analog", async () => {
      ...
      controller.pageInitialized(page, app);
      controller.page.params.condition = CalibrationPointConstants.CONDITION_ASFOUND;;


      const whichFieldTypeChanged = "input";
      const plantype = "ANALOG"

      const fieldName = controller.getModifiedFieldName(plantype, whichFieldTypeChanged);
    ...
    describe("calculateCalPointTolerances", () => {
      it("shouldCalculateToleranceErrorsWhenLocaleUsesCommaAsDecimalSeparator", async () => {
        ...
        const datasheetDS = new Datasource(
          new JSONDataAdapter({
            idAttribute: "pluscwodsid",
            src: testDatasheetDaData.member,
          }),
          {
            autoSave: false,
            idAttribute: "pluscwodsid",
        ...
        const assetFunctionDS = new Datasource(
          new JSONDataAdapter({
            idAttribute: "pluscwodsinstrid",
            src: testDatasheetDaData.member[0].pluscwodsinstr,
          }),
          {
            autoSave: false,
            idAttribute: "pluscwodsinstrid",
        ...
        const calpointsDS = new Datasource(
          new JSONDataAdapter({
            idAttribute: "pluscwodspointid",
            src: testDatasheetDaData.member[0].pluscwodsinstr[0].pluscwodspoint,
          }),
          {
            autoSave: false,
            idAttribute: "pluscwodspointid",
...
describe('getLocaleNumber', () => {
  ...
  const loadDsConfig = async () => ({ name: "dsconfigtest", items: testDsconfigData.member });
});
