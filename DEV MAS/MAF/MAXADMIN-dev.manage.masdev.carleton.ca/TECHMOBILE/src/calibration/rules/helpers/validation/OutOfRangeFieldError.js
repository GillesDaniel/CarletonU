...
class OutOfRangeFieldError extends FieldError {
  ...
  createMessage(calpoint, fieldName, assetfunction, locale) {
    ...
    if (assetfunction.cliplimitsin) {
      lowerbound = toDisplayableValue(assetfunction.instrcalrangefrom, numberFormatOptions, locale);
      upperbound = toDisplayableValue(assetfunction.instrcalrangeto, numberFormatOptions, locale);
    
    // ron1 lower and upper
    } else if (calpoint.ron1lower && calpoint.ron1upper) {
      lowerbound = toDisplayableValue(calpoint.ron1lower, numberFormatOptions, locale);
      upperbound = toDisplayableValue(calpoint.ron1upper, numberFormatOptions, locale);
    }
