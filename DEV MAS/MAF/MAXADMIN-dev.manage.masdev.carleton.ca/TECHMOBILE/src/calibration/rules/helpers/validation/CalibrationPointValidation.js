...
class CalibrationPointValidation {
  ...
  static validateField(calpoint, fieldName, assetfunction, locale, allowEmpty) {
    ...
    const outputExceeded = fieldName.startsWith(CalibrationPointConstants.CONDITION_ASFOUND)
     ...
     */
    const value = fromDisplayableValue(strValue, locale);

    if (fieldName.endsWith("input") && this.isInputOutOfRange(value, calpoint, assetfunction)) {
      errors.push(
        // Should generate a message like: "The As Found input 1.125 is outside the range of 0.00 to 1.00."
        new OutOfRangeFieldError(
          calpoint,
          fieldName,
          assetfunction,
          locale
  ...
  static validateFormErrorsOnly(
    ...
    return this
      .validateForm(calpoints, condition, assetfunction, locale, allowEmpty)
      .filter(error => error.getErrorType() === CalibrationPointConstants.FIELD_ERROR);
  }
  ...
  static countErrors(errors) {
    return errors.filter(error => error.getErrorType() === CalibrationPointConstants.FIELD_ERROR).length;
  }
  ...
  static isNonEmptyNumber(strValue, locale) {
    return (
      !isEmpty(strValue) && LocaleNumberUtil.isValidNumber(strValue, locale)
    );
  }
