...
class CalibrationPointHandler {
  ...
  async _completeReadings(
    ...
    // Synchronize data hierarchically across calpoints, asset function and datasheet
    this.synchronizeAllDatasources();
    ...
    let missingOrBrokenStatus = CalibrationHelper.checkMissingOrBrokenStatus(datasheetDS.currentItem.pluscwodsinstr, conditionPrefix);
    if (missingOrBrokenStatus) {
      datasheetDS.currentItem[`${conditionPrefix}calstatus`] = missingOrBrokenStatus;
    }

    // Datasheet should have all changes in it
  ...
  static isWarningMessageShownOnDatasheetChanges(mobilemaxvars) {
    ...
    const [maxvars] =
      CommonUtil.filterMobileMaxvars(
    ...
    isWarningShownOnDatasheetChanges =
      maxvars?.varvalue ===
      DatasheetConstants.SHOW_WARNING_ON_DATASHEET_CHANGES;
  ...
  _getDataSheetStatus(assetfunctions, conditionPrefix) {
    ...
    if (!missingOrBrokenStatus) {
      for (let i = 0; i < assetfunctions.length; i++) {
        assetfunction = assetfunctions[i];
  ...
  /**
   * Synchronize data hierarchically across calpoints, asset function
   * and datasheet. Datasheet should have all changes in it.
   *
   * This function gets all updated datasources and update each item
   * with its child data:
   *
   * - Calibration points will be updated into asset function.
   * - Asset function will be updated into datasheet.
   *
   * Once all data is synchronized, we save it back using the available
   * setter functions.
   *
   * @returns Boolean - Returns whether the synchronization succeeded or not.
   */
  synchronizeAllDatasources() {
    // Datasources
    const calpointsDS = this.getCalpointsDS();
  ...
  // TODO: GRAPHITE-76034 fix: replace getRelatedDSConfig() with getDatasheetDS() in CalibrationPointHandler
  /* istanbul ignore next */
  async getRelatedDSConfig() {
    const app = this.getDatasheetDS().getApplication();
    ...
    if (pluscwodsinstr) {
      const { dsplannum, revisionnum } = pluscwodsinstr.currentItem;

      await dsconfig.load({
        where: `dsplannum="${dsplannum}" AND revisionnum=${revisionnum}`,
      });
    } else {
      await dsconfig.load();
    }

    return dsconfig.item;
  }
  ...
  performCalculationForRepeatables(calpoints, condition) {
    ...
    const calc = new DataSheetCalculation(dsconfigDS.currentItem, locale);
    calc.calculateAvgAndStdDeviation(condition, assocpoints, avgpoint);
