...
describe("CalibrationPointHandler", () => {
  ...
  describe("synchronizeAllDatasources", () => {
    it("should do nothing if index is less than 0", async () => {
      ...
      handler.getAssetFunctionDS = jest.fn(() => assetfunction);
      ...
      handler.getDatasheetDS = jest.fn(() => datasheetDS);
      handler.synchronizeAllDatasources();
    });
  });
  ...
  describe("_updateDataSheetStatus", () => {
    ...
    it("Should update datasheet status to empty if any asset function status is MISSING.", async () => {
      ...
      // Assert
      expect(handler.autoUpdateDataSheetStatus).toHaveBeenCalled();
      expect(datasheet["asfoundcalstatus"]).toEqual("MISSING");
    });
  ...
  describe("_getDataSheetStatus", () => {
    ...
    describe("(calpoint)", () => {
      ...
      it("Should return tolerance status when asset function tolerance is exceeded and tolerance object width should match widestTolWidth", async () => {
        // Arrange
        const app = await initializeApp();
        const ds = await loadDatasheetDS(app, TEST_WODSNUM);
        const calpoints = await loadCalpointsDS(ds, TEST_PLUSCWODSINSTRID);
        ...
        const tol = handler._getToleranceForStatus(
          assetfunctions[0],
          CalibrationPointConstants.CONDITION_ASFOUND,
          assetfunctions.asfoundcalstatus
        );  
        ...
        // Assert
        expect(tol.width).toEqual(assetfunctions[0].widestTolwidth);
        expect(status).toEqual(DatasheetConstants.STATUS_WARNING);
      });
