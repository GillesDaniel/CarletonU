...
class AssetFunctionsController extends BaseController {
   ...
   * @returns void.
   */
  async loadCalibrationPointsRepeatablePage(assetfunction) {
    // Load datasources
    const assetfunctionDS = this.app.state.assetFunctionsDetailsDS;
    const calpointDS = await assetfunctionDS.getChildDatasource(
      "pluscwodspoint",
      assetfunction,
      DatasourceConstants.CALIBRATION_POINT_CHILD_OPTIONS
    );

    await calpointDS.load();
   ...
   * @returns Boolean - Returns whether the page was redirected or not.
   */
  async loadCalibrationPointPage(item, overrideParams = {}) {
    ...
    //istanbul ignore else
    if (assetfunctionDS) {
      // Load calibration point datasource
      const calpointDS = await assetfunctionDS.getChildDatasource(
        "pluscwodspoint",
        item,
        DatasourceConstants.CALIBRATION_POINT_CHILD_OPTIONS
      );
      const response = await calpointDS.load();

      if (!response.length) {
        this.app.state.nopointexist = true;
      }
  ...
  */
  ...
  async saveAssetData() {
    ...
    const datasheetDS = await helper.updateCalpointsIntoAssetFunction();
  ...
  /*
   * Reload top-level datasource and wait for relevant child datasource to be ready
   */
  //istanbul ignore next
  async resetTopLevelChildDs(item) {
    const ds = this.app.findDatasource("pluscWoDs");
    await ds.forceReload();
    const assetFunctionsDetailsDS = await ds.getChildDatasource(
      "pluscwodsinstr",
      item,
      DatasourceConstants.ASSET_FUNCTION_CHILD_OPTIONS
    );
    await assetFunctionsDetailsDS.load();
    this.app.state.assetFunctionsDetailsDS = assetFunctionsDetailsDS;
  }
  ...
  async changeAssetFunctionStatus(item) {
    ...
    if (index >= 0) {
      ...
      datasheetDS.currentItem[`${condition}calstatus`] =
        await CalibrationHelper.checkMissingOrBrokenStatus(
