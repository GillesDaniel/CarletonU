  ...
  async dialogOpened() {
    ...
    if(selectedItem?.measurementvalue || selectedItem?.observation) {
      ...
      selectedItem.computedMeterCurTime = this.page.parent.getApplication().dataFormatter
        ...
        .getTime();
      woPlanTaskDetailds.item.computedMeterCurDate = selectedItem.computedMeterCurDate;
      woPlanTaskDetailds.item.computedMeterCurTime = selectedItem.computedMeterCurTime;
    } else {
      selectedItem.newreading = null;
      const date = new Date();
      woPlanTaskDetailds.item.computedMeterCurDate = date;
      woPlanTaskDetailds.item.computedMeterCurTime = date.getTime();;
    }
