...
class WorkOrderCreateController {
  ...
  async openNewWorkOrder() {
    ...
    await WOCreateEditUtils.resetDataSource(this.app, 'alndomainData');
  ...
  */
  ...
  async prepareDatatoUpdate(woCreateResource){
    ...
    if(this.app.state.currentMapData?.coordinate?.length) {
      ...
      if (!(serviceAddress && serviceAddress.longitudex && serviceAddress.latitudey)) {
 ...
 if ((workorder.wopriority &&
   ...
   */
   async findAsset(item) {
    ...
    } else if (assetData.length > 1 && assetData.findIndex((asset) => asset.assetnum === this.page.datasources.dsCreateWo.item.assetnum) < 0) {
      // Clear assetnum only if it does not exist in location's assets
      item.asset = assetData;
      WOCreateEditUtils.validateAsset(this.app, this.page, this.page.datasources.dsCreateWo, item);
