...
it("should open create Workorder page", async () => {
  ...
  const alndomainData = newStatusDatasource(statusitem, 'alndomainData');
  app.registerDatasource(alndomainData);
...
it("should not clear Asset if location has the same asset ", async () => {
  ...
  ds.item.assetnum = "10001";
  await controller.findAsset(item);
  expect(validateAssetLocationStub.called).toBe(true);
  WOCreateEditUtilsstub.restore();
...
it('Validates the startMeasureCal()', async () => {
  const controller = new WorkOrderCreateController();
  const app = new Application();
  const page = new Page({ name: "createwo" });
  app.registerController(controller);  
  ...
  controller.pageInitialized(page, app);
  // Arrange
  page.state.measureUpdate = false
  const evt = { item: { startfeaturelabel: "MP 10" ,startoffset:"10",startassetfeatureid:"110"} };
  await controller.startMeasureCal(evt);
  expect(page.state.startOffsetReadOnly).toBe(true);
});
...
it('Validates the startFeatureLabelVal()', async () => {
  const controller = new WorkOrderCreateController();
  const app = new Application();
  const page = new Page({ name: "createwo" });
  app.registerController(controller);
