...
describe('rejectStatus', () => {
  it('should set the loading status to false', async () => {
    ...
    app.findPage.mockReturnValueOnce(true);
    ...
    expect(app.setCurrentPage).toHaveBeenCalledWith({ name: 'schedule', resetScroll: true });

    app.findPage.mockReturnValueOnce(false);
    await dialog.rejectStatus();
    expect(page.parent.state.loadingstatus).toBe(false);
    expect(page.parent.findDialog).toHaveBeenCalledWith(page.parent.state.statusDialog);
    expect(app.setCurrentPage).toHaveBeenCalledWith({ name: 'approvals', resetScroll: true });
  });
