...
class SchedulePageController {
  ...
  */
  ...
  async rejectWO(event) {
    ...
    if(!rejectDS?.items?.length) {
      let dnewreadingDS = this.app.findDatasource("rejectdomainData");
    ...
    this.page.showDialog("rejectAssignment");
    this.page.state.workloading = false;
  }
  ...
  */
  ...
  async openSignatureDialog(event) {
    ...
    const woDetailds = this.app.findDatasource("wodetails");
...
*/
  ...
  async onUpload() {
    ...
    const workorder = {
      item: woDetailds.item,
      datasource: woDetailds,
      action: "start",
      worktype: "work"
    }
  ...
  async saveUpdateMeterDialog() {
    ...
    //istanbul ignore else
    if (!this.isRollover) {
      let assetNumData =item?.asset?.[0]?.assetnum;
  ...
  async closeUpdateMeterDialog(event) {
    ...
    this.page.findDialog('update_meterReading_drawer')?.closeDialog();
  }
  ...
  async pageResumed(page) {
    ...
    this.app.findPage("reserveMaterials").state.reservedItemAdded = false;
  ...
  trackUserLogin(page) {
    ...
    let firstLogoutDate = this.app?.client?.userInfo?.user?.logouttracking?.attemptdate;
    ...
    this.app.state.firstLogin = firstLoginData.isFirstLogIn;
  }
 ...
 */
 //istanbul ignore next
 meterValidateList(validateEvent){
  CommonUtil.meterValidate(validateEvent,'saveDiscardMeterReadingList',this.page);
}
}
