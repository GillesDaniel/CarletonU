...
const computedOthersRemaining = async (data, ponum, app) => {
  const result = {};
  result[RETURN_PAGE] = [];
  result[VOID_PAGE] = [];
  result[ASSET_PAGE] = [];
  result[RECEIPT_PAGE] = [];
  const matrectransitems = data.matrectransitems;
  ...
  RECEIPT = app.state.issuetypereceipt;
  RETURN = app.state.issuetypereturn;
  VOIDRECEIPT = app.state.issuetypevoid;
  TRANSFER = app.state.issuetypetransfer;

  matrectransitems.forEach((receipt) => {
    matreclist.push(receipt);
    // istanbul ignore else
    if (receipt.status_maxvalue && receipt.status_maxvalue !== STATTRANSFER) {
  ...
  if (isFromMXRecept) {
    if (item.issuetype === RECEIPT) {
      ...
      if (existingrec) {
        ...
        // istanbul ignore else
        if (Number(item.acceptedqty) > 0 && !item.rotating) {
