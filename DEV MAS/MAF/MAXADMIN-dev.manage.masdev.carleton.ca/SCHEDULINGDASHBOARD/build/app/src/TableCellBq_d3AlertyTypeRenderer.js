...
const TableCellBq_d3AlertyTypeRenderer = observer(props => {
    const app = props.app;
...
const jOptimizeSummaryDS = props.jOptimizeSummaryDS || app.findDatasource("jOptimizeSummaryDS");
    ...
    return (
      ...
      <DatasourceLabel  theme={"field"} label={app.format((item?item.alerttype:undefined),'alerttype',null,datasource)} spacing={"none"} padding={"none"} id={'ajez_[' + props.rowId + ']'} datadraggable={"true"} datacomponenttype={"label"} datacomponentoriginalid={"ajez_"} datasource={datasource} item={item} field={"alerttype"}></DatasourceLabel>

    
    );
  });

export default inject('app')(TableCellBq_d3AlertyTypeRenderer);
