...
const TableCellB3gyyWopriorityRenderer = observer(props => {
    const app = props.app;
...
const jresourceLevelingTaskDS = props.jresourceLevelingTaskDS || app.findDatasource("jresourceLevelingTaskDS");
    ...
    return (
      ...
      <SmartInput id={'bj3gs[' + props.rowId + ']'} value={(item?item.wopriority:undefined)} max={999} min={0} hideLabel={true} hideStepButtons={true} size={"none"} dateType={"date"} datadraggable={"true"} datacomponenttype={"smart-input"} datacomponentoriginalid={"bj3gs"} datasource={datasource} item={item} field={"wopriority"} dataFormatter={app.formatter}/>
    
    );
  });

export default inject('app')(TableCellB3gyyWopriorityRenderer);
