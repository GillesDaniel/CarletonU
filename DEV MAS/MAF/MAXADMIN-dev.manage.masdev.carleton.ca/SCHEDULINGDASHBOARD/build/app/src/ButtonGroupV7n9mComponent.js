...
const ButtonGroupV7n9mComponent = (props) => {
  const app = props.app;
...
const jresourceLevelingTaskDS = props.jresourceLevelingTaskDS || app.findDatasource("jresourceLevelingTaskDS");
  ...
  return (<Observer>{()=>(<><ButtonGroup id={"v7n9m"} align={"start"} padding={true} datadraggable={"true"} droppable={"true"} datacomponenttype={"button-group"} datacomponentoriginalid={"v7n9m"}>
<Button hidden={!skdprojectsDS.item.lastpublishdate} id={"gwqee"} small={true} tagValueLimit={99} datadraggable={"true"} datacomponenttype={"button"} datacomponentoriginalid={"gwqee"} kind={"ghost"} iconName={"carbon:information"} tooltip={{
        content: <Label  theme={"header-small"} label={app.getLocalizedLabel("wpg9w_label", `Last published on {0}`, [app.format(skdprojectsDS.item.lastpublishdate,'lastpublishdate', null, skdprojectsDS)])} wrap={true} id={"wpg9w"} padding={"default"} datadraggable={"true"} datacomponenttype={"label"} datacomponentoriginalid={"wpg9w"}></Label>
,
        button: null,
        link: null,
...
export default ButtonGroupV7n9mComponent;
