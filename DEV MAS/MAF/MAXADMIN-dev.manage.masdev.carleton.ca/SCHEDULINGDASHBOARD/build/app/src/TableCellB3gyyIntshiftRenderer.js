...
import { FieldHelp } from '@maximo/react-components';

const TableCellB3gyyIntshiftRenderer = observer(props => {
    const app = props.app;
...
const jresourceLevelingTaskDS = props.jresourceLevelingTaskDS || app.findDatasource("jresourceLevelingTaskDS");
    ...
    datacomponentoriginalid={"x8qpz"}
    datacomponenttype={"box"}
    datadraggable={"true"}
    droppable={"true"}
    
  >
<FieldHelp id="{'mrnz8[' + props.rowId + ']'}_field_help"
        datasource={datasource}
        field={"intshift"}>

      <DatasourceDropdown
      id={'mrnz8[' + props.rowId + ']'}
      onChange={(event)=>{
        
        item.intshift=event.selectedItem.id;
      }}
      maxItems={100}
      datasource={skdprojectshiftsDS}
      datasourceItemValueField={"shiftnum"}
      
      hideLabel={true}
      disabled={!(item?item.interruptible:undefined)}
      datasourceItemTextField={"shiftnum"}
      dataFormatter={app.formatter}
      
      boundDatasource={datasource}
            boundDatasourceItem={item}
            boundDatasourceField={"intshift"}
            selectedItem={(item?item.intshift:undefined)}
...
export default inject('app')(TableCellB3gyyIntshiftRenderer);
