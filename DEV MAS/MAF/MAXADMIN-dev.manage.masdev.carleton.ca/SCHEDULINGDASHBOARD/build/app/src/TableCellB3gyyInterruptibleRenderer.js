...
const TableCellB3gyyInterruptibleRenderer = observer(props => {
    const app = props.app;
...
const jresourceLevelingTaskDS = props.jresourceLevelingTaskDS || app.findDatasource("jresourceLevelingTaskDS");
    ...
    datacomponentoriginalid={"r63q9"}
    datacomponenttype={"box"}
    datadraggable={"true"}
    droppable={"true"}
    
  >
<SmartInput id={'bd_ma[' + props.rowId + ']'} value={(item?item.interruptible:undefined)} hideLabel={true} size={"none"} dateType={"date"} datadraggable={"true"} datacomponenttype={"smart-input"} datacomponentoriginalid={"bd_ma"} datasource={datasource} item={item} field={"interruptible"} dataFormatter={app.formatter}/></Box>

    
    );
  });

export default inject('app')(TableCellB3gyyInterruptibleRenderer);
