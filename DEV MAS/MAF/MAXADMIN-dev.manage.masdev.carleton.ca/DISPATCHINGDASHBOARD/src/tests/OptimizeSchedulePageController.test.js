...
describe('OptimizeSchedulePageController', () => {
  it('pageResumed will retry', async () => {
    ...
    expect(rundsLoadSpy).toHaveBeenCalledWith(
  ...
  it('correct status tag is generated', () => {
    ...
    tag = controller.computeOptimizationStatusTag(stoppedItem);
    expect(JSON.stringify(tag[0])).toContain('gray');

    tag = controller.computeOptimizationStatusTag(stoppingItem);
    expect(JSON.stringify(tag[0])).toContain('blue');
  ...
  it('goes to  Schedule details page', async () => {
    ...
    page.params.href = 'href1';
    page.params.projectname = 'BST1';
    ...
    expect(setCurrentPageSpy).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        params: expect.objectContaining({ scenario: expect.anything() }),
      })
    );
  });
  ...
  it('invokes optimization stop request - success', async () => {
    ...
    const invokeActionSpy = jest.spyOn(projectsDS, 'invokeAction');

    await controller.stopOptimizationRun();
    expect(invokeAction.called).toBe(true);
    expect(invokeActionSpy).toHaveBeenCalledWith(
      'finishSKDODMERUN',
      expect.anything()
    );
    ...
    expect(getLocalizedLabelSpy).toHaveBeenCalledWith(
      ...
      expect.stringContaining('Optimization stopped')
    );
  });
