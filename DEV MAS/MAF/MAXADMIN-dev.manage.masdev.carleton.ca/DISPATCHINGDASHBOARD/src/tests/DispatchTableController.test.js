...
describe('DispatchTableController', () => {
  ...
  describe('onAfterLoadData', () => {
    it('dashboard KPIs should be fetched', async () => {
      ...
      const jemergencyKPIds = newDatasource(
        sampleItem,
        'member',
        'id',
        'jemergencyKPIds'
      ...
      app.registerDatasource(jemergencyKPIds);
      ...
      const emergencyKPIds = app.findDatasource('jemergencyKPIds');
      const emergencyItems = await emergencyKPIds.load();
      ...
      expect(emergencyItems[0].emergencies).toBe(1);
    });
  ...
  describe('getResourcesNum', () => {
    it('should return the number of resources', async () => {
      const item = JSON.parse(JSON.stringify(projectItemValidRunID));
      const app = new Application();
      const page = new Page({ name: 'dispatch' });
      ...
      app.initialize();
      await runds.load();
      const resourcesNum = controller.getResourcesNum(item);
      expect(resourcesNum).toBe(27);
    });
  });
});
