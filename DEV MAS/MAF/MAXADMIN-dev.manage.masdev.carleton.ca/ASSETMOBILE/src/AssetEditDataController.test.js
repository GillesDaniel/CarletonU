...
import assetLrm from "./test/lrm-list-json-data";
import linearDirectionList from "./test/linear-direction-json-data";
...
it('should set isLinearValid state to true if LRM, start measure and end measure values are valid', async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  app.client = {
    userInfo: {
  ...
  await app.initialize();
  controller.pageInitialized(page, app);
  page.state.validLrm = true;
  page.state.validLrmStartMeasure = true;
  page.state.validLrmEndMeasure = true;

  await controller.validateLinearFields();
  expect(page.state.isLinearValid).toEqual(true);
});

it('should set validLrmStartMeasure to true if item.startmeasure is defined', async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: 'editasset' });
  let mockValidateLinearFields = jest.fn();
  const ds = newDatasource(assetitem, 'dsEditAsset');
  page.registerDatasource(ds);
  app.registerController(controller);
  app.registerPage(page);
  ...
  controller.validateLinearFields = mockValidateLinearFields;
  controller.validateStartMeasureEdit(event);
  expect(page.state.validLrmStartMeasure).toBe(true);
  await expect(mockValidateLinearFields.mock.calls.length).toEqual(1);
}
);
it('should set validLrmStartMeasure to false if item.startmeasure is undefined', async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: 'editasset' });
  let mockValidateLinearFields = jest.fn();
  ...
  controller.validateLinearFields = mockValidateLinearFields;
  controller.validateStartMeasureEdit(event);
  expect(page.state.validLrmStartMeasure).toBe(false);
  await expect(mockValidateLinearFields.mock.calls.length).toEqual(1);
}
);
it('should set validLrmEndMeasure to true if item.endmeasure is defined', async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: 'editasset' });
  let mockValidateLinearFields = jest.fn();
  ...
  controller.validateLinearFields = mockValidateLinearFields;
  controller.validateEndMeasureEdit(event);
  expect(page.state.validLrmEndMeasure).toBe(true);
  await expect(mockValidateLinearFields.mock.calls.length).toEqual(1);
}
);
it('should set validLrmEndMeasure to false if item.endmeasure is undefined', async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: 'editasset' });
  let mockValidateLinearFields = jest.fn();
  ...
  controller.validateLinearFields = mockValidateLinearFields;
  controller.validateEndMeasureEdit(event);
  expect(page.state.validLrmEndMeasure).toBe(false);
  await expect(mockValidateLinearFields.mock.calls.length).toEqual(1);
}
);

it("it should select LRM", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  const ds = newDatasource(assetitem, "dsEditAsset");
  ...
  const lrmLookup = new Dialog({
    name: "lrmLookupEdit",
  });
  page.registerDialog(lrmLookup);

  lrmLookup.closeDialog = jest.fn();

  const itemSelected = {
    lrm: "MILES",
  };
  await controller.chooseLRMEdit(itemSelected);
  expect(ds.item.lrm).toBe("MILES");
  expect(page.state.validLrm).toBe(true);
});

it("it should select linear direction", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  const ds = newDatasource(assetitem, "dsEditAsset");
  ...
  await controller.chooseLinearDirectionEdit(itemSelected);
  expect(ds.item.direction).toBe("SOUTH");
});

it("it should open LRM dialog", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  let mockedFn = jest.fn();
  page.showDialog = mockedFn;
  const ds = newDatasource(assetitem, "dsEditAsset");
  const lrmDS = newDatasource(assetLrm, 'assetLRMEdit');
  sinon.stub(lrmDS, "clearQBE").returns(true);
  sinon.stub(lrmDS, "searchQBE").returns(assetLrm.member);


  page.registerDatasource(ds);
  app.registerDatasource(lrmDS);
  app.registerController(controller);
  page.state = {
    editorValue: "<p>Test</p>",
  };
  ...
  controller.openLrmLookupEdit();
  expect(page.showDialog.mock.calls).toBeDefined();
});

it("it should open linear direction dialog", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  let mockedFn = jest.fn();
  ...
  const typeDS = newDatasource(linearDirectionList, "synonymdomainData");
  sinon.stub(typeDS, "initializeQbe").returns(true);
  sinon.stub(typeDS, "searchQBE").returns([linearDirectionList.member[0]]);
  sinon.stub(typeDS, "setQBE").returns(true);
  ...
  controller.openLinearDirectionLookupEdit();
  expect(page.showDialog.mock.calls).toBeDefined();
});

it("should set validLrm flag to true if value exists", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  const ds = newDatasource(assetitem, 'dsEditAsset');
  ...
  const lrmDS = newDatasource(assetLrm, 'assetLRMEdit');
  sinon.stub(lrmDS, 'initializeQbe').returns(true);
  sinon.stub(lrmDS, 'searchQBE').returns([assetLrm.member[0]]);
  sinon.stub(lrmDS, 'setQBE').returns(true);

  page.registerDatasource(ds);
  page.registerDatasource(lrmDS);
  app.registerController(controller);
  page.state = {
    editorValue: "<p>Test</p>",
  };
  ...
  await controller.validateLrmEdit(event);
  expect(page.state.validLrm).toBe(true);
});

it("should set validLrm flag to false if value doesn't exist", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  const ds = newDatasource(assetitem, 'dsEditAsset');
  ...
  const lrmDS = newDatasource(assetLrm, 'assetLRMEdit');
  sinon.stub(lrmDS, 'initializeQbe').returns(true);
  sinon.stub(lrmDS, 'searchQBE').returns([]);
  sinon.stub(lrmDS, 'setQBE').returns(true);
  ...
  await controller.validateLrmEdit(event);
  expect(page.state.validLrm).toBe(false);
});
it("should set validLrm flag to true if both values doesn't exist", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  const ds = newDatasource(assetitem, 'dsEditAsset');
...
it("should set validLinearDirection flag to true if value exists", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  const ds = newDatasource(assetitem, "dsEditAsset");
  ...
  await controller.validateLinearDirectionEdit(event);
  expect(page.state.validLinearDirection).toBe(true);
});

it("should set validLinearDirection flag to true if both values doesn't exist", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  const ds = newDatasource(assetitem, "dsEditAsset");
...
it("should set validLinearDirection flag to false if value doesn't exist", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  const ds = newDatasource(assetitem, "dsEditAsset");
  ...
  await controller.validateLinearDirectionEdit(event);
  expect(page.state.validLinearDirection).toBe(false);
});

it("it should verify asset is linear ", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  const ds = newDatasource(assetitem, "dsEditAsset");
  ...
  await app.initialize();
  await ds.load();
  controller.page.state.isLinear = true;

  // ds.currentItem.islinear = false
  controller.handleLinearToggleEdit();
  expect(page.state.isLinear).toBe(false);
});

it("it should verify asset is not linear ", async () => {
  const controller = new AssetEditDataController();
  const app = new Application();
  const page = new Page({ name: "editasset" });
  const ds = newDatasource(assetitem, "dsEditAsset");
  ...
  await app.initialize();
  await ds.load();
  controller.page.state.isLinear = false;
...
it("should open Classification lookup", async () => {
  ...
  app.lastPage = { name: "editasset" };
  await app.initialize();
  controller.openClassificationLookupEdit();
  expect(page.showDialog.mock.calls.length).toBeDefined();
});

it("should not open display warning popup, if asset moved successfully", async () => {
  let mockedFn = jest.fn();
  const controller = new AssetEditDataController();
  const app = {
    currentPage :{
  ...
  const mockShowUnsavedChanges = jest.fn();
  
  page.state.editorValue = "<p>Test</p>";
  app.lastPage = { name: "editasset" };
  app.userInteractionManager.get = mockShowUnsavedChanges;
  

  const moveResponse = {
    "warnings": [{ "message": "BMXAA0067I - Asset 09JANAST in site BEDFORD was moved successfully.", "key": "AssetMoveWasSuccess", "group": "asset" },
    { "message": "BMXAA0067I - Asset 09JANAST in site BEDFORD was moved successfully.", "key": "AssetMoveWasSuccess", "group": "asset" }]
  };
  controller.displayWarningPopup(moveResponse);
