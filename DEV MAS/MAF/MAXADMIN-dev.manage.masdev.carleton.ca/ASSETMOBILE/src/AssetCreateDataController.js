...
class AssetCreateDataController {
  ...
  pageResumed() {
    ...
    this.page.state.linearLicense = this.app.checkLicense("LINEAR");
  ...
  setDefaultValidationParams() {
    ...
    this.page.state.validLinearDirection = true;
    this.page.state.saveInProgress = false;
    this.page.state.isLinear = false;
    this.page.state.validLrm = true;
    this.page.state.validLrmStartMeasure = true;
    this.page.state.validLrmEndMeasure = true;
    this.updateSaveButtonVisibility();
  }

  async reloadClassifications() {
    if (Device.get().isMaximoMobile) {
      let classificationDS = this.app.findDatasource("AssetClassDomain");

      if (classificationDS) {
        await classificationDS.forceReload();
      }
    }
  }
  ...
  async validateLocation() {
    ...
    if (this.page.state.selectedAssetLocation && assetItem.location && assetItem.location !== this.page.state.selectedAssetLocation) {
      items = [];
    ...
    if ((items && items.length > 0) || assetItem.location.trim() == '') {
  ...
  async validateFailureCode() {
    ...
    if ((items && items.length > 0) || assetItem.failurecode.trim() == '') {
  ...
  async validateManufacture() {
    ...
    if ((items && items.length > 0) || assetItem.manufacturer.trim() == '') {
  ...
  async validateVendor() {
    ...
    if ((items && items.length > 0) || assetItem.vendor.trim() == '') {
  ...
  async validateType() {
    ...
    if ((items && items.length > 0) || assetItem.assettype.trim() == '') {
  ...
  async validateLinearDirection(event) {
    const assetItem = this.app.findDatasource("dsCreateAsset").item;
    let ds = this.app.findDatasource("synonymdomainData");
    await ds.initializeQbe();
    ds.setQBE("domainid", "=", "LINEARDIRECTION");
    ds.setQBE("value", "=", assetItem.direction);

    const items = await ds.searchQBE();

    if ((items && items.length > 0) && (event.item.direction.length > 0)) {
      this.clearWarnings("direction");
      this.page.state.validLinearDirection = true;
    } else if (event.item.direction.length > 0) {
      const errorMessage = this.app.getLocalizedLabel(
        `direction_invalid_msg`,
        `BMXAA4190 - Direction is not valid.`
      );
      const errorField = "direction";
      this.showWarnings(errorField, errorMessage);

      this.page.state.validLinearDirection = false;
    } else {
      this.clearWarnings("direction");
    }
  ...
  async validateStartMeasure(event) {
    event.item?.startmeasure >= 0 ? this.page.state.validLrmStartMeasure = true : this.page.state.validLrmStartMeasure = false;
    this.updateSaveButtonVisibility();
  }

  async validateEndMeasure(event) {
    event.item?.endmeasure > 0 ? this.page.state.validLrmEndMeasure = true : this.page.state.validLrmEndMeasure = false;
    this.updateSaveButtonVisibility();
  }

  async validateLrm(event) {
    const assetItem = this.app.findDatasource("dsCreateAsset").item;

    let ds = this.app.findDatasource("assetLRM");
    await ds.initializeQbe();
    ds.setQBE("lrm", "=", assetItem.lrm);

    const items = await ds.searchQBE();

    if ((items.length > 0) && (event.item.lrm.length > 0)) {
      this.clearWarnings("lrm");
      this.page.state.validLrm = true;
    } else if (event.item.lrm.length > 0) {
      const errorMessage = this.app.getLocalizedLabel(
        `lrm_invalid_msg`,
        `BMXAA4191 - LRM is not valid.`
      );
      const errorField = "lrm";
      this.showWarnings(errorField, errorMessage);

      this.page.state.validLrm = false;
    } else {
      this.clearWarnings("lrm");
    }
  ...
  */
  ...
  updateSaveButtonVisibility() {
    const ds = this.page.datasources.dsCreateAsset;
      ...
      !this.page.state.hasAssetNumber ||
      !this.page.state.validType ||
      !this.page.state.validManufacture ||
      !this.page.state.validVendor ||
      !this.page.state.validFailureCode ||
      !this.page.state.validLocation ||
      !this.page.state.validLinearDirection ||
      !this.page.state.validLrm ||
      !this.page.state.validLrmStartMeasure ||
      !this.page.state.validLrmEndMeasure ;
  }
  ...
  async createAsset(evt) {
    ...
    if (asset) {
      ...
      assetCreateResource.currentItem.islinear = this.page.state.isLinear;
      ...
      if (notReadyStatusItem.length > 0) {
        const defaultStatusItem = notReadyStatusItem.find((item) => item.defaults);
        if (defaultStatusItem) {
          assetCreateResource.currentItem.status_description = defaultStatusItem.description;
        }
      }
  ...
  chooseLinearDirection(itemSelected) {
    let dsCreateAsset = this.app.findDatasource("dsCreateAsset");
    dsCreateAsset.item.direction = itemSelected.value;
  ...
  async chooseLRM(itemSelected) {
    let dsCreateAsset = this.app.findDatasource("dsCreateAsset");
    dsCreateAsset.item.lrm = itemSelected.lrm;
    const LRMDialog = this.page.findDialog("lrmLookup");
    //istanbul ignore else
    if (LRMDialog) {
      await LRMDialog.closeDialog();
    }
  ...
  handleLinearToggle() {
    ...
    this.page.state.isLinear = !this.page.state.isLinear;

    if (!this.page.state.isLinear) {
      dsCreateAsset.item.direction = "";
      dsCreateAsset.item.lrm = "";
      dsCreateAsset.item.startmeasure = "";
      dsCreateAsset.item.endmeasure = "";
      dsCreateAsset.item.startdescription = "";
      dsCreateAsset.item.enddescription = "";
      dsCreateAsset.updateRequired('lrm', false, dsCreateAsset.item, false)
      dsCreateAsset.updateRequired('startmeasure', false, dsCreateAsset.item, false)
      dsCreateAsset.updateRequired('endmeasure', false, dsCreateAsset.item, false)
      this.page.state.validLrm = true;
      this.page.state.validLrmStartMeasure = true;
      this.page.state.validLrmEndMeasure = true;
      this.clearWarnings("lrm");
      ...
      dsCreateAsset.updateRequired('lrm', true, dsCreateAsset.item, false)
      dsCreateAsset.updateRequired('startmeasure', true, dsCreateAsset.item, false)
      dsCreateAsset.updateRequired('endmeasure', true, dsCreateAsset.item, false)
      this.clearWarnings("lrm");
      this.clearWarnings("startmeasure");
      this.clearWarnings("endmeasure");
      this.page.state.validLrm = false;
      this.page.state.validLrmStartMeasure = false;
      this.page.state.validLrmEndMeasure = false;
  ...
  async updateSpecificationAttributes(assetSpecItems) {
    ...
    if (assetSpecItems.length > 0) {
      assetSpecItems.forEach((element) => { specId.push(element.assetattrid); });
  ...
  async openLinearDirectionLookup() {
    let synonymDS = this.app.findDatasource("synonymdomainData");
    await synonymDS.initializeQbe();
    synonymDS.setQBE("domainid", "=", "LINEARDIRECTION");
    await synonymDS.searchQBE();

    this.page.showDialog("linearDirectionLookup");
  }

  async openLrmLookup() {
    let assetLRM = this.app.findDatasource("assetLRM");
    assetLRM.clearQBE();
    assetLRM.searchQBE();
    this.page.showDialog("lrmLookup");
  }
