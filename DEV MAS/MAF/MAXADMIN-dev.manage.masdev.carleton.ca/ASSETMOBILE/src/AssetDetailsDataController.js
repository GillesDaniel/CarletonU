...
class AssetDetailsDataController {
  ...
  computedStartLinearDescription(item) {
    let startValue = '';

    //istanbul ignore else
    if(item?.startmeasure != null) {
      startValue = item?.startmeasure;
    }

    //istanbul ignore else
    if(item?.startdescription) {
      startValue = startValue + ' ' +  item?.startdescription;
    }
    
    return startValue;
  }

  computedEndLinearDescription(item) {
    let endValue = '';

    //istanbul ignore else
    if(item?.endmeasure != null ) {
      endValue = item?.endmeasure;
    }

    //istanbul ignore else
    if(item?.enddescription) {
      endValue = endValue + ' ' +  item?.enddescription;
    }
 ...
 computedServiceAdd(item) {
  const selectedFields = ['streetaddress', 'addressline2', 'addressline3', 'city', 'regiondistrict', 'country', 'stateprovince', 'postalcode'];

  const serviceAddressDSItem = item;


  const selectedValues = selectedFields.map(field => serviceAddressDSItem[field]).filter(value => value !== undefined);;
  const addressString = selectedValues.join(', ');
