  ...
  member: [
    {
      ...
      moddowntimehist_collectionref:
        "oslc/os/mxapiasset/_MTMxMTAvQkVERk9SRA--/moddowntimehist",
...
it("should open create Asset page", async () => {
  ...
  const asset = {
    ...
    moddowntimehist_collectionref:
      "oslc/os/mxapiasset/_MTk3OS9CRURGT1JE/moddowntimehist",
  ...
  let addNewstub = sinon
    .stub(ds, "addNew")
    .returns({ assetid: "1234", assetnum: 0 });
...
it("should save created Asset with system default status description if no default synonym exists", async () => {
  const controller = new AssetCreateDataController();
  const app = new Application();
  app.client = {
    userInfo: {
  ...
  const typeDS = newDatasource(assettypes, "synonymdomainData");
  sinon.stub(typeDS, "initializeQbe").returns(true);
  sinon.stub(typeDS, "searchQBE").returns([assettypes.member[1]]);
  sinon.stub(typeDS, "setQBE").returns(true);
  page.registerDatasource(typeDS);
  ...
  window.setTimeout(() => {
    // status description will be undefined in mobile app as it loads data locally
    expect(ds.currentItem.status_description).toBe(undefined);
    expect(savestub.called).toBe(true);
    expect(savestub.displayName).toBe("save");
  }, 500);
...
it("should save created Asset while selecting classification ", async () => {
  ...
  const assetAttributeDS = newDatasource(
    assetAttributedata,
    "assetAttributeDS"
  );
  ...
  sinon
    .stub(assetAttributeDS, "searchQBE")
    .returns([assetAttributedata.member[0]]);
  ...
  const classListDummyDS = newClassificationDS(
    classificationData,
    "classListDummyDS"
  );
...
it("it should open linear direction dialog", async () => {
  ...
  controller.openLinearDirectionLookup();
  expect(page.showDialog.mock.calls).toBeDefined();
});
...
it("it should open LRM dialog", async () => {
  ...
  page.showDialog = mockedFn;
  const ds = newDatasource(assetitem, "dsCreateAsset");
  const lrmDS = newDatasource(assetLrm, 'assetLRM');
...
it("it should select linear direction", async () => {
  ...
  await controller.chooseLinearDirection(itemSelected);
...
it("it should select LRM", async () => {
  ...
  await controller.chooseLRM(itemSelected);
...
/**
 * Test case to verify the functionality of the validateLinearDirection method.
 */
...
it("should set validLinearDirection flag to true if value exists", async () => {
  ...
  await controller.validateLinearDirection(event);
  expect(page.state.validLinearDirection).toBe(true);
});

// Generated by WCA for GP

it("should set validLinearDirection flag to false if value does not exist", async () => {
  const controller = new AssetCreateDataController();
  const app = new Application();
  const page = new Page({ name: "createasset" });
  const ds = newDatasource(assetitem, 'dsCreateAsset');
...
it("should set start measure value for required ", async () => {
  const controller = new AssetCreateDataController();
  const app = new Application();
  const page = new Page({ name: "createasset" });
  const ds = newDatasource(assetitem, 'dsCreateAsset');
  ...
  await controller.validateStartMeasure(event);
  expect(page.state.validLrmStartMeasure).toBe(true);
});

it("should validate blank start measure value for required ", async () => {
  const controller = new AssetCreateDataController();
  const app = new Application();
  const page = new Page({ name: "createasset" });
  const ds = newDatasource(assetitem, 'dsCreateAsset');
...
it("should set end measure value for required ", async () => {
  const controller = new AssetCreateDataController();
  const app = new Application();
  const page = new Page({ name: "createasset" });
  const ds = newDatasource(assetitem, 'dsCreateAsset');
  ...
  await controller.validateEndMeasure(event);
  expect(page.state.validLrmEndMeasure).toBe(true);
});

it("should validate end measure value for required if user does not enter", async () => {
  const controller = new AssetCreateDataController();
  const app = new Application();
  const page = new Page({ name: "createasset" });
  const ds = newDatasource(assetitem, 'dsCreateAsset');
...
// Generated by WCA for GP
/**
 * Test case to verify the functionality of the validateLrm method.
 */
  ...
  it("should set validLrm flag to true if value exists", async () => {
    ...
    await controller.validateLrm(event);
    expect(page.state.validLrm).toBe(true);
  });

  it("should set validLrm flag to false if value does not exist", async () => {
    const controller = new AssetCreateDataController();
    const app = new Application();
    const page = new Page({ name: "createasset" });
    const ds = newDatasource(assetitem, 'dsCreateAsset');
