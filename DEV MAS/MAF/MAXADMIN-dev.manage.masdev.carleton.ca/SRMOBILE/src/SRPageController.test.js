...
it("should load completed and unsynced SR list data", async () => {
  ...
  //Test filter SR method separately
  await controller.filterSrByStatus(["CANCELLED", "CLOSED", "REJECTED", "HISTEDIT", "RESOLVED"]);
});
...
it("should display proper information of SRs created on device", async () => {
  let app = await getApp();
  let page = app.currentPage;
  let controller = page.controllers[0];
  ...
  //Set unsynced dropdown as selected
  page.state.selectedDropdown = "unsyncedrequests";

  //Call load of unsync list
  app.state.refreshActiveRequests = true;
  await controller.pageResumed(page);

  //Call load of unsync individual record
  const item = {
    href: "TEMP/TEST123",
    record: jest.fn(),
    transaction: jest.fn(),
    computedDoclinksCount: 5,
    computedWorklogCount: 2
  }
  controller.showSRDetail(item);

  //Selected SR info should be the same as the one comming from list
  expect(app.state.selectedSR.computedDoclinksCount).toEqual(5);
  expect(app.state.selectedSR.computedWorklogCount).toEqual(2);
});
