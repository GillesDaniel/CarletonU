...
import numericDomData from './test/test-numericdom-data.js';
...
describe("AppController", () => {
  ...
  it("must handle exceptions during loadings", async () => {
    let app = await getApp();
    const controller = app.controllers[0];

    //Force system property failure
    delete app.client.systemProperties;
    controller.loadSystemProperties();

    //Force maxvars failure
    delete app.datasources;
    controller.loadMaxVars();
  });
