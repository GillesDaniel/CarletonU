...
class CountBookDetailPageController {
  ...
  pageResumed(page, app) {
    ...
    this.page.state.shouldLoadAll = true;
  ...
  */
  async getAllItemsFromDataSource(datasource, forceSync) {
    this.page.state.shouldLoadAll = false;
    const allitems = [];
    let allLoaded = true;
    ...
    // for all loaded case, all items loaded, initializeItemsOnTab won't be called
    // istanbul ignore else
    if (allLoaded) {
      this.app.state.countbooktabloading = false;
    }
  ...
  async onAfterLoadData(dataSource, items) {
    ...
    if (this.app.state.isMobileContainer && !this.page.state.isfromsave && dataSource.name === this.getDataSouceName_countBookDetailDSALL()) {
      ...
      if (this.page.state.shouldLoadAll) {
        const self = this;
        // must use setTimeout here to get item loaded result other than undefined
        setTimeout(() => {
          // istanbul ignore next
          self.getAllItemsFromDataSource(dataSource, false);
        }, 100);
      }
    }

    // istanbul ignore else
    if (this.app.state.isMobileContainer && !this.page.state.isfromsave && dataSource.name === this.getCountBookDSName4Counted()) {
      for (var each of items) {
        this.computeMatch(each);
      }
    }
    
    // istanbul ignore else
    if (!this.app.state.isMobileContainer && dataSource.name === this.getDataSouceName_countBookDetailDS4Saving()) {
      for (var each of items) {
        this.computeMatch(each);
        this.page.state.allitems.push(each);
      }
  ...
  async loadInCountedCBLineDS(items) {
    ...
    cnLineList.clearState();
    let newCBLineItems = { cbLineItems: cbLineItems };
    await cnLineList.load({ noCache: true, src: newCBLineItems, searchQBE: prevSearch });
  }
  ...
  updateItemsIdValueMap(cbLineItems) {
    ...
    cbLineItems.forEach((item, i) => {
      ...
      this.computeMatch(cbLineItems[i]);
    });
  }
  ...
  async updatePhyscnt(args) {
    ...
    if (args.haswarning || args.physcnt === undefined || args.physcnt === null || args.physcnt === "" || (args.rotating && (args.physcnt > 1 || args.physcnt < 0))) {
      ...
      moment(this.page.state.counteditem_id_date_map_4countbook.get(args.countbooklineid)).format('MM/DD/YYYY') === moment(currentDate).format('MM/DD/YYYY'))
    {
      return;
    }
  ...
  async computeMatch(args) {
    ...
    const notMatchMessage = this.app.getLocalizedLabel(
      'notMatch',
      'The physical count does not match the current balance.'
    );
    //istanbul ignore else  
    if (args.accuracy == 100) {
      ...
      if (args.accuracy - 100 > args.realinvptol) {
        args.match = false;
        args.errormsgtext = notMatchMessage;
      } else {
        args.match = true;
      }
    } else if (args.accuracy < 100) {
    ...
    const exceedlimitMessage = this.app.getLocalizedLabel(
      'exceedlimit',
      'The item amount that you entered might exceed the allowed accuracy limit.'
    );
    if(!args.rotating && (args.physcnt !== 0 && args.curbal >0 && (args.physcnt > 10*args.curbal || args.physcnt < (args.curbal/10)))){
      args.errormsgtext = exceedlimitMessage;
      args.notexceed = false;
    }else{
      args.notexceed = true;
    }
  }
  ...
  async refreshCountBookList(){
    if(this.app.device.isMaximoMobile){
      const countBook = this.app.datasources.countbookds4comp.find( cb => cb.href===this.page.params.href); 
      countBook.status =this.statusDomain.COMP.value;
      countBook.status_description =this.statusDomain.COMP.description;
      countBook.status_maxvalue = this.statusDomain.COMP.value;
      countBook.status = this.statusDomain.COMP.value;
      this.app.datasources.countbookds4comp.save({mobileLocalOnly:true}) ;
    }

    // istanbul ignore else
    if (!this.app.datasources.countbookds4comp.getSchema()) {
      await this.app.datasources.countbookds4comp.initializeQbe();
    }
    
    this.app.datasources.countbookds4comp.searchQBE(undefined, true, {mobileFullTableSearch: true});
  }
  async completeCountbook() {
    ...
    if (cancomplete) {
      ...
      try {
        ...
        await this.refreshCountBookList();
      } catch (error) {
        // istanbul ignore next
        log.t(TAG, error);
      } finally {
