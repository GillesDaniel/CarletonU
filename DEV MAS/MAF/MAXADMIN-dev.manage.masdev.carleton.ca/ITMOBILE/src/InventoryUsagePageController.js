...
const SHIP_PO_REQUIRED = "sysMsgDialog_poRequired"
...
class InventoryUsagePageController {
      ...
      if (typeList.includes(this.page.params.usetype)) {
        ...
        this.page.state.finalActionLabel =
          this.page.state.usetype_maxvalue === ISSUE
            ? this.app.getLocalizedLabel("issue_button", "Complete issue")
            : this.app.getLocalizedLabel(
                "transfer_button",
                "Complete transfer"
              );
  ...
  async setUpInitialDataSource(reserveditems) {
    ...
    if (!this.page.state.itemUrl) {
      ...
      if (!this.page.state.addingmoreitems) {
       ...
       if(invUsageItem.length > 0) {
          invUsageItem.forEach((invusage) => {
                  if(invusage.invuseline.length > 0){
      ...
      this.invuselines = invuseline.map((item) =>
        Object.assign(item, {
          ...
          wonum:
            item.invreserve &&
            item.invreserve[0] &&
            item.invreserve[0].workorder &&
            item.invreserve[0].workorder[0]
              ? item.invreserve[0].workorder[0].displaywonum
              : item.workorder && item.workorder[0]
              ? item.workorder[0].displaywonum
              : item.wonum,
          taskid:
            item.invreserve &&
            item.invreserve[0] &&
            item.invreserve[0].workorder &&
            item.invreserve[0].workorder[0]
              ? item.invreserve[0].workorder[0].displaytaskid
              : item.workorder && item.workorder[0]
              ? item.workorder[0].displaytaskid
              : item.taskid,
          enteredastask:
            item.invreserve &&
            item.invreserve[0] &&
  ...
  async updateByCopyInfoFromNewSelected(
    ...
    item.wonum = newSelectedItem.wogroup;

    item.taskid = newSelectedItem.taskid;

    if (newSelectedItem.taskid !== undefined) {
      item.enteredastask = true;
    }
    ...
    item.anywhererefid = new Date().getTime() * 100 + numdefined;
    return item;
  }
  ...
  async saveInventoryUsage(needsSave = true) {
    ...
    // TODO - should use wonum + taskid + enteredastask (=1) when there is taskid
    // But currently it's not working when setting both wonum + taskid from mobile.
    // It only supports using refwo for now. This needs Maximo side code update.
    // istanbul ignore else
    // if (this.invUsageDS.item) {
    //   this.invUsageDS.item.invuseline?.forEach((line) => {
    //     //istanbul ignore else
    //     if (line["requestnum"] === undefined && line["taskid"] !== undefined) {
    //       // Should remove refwo and set refwo from maximo side. Just it's not working for now.
    //       delete line["refwo"];
    //     }
    //   });
    // }
      ...
      if (lineItem.invuselinesplit && lineItem.invuselinesplit.length) {
        ...
        lineItem.invuselinesplit.forEach((splitItem) => {
          ...
          if (!splitItem.anywhererefid) {
            splitItem.anywhererefid = lineItem.anywhererefid * 100 + index;
          }
        });
        //istanbul ignore else
  ...
  async triggerSaveProcess(ds) {
    ...
    if (response.error) {
      ...
      // update this.invusagejsonds
      let invuselineList = [];
      let uniqueinvuselineList = [];
      let oldInvUsageJSONItems = this.invusagejsonds.items;

      if(this.app.device.isMaximoMobile){
         uniqueinvuselineList = ds.currentItem?.invuseline?.filter((item) => item.itemnum && item._id != undefined);
      }else{
         uniqueinvuselineList = ds.currentItem?.invuseline;
      }

      //istanbul ignore else

      //logic to remove the unwanted items without itemnum
   if(uniqueinvuselineList && uniqueinvuselineList.length > 0){
        uniqueinvuselineList?.forEach((invuseline) => {
    ...
    for (let key in changes) {
      for (let field of fields) {
        ...
        if (changes[key][field] !== undefined) {
          // wonum is set in openDetailsLineItem for display purpose. Usually it's not changed because it's already set using information from workorder.
          // This happens when user entered an non-existing wonum from mobile and save, while no actual work order data, thus wonum was set as undefined.
          // Then in openDetailsLineItem it was set to value of refwo.
          if (
            field === "wonum" &&
            changes[key][field][0].object.wonum ===
              changes[key][field][0].object.refwo &&
            typeof changes[key][field][0].object.enteredastask === "boolean" &&
            changes[key][field][0].object.enteredastask === false
          ) {
            invuselineChanged = false;
          } else {
  ...
  async openDetailsLineItem(item) {
    ...
    // For display purpose. On UI we use wonum for editable field display.
    if (this.page.state.invUsageLineItem.enteredastask) {
      //istanbul ignore else
      if (
        this.page.state.invUsageLineItem.workorder !== undefined &&
        this.page.state.invUsageLineItem.workorder.length > 0
      ) {
        this.page.state.invUsageLineItem.wonum =
          this.page.state.invUsageLineItem.workorder[0].displaywonum;
        this.page.state.invUsageLineItem.taskid =
          this.page.state.invUsageLineItem.taskid;
      }
    } else {
      this.page.state.invUsageLineItem.wonum =
        this.page.state.invUsageLineItem.refwo;
    }
  ...
  async deleteAsset(item) {
    ...
    if (this.invUsageDS.state.currentItemIndex !== -1) {
      ...
      childInvUseLineDS.getById(
        childInvUseLineDS.getId(this.page.state.invUsageLineItem),
        true
      );
  ...
  onDetailClose() {
    ...
    // Sets back refwo if there is existing refwo in the scenario that user updated wonum
    // Both undefined and false, it's treated as enteredastask false - not task level
    if (
      this.page.state.invUsageLineItem.enteredastask === undefined ||
      (typeof this.page.state.invUsageLineItem.enteredastask === "boolean" &&
        this.page.state.invUsageLineItem.enteredastask === false)
    ) {
      this.page.state.invUsageLineItem.refwo =
        this.page.state.invUsageLineItem.wonum;
    } else {
      // TODO Remove attribute of refwo to update - need to remove refwo and let maximo side set the new refwo.
    }
  ...
  async deleteSplit(skipClose) {
    ...
    if (this.invUsageDS.state.currentItemIndex !== -1) {
      ...
      const childInvSplitDS = childInvUseLineDS.getChildDatasource(
        ...
        {
          idAttribute: "invuselinesplitid",
          query: { relationship: "invuselinesplit" },
        }
      );
      const splitItems = await childInvSplitDS.load();
      // skip if the split item not saved
  ...
  /**
   * On Change Method for field wonum
   */
  // onWonumChange(invUsageLineItem) {
  //   invUsageLineItem.taskid = undefined;
  //   this.validateInvUsageLineItem(invUsageLineItem);
  // }

  // istanbul ignore next
  onWonumTaskIdChange(invUsageLineItem) {
    let pageStateItem = this.page.state.invUsageLineItem;
    if (invUsageLineItem.taskid !== undefined) {
      // Needs to set to wonum instead of refwo
      // because when there is taskid, refwo should be like Txxxx, which needs to be set from maximo side.
      // istanbul ignore else
      //invUsageLineItem.refwo = undefined;
      if (pageStateItem) {
        pageStateItem.taskid = invUsageLineItem.taskid;
        pageStateItem.enteredastask = true;
      } else {
        //invUsageLineItem.wonum = undefined;
        pageStateItem.enteredastask = false;
      }
    }

    if (pageStateItem) {
      if (invUsageLineItem.wonum !== undefined) {
        pageStateItem.wonum = invUsageLineItem.wonum;
      }
    }

    if (pageStateItem) {
      let invUseLineItemDS = this.invUsageDS.item.invuseline;
      for (let i = 0; i < invUseLineItemDS.length; i++) {
        if (pageStateItem.invuselinenum === invUseLineItemDS[i].invuselinenum) {
          invUseLineItemDS[i].wonum = pageStateItem.wonum;
          if (invUsageLineItem.enteredastask) {
            invUseLineItemDS[i].taskid = pageStateItem.taskid;
          }
        }
      }
    }
  ...
  validateInvUsageLineItem(invUsageLineItem) {
    ...
    this.page.state.invUsageLineItemFields.required.fromCondition =
      ...
      invUsageLineItem.conditionenabled;
    ...
    if (this.page.state.usetype_maxvalue === ISSUE) {
      ...
      if (invUsageLineItem.item?.rotating) {
        ...
        if (invUsageLineItem.taskid !== undefined) {
          ...
          // istanbul ignore else
          //if (invUsageLineItem.wonum === undefined) {
          //  invUsageLineItem.wonum = invUsageLineItem.refwo;
          //}
          //invUsageLineItem.refwo = undefined;
          invUsageLineItem.enteredastask = true;
        } else {
          //invUsageLineItem.wonum = undefined;
          invUsageLineItem.enteredastask = false;
        }
        
        this.page.state.invUsageLineItemFields.required.location =
          invUsageLineItem.location === undefined &&
          invUsageLineItem.assetnum === undefined &&
          (!invUsageLineItem.wonum || invUsageLineItem.wonum?.trim() === "");
        this.page.state.invUsageLineItemFields.required.asset =
          invUsageLineItem.location === undefined &&
          invUsageLineItem.assetnum === undefined &&
          (!invUsageLineItem.wonum || invUsageLineItem.wonum?.trim() === "");

        //let refwo = invUsageLineItem.refwo;
        let wonum = invUsageLineItem.wonum;
        this.page.state.invUsageLineItemFields.required.refwo =
          invUsageLineItem.location === undefined &&
          invUsageLineItem.assetnum === undefined &&
          (!wonum || wonum.trim() === "");
  ...
  async loadOptions() {
    let addoptions = [
      ...
      //Remove menus should not be displayed
      // {
      //   _id: 2,
  ...
  shipValidation() {
    ...
    let txfrreqpo = this.invUsageDS?.item?.txfrreqpo;
    const poNum = new Set();
    const itemList = [];
    let itemId = 0;

    this.invUsageDS?.item?.invuseline?.forEach((item) => {
      poNum.add(item.ponum);
    })
    this.invusagejsonds.getItems().forEach((item) => {
    ...
    this.loadMsgDS(itemList);
         //MAXMOA-2904
         if(txfrreqpo === "ORG" && poNum.has(undefined)){
          this.page.state.poRequired = true;
       }else if(txfrreqpo === "SITE" && poNum.has(undefined)){
         this.page.state.poRequired = true;
       }else{
           this.page.state.poRequired = false;
       }
   
   
       return toSiteSet.size === 1 && this.page.state.poRequired === false;
  }
  ...
  doShip() {
    if (this.shipValidation()) {
      ...
      this.page.state.shipVlidationErr_inconsistent =
        ...
        if(this.page.state.poRequired){
          this.page.showDialog("sysMsgDialog_poRequired_shipped");
        }else{
  ...
  */
  ...
  handleWOBarcodeScan(scandata) {
    this.page.state.invUsageLineItem.wonum = scandata.value;
    this.validateInvUsageLineItem(this.page.state.invUsageLineItem);
  }
  ...
  */
  ...
  checkTransferMaxvar() {
    ...
    let txfrreqpo = currentUsage.txfrreqpo;
    ...
    currentUsage.invuseline?.forEach((item) => {
        poNum.add(item.ponum);
    })
    ...
    this.invusagejsonds.getItems().forEach((item) => {
      ...
      // istanbul ignore else
      if (toorgid === "") {
        toorgid = item.toorgid || this.app.client.userInfo.defaultOrg;
      }
      // istanbul ignore else
      if (tositeid === "") {
  ...
  doTransfer() {
    ...
    // istanbul ignore if
    if(this.page.state.poRequired){
      this.page.showDialog(SHIP_PO_REQUIRED);
    } else {
