...
it('Load onAfterLoadData with app.state.isMobileContainer = true', async () => {
  ...
  countBookDetailPageController.onBeforeLoadData(newDetailDatasource4ALL);
  await countBookDetailPageController.onAfterLoadData(newDetailDatasource4ALL, countBookLineData.member);
  await countBookDetailPageController.onAfterLoadData(countBookDetaildsCounted, countBookLineData.member);
...
it('Load onAfterLoadData with app.state.isMobileContainer = false', async () => {
  ...
  await countBookDetailPageController.onAfterLoadData(countBookDetaildsCounted, items);

  app.state.isMobileContainer = true;
  newDetailDatasource4ALL.dataAdapter.totalCount = 100;
  app.state.loadcount = 10;
  countBookDetailPageController.onAfterLoadData(newDetailDatasource4ALL, [1]);

  const newDetailDatasource4ALL2 = ICTestUtil.registerNewDetailAllDatasource(countBookDetailPage, {member: []}, 'countBookDetailDSDummy2');

  newDetailDatasource4ALL2.dataAdapter.totalCount = 1;
  await countBookDetailPageController.getAllItemsFromDataSource(newDetailDatasource4ALL2);
});
