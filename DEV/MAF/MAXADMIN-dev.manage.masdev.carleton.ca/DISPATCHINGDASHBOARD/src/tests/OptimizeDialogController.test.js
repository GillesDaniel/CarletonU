...
describe('OptimizeDialogController', () => {
  it('save optimization parameters and invoke optimizer', async () => {
    ...
    page.state.runlatesthref = 'href1';
    page.params.projectname = 'BST';
    ...
    expect(setCurrentPageSpy).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        params: expect.objectContaining({ href: expect.anything() }),
      })
    );
  ...
  it('save optimization parameters and invoke optimizer, no redirect', async () => {
    ...
    const dialogSpy = jest.spyOn(app, 'showDialog');
    ...
    expect(app.toast).toHaveBeenCalledWith(
      expect.stringContaining('Optimization started for'),
      'info'
    );
    ...
    expect(dialogSpy).toHaveBeenCalledWith('optimizationInProgressDialog');
  });
  ...
  it('toast notification after unsuccessful optimization', async () => {
    ...
    expect(invokeAction.called).toBe(true);
    await expect(app.toast.mock.calls.length).toBe(1);
    expect(app.toast).toHaveBeenCalledWith('Failed to optimize', 'error');
    expect(getLocalizedLabelSpy).toHaveBeenCalledWith(
