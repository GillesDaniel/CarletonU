...
class DispatchTableController {
  ...
  async onAfterLoadData(dataSource, items) {
    ...
    const jemergencyKPIds = this.app.findDatasource('jemergencyKPIds');
    DispatchTableController.resetDataSource(jemergencyKPIds);
    ...
    await jemergencyKPIds.load({
      src: { emergencies: totalEmergencyActivities },
    });
  }
  ...
  getResourcesNum(projectItem) {
    ...
    runds.forEach((runItem) => {
      if (
        ...
        resourcesNum = kpiValues.resourcesNum;
      }
    });
    return resourcesNum;
  }
