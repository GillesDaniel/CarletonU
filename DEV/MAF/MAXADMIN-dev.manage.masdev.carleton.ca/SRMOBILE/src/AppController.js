...
class AppController {
	...
	if (incomingContext && incomingContext.editTrans)
	{
		this.app.setCurrentPage(
			{ 	
				name: 'createSR', 
				resetScroll: true, 
				params:	{href: incomingContext.href, editTrans: true}
			}
		);
	}
  }
  ...
  initializeMap() {
    try {
      ...
      // istanbul ignore next -- map not valid during Jest
      this.mapPreloadAPI
        .validateMapConfiguration(this.app)
        .then(validMapConfiguration => {
          //istanbul ignore next (Map is not valid during Jest)
    ...
    } catch (err) /* istanbul ignore next (Could not find a way to force exception in test) */ {
      log.e(TAG, "Error: " + err);
  ...
  async loadMaxVars() {
    try {
      ...
      if(orgDS.items && orgDS.items.length) {
        this.app.state.maxvarCoordinate = orgDS.items[0].mobilemaxvars.filter(item => item.varname === 'COORDINATE')[0]?.varvalue;
      }
    } catch (err) {
      log.e(TAG, "Error: " + err);
  ...
  async loadNumericValues() {
    this.app.state.numericDom.loading = true;
    this.app.state.numericDom.priorityDescriptionList = {};
    try {
      let numericDomDS = this.app.findDatasource("numericDomainDS");
      await numericDomDS.initializeQbe();
      numericDomDS.setQBE("domainid", "=", "TICKETPRIORITY");
      await numericDomDS.searchQBE().then((response) => {
        response.forEach((element) => {
          this.app.state.numericDom.priorityDescriptionList[element.value] = element.description;
        });
      });
    } finally {
      this.app.state.numericDom.loading = false;
    }
  }
