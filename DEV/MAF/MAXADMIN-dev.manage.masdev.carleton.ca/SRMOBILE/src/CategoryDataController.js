...
class CategoryDataController {
  ...
  onAfterLoadData(ds, items) {
    ...
    items.forEach((item) => {
      ...
      if (this.app.state.isMobileContainer) {
        ...
        if (!ds.dataAdapter.items) {
          ds.dataAdapter.items = [];
        }
        ds.dataAdapter.items.push(item);
      }
    });
  }
  ...
  /**
   * Do nothing for top categories as they do not use display icon
   */
  calculateDisplayIcon() {
    return "";
  }
