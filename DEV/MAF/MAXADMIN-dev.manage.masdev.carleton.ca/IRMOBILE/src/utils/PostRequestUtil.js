...
const setRealValues2AddedNewDSRec = (selecteditem, actionname, item, app) => {
  ...
  item.issuetype = app.state.issuetypereceipt;
  //istanbul ignore else
  if (actionname) {
    if (actionname === ACTIONNAME_RETURN) {
      item.issuetype = app.state.issuetypereturn;
      //istanbul ignore else
    ...
    if (actionname === ACTIONNAME_VOIDRECEIPT) {
      item.issuetype = app.state.issuetypevoid;
      //istanbul ignore else
      if (selecteditem.qtyrequested !== undefined) {
    ...
    //istanbul ignore else
    if (
      actionname === ACTIONNAME_RECEIVE &&
      selecteditem.lottype_maxvalue === LOT
    ) {
