...
class ActionsPageController {
  ...
  pageResumed(page, app) {
    ...
    let dsList = [
      this.polineds,
      this.polistmatrectrans,
      this.recds,
      this.matds,
      this.assetinputds, 
      this.assetrtds,
      this.materialrtds, 
      this.voidreceiptinputds, 
      this.receiptds,
      this.dspolist,
      this.actionds,
      this.actionds2, 
  ...
  clearDS(dsList) {
    for (let i=0; i < dsList.length ; i++) {
      dsList[i].clearState();
      dsList[i].resetState();
      dsList[i].resetSearchAttributes();
    }
  }
    ...
    this.polineitems.forEach((item) => {
      promisePool.push(
        CommonUtil.computedRecInspRemaining(
          {
            matrectransitems: this.matrectransitems,
  ...
  onAfterLoadData(dataSource, items) {
    ...
    // istanbul ignore else
    if (dsName === POLINEDS) this.polineitems = items;
    // istanbul ignore else
    if (dsName === POLISTMATRECTRANSDS) this.matrectransitems = items;
    // istanbul ignore else
  ...
  async actionMenu(page) {
    ...
    await this.dspolist.load({ noCache: true, itemUrl: page.params.href });
  }
