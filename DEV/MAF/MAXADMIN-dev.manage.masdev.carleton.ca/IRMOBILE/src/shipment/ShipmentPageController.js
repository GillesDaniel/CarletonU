...
class ShipmentPageController {
  ...
  pageResumed(page, app) {
    ...
    this.setIssueTypeDomain();
  }
  async setIssueTypeDomain() {
    let synonymDomainsDS = this.app.findDatasource("synonymdomainDS");
    synonymDomainsDS.setQBE("domainid", "ISSUETYP");
    let domainResponse = await synonymDomainsDS.searchQBE();
    domainResponse.forEach((domainrec) => {
      // istanbul ignore else
      if (domainrec.valueid === "ISSUETYP|SHIPRECEIPT") {
        this.app.state.issuetypeshipreceipt = domainrec.value;
      }
      // istanbul ignore else
      if (domainrec.valueid === "ISSUETYP|SHIPRETURN") {
        this.app.state.issuetypeshipreturn = domainrec.value;
      }
      // istanbul ignore else
      if (domainrec.valueid === "ISSUETYP|VOIDSHIPRECEIPT") {
        this.app.state.issuetypeshipvoid = domainrec.value;
      }
      // istanbul ignore else
      if (domainrec.valueid === "ISSUETYP|TRANSFER") {
        this.app.state.issuetypetransfer = domainrec.value;
      }
      // istanbul ignore else
      if (domainrec.valueid === "ISSUETYP|SHIPTRANSFER") {
        this.app.state.issuetypeshiptransfer = domainrec.value;
      }
    });
  }
    ...
    newShipmentLineDS.items.forEach((item) => {
      promisePool.push(
        CommonUtil.computedRecInspRemaining(
          {
            shipmentMatrectransItems: newMatrectransDS.items,
  ...
  onAfterLoadData(dataSource, items) {
    const dsName = dataSource.name;
    // istanbul ignore else
    if (dsName === SHIPMENTDS) this.shipmentList = items;
    // istanbul ignore else
    if (
