...
class ShipmentActionsPageController {
  ...
  onAfterLoadData(dataSource, items) {
    ...
    // istanbul ignore else
    if (dsName === SHIPMENTLINEDS) this.shipmentlineItems = items;
    // istanbul ignore else
    if (dsName === SHIPMENTMATRECTRANSDS) this.shipmentMatrectransItems = items;
    // istanbul ignore else
    ...
    this.shipmentlineItems.forEach((item) => {
      promisePool.push(
        CommonUtil.computedRecInspRemaining(
          {
            shipmentMatrectransItems: this.shipmentMatrectransItems,
