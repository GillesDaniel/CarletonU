...
const computedOthersRemaining = async (data, shipmentnum, app) => {
  const result = {};
  result[SHIPRETURN_PAGE] = [];
  result[SHIPVOID_PAGE] = [];
  result[ASSET_PAGE] = [];
  result[SHIPRECEIPT_PAGE] = [];
  const shipmentMatrectransItems = data.shipmentMatrectransItems;
  ...
  const masterlist = [];
  SHIPTRANSFER = app.state.issuetypeshiptransfer;
  ISSUETYPETRANSFER = app.state.issuetypetransfer;
  SHIPRECEIPT = app.state.issuetypeshipreceipt;
  SHIPRETURN = app.state.issuetypeshipreturn;
  VOIDSHIPRECEIPT = app.state.issuetypeshipvoid;

  shipmentMatrectransItems.forEach((receipt) => {
  ...
  shipmentMatrectransItems.forEach((receipt) => {
    ...
    if (receipt.issuetype && receipt.issuetype !== SHIPTRANSFER) {
      ...
      let rotassetnum = lines[0] ? lines[0].rotassetnum : "";
  ...
  if (isMaximoMobile) {
    ...
    localReceiptTransactions.forEach((receipt) => {
      ...
      if (
        ...
        let rotassetnum = lines[0] ? lines[0].rotassetnum : "";
        let description = lines[0] ? lines[0].description : "";
...
const calculateQuantity = (
  ...
  matreclist.forEach((matrec) => {
    ...
    // istanbul ignore else
    if (
      ([SHIPRECEIPT, ISSUETYPETRANSFER].includes(matrec.issuetype) &&
      ...
      localmatreclist.forEach((localmatrec2) => {
        ...
        if (
          isFollowup &&
          [SHIPRETURN, VOIDSHIPRECEIPT].includes(localmatrec2.issuetype)
        ) {
        ...
        if (isFollowup) {
          if (
            ...
            localmatrec2.issuetype !== VOIDSHIPRECEIPT
          ) {
            ...
            // istanbul ignore else
            if (localmatrec2.issuetype === VOIDSHIPRECEIPT) {
              qtyToBeReturnedlocal += Number(localmatrec2.receiptquantity);
              qtyToBeVoidlocal += Number(localmatrec2.receiptquantity);
              inspectquantitysum += Number(localmatrec2.receiptquantity);
            }
      ...
      if (!local) {
        ...
        matreclist.forEach((matrec2) => {
          ...
          if (
            ...
            [SHIPRETURN, VOIDSHIPRECEIPT].includes(matrec2.issuetype)
          ) {
            //Return and Void record should be considered when calculate the return qty
...
const computedRecInspRemaining = async (data, item, app) => {
  const result = {};
  ...
  SHIPRETURN = app.state.issuetypeshipreturn;
  VOIDSHIPRECEIPT = app.state.issuetypeshipvoid;
  SHIPTRANSFER = app.state.issuetypetshipransfer;
  ISSUETYPETRANSFER = app.state.issuetypetransfer;

  // istanbul ignore else
  if (item.linetype_maxvalue !== STDSERVICE) {
    let localReceiptTransactions = {};
    shipmentMatrectransItems.filter((i) => i.issuetype !== SHIPTRANSFER);
    ...
    shipmentMatrectransItems.forEach((receipt) => {
      ...
      if (receipt.shipmentlinenum === item.shipmentlinenum) {
        ...
        // istanbul ignore else
        if ([SHIPRECEIPT, VOIDSHIPRECEIPT].includes(receipt.issuetype)) {
        ...
        if (isMaximoMobile) {
          ...
          localReceiptTransactions.forEach((apireceipt) => {
            ...
            if (
              ...
              // }
              // istanbul ignore else
              if (apireceipt.issuetype === VOIDSHIPRECEIPT) {
                receiptQty =
              ...
              if (
                ...
                (receipt.externalrefid &&
                  apireceipt.orgrcvexternalrefid === receipt.externalrefid &&
                  [SHIPRECEIPT, VOIDSHIPRECEIPT].includes(
      ...
      localReceiptTransactions.forEach((receipt) => {
        ...
        if (
          ...
          if (
            ...
            // istanbul ignore else
            if (
              [SHIPRECEIPT, SHIPRETURN, VOIDSHIPRECEIPT].includes(
                receipt.issuetype
              )
            ) {
            ...
            localReceiptTransactions.forEach((receipt2) => {
              ...
              if (
                ...
                if (
                  ...
                  // }
                  // istanbul ignore else
                  if (receipt2.issuetype === VOIDSHIPRECEIPT) {
...
const buildShipReceiptItem = (
  ...
  if (isFromMXRecept) {
      ...
      if (existingrec) {
        ...
        if (
          ...
          if (item.rotating) {
            ...
            existingrec.status_maxvalue = WASSET;
            existingrec.actualdate = item.actualdate;
          } else {
            existingrec.status = COMP;
            existingrec.status_maxvalue = COMP;
        ...
        if (!item.orgrcvexternalrefid) {
          ...
          if (item.inspectionrequired) {
            ...
            if (item.siteid !== item.fromsiteid && item.rotating) {
              itemStatus = WASSET;
            } else {
              itemStatus = COMP;
            }
    ...
    } else if (item.issuetype === VOIDSHIPRECEIPT) {
      let existingrec = result_RECEIPT_PAGE.find(
        (i) => i._id === item.receiptref || i._id === item.orgrcvexternalrefid
      );
      // istanbul ignore else
    ...
    let objReturned = {
      ...
      description: description,
      status: itemStatus,
      status_maxvalue: item.status_maxvalue ? item.status_maxvalue : itemStatus,
