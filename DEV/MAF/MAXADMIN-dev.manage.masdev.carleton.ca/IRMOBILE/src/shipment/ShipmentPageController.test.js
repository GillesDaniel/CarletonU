...
import issuetypeDomainData from "./test/shipment-issuetype-domain-data";
...
it("shipment list page with default storeroom", async () => {
  ...
  const synonymdomainDS_issueType = newSynoTypeDatasource(
    issuetypeDomainData,
    "synonymdomainDS"
  );
  synonymdomainDS_issueType.registerController(spController);
  app.registerDatasource(synonymdomainDS_issueType);
  sinon.stub(synonymdomainDS_issueType, "setQBE");
  sinon
    .stub(synonymdomainDS_issueType, "searchQBE")
    .returns(issuetypeDomainData.member);
...
it("shipment list page fake datasource copy", async () => {
  ...
  sinon
    .stub(shipmentMatrectransDS, "getItems")
    .returns(shipmentmatrectransitem.member);
...
it("shipment list page in mobile", async () => {
  ...
  sinon.stub(shipmentassetinputDS, "getItems").returns([]);
  sinon
    .stub(shipmentMatrectransDS, "load")
    .returns(shipmentmatrectransitem.member);
  sinon
    .stub(shipmentMatrectransDS, "getItems")
    .returns(shipmentmatrectransitem.member);
  ...
  spController.pageResumed(spPage, app);
  spController.onBeforeLoadData(dsshipmentlist);
  spController.onAfterLoadData(dsshipmentlist, shipmentlistitem.member);
  shipmentlineds.dependsOn = dsshipmentlist;
  shipmentlineds.lastQuery = {
    selectedRecordHref: items[0].href,
  };
  spController.onAfterLoadData(shipmentlineds, shiplineItems);
...
it("shipment list page session key", async () => {
  ...
  spController.pageResumed(spPage, app);
  spController.onBeforeLoadData(dsshipmentlist);
  spController.onAfterLoadData(dsshipmentlist, dsshipmentlist.getItems());
  shipmentlineds.dependsOn = dsshipmentlist;
  shipmentlineds.lastQuery = {
    selectedRecordHref: items[0].href,
  };
  spController.onAfterLoadData(shipmentlineds, shipmentlineitem.member);
  for (let i = 0; i < 10; i++) {
