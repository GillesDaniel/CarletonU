...
class POListPageController {
  ...
  async setIssueTypeDomain() {
    ...
    domainResponse.forEach((domainrec) => {
      // istanbul ignore else
      if (domainrec.valueid === "ISSUETYP|RECEIPT") {
        this.app.state.issuetypereceipt = domainrec.value;
      }
      // istanbul ignore else
      if (domainrec.valueid === "ISSUETYP|RETURN") {
        this.app.state.issuetypereturn = domainrec.value;
      }
      // istanbul ignore else
      if (domainrec.valueid === "ISSUETYP|VOIDRECEIPT") {
        this.app.state.issuetypevoid = domainrec.value;
      }
      // istanbul ignore else
    ...
    newPOLineDS.items.forEach((item) => {
      promisePool.push(
        CommonUtil.computedRecInspRemaining(
          {
            matrectransitems: newMatrectransDS.items,
  ...
  onAfterLoadData(dataSource, items) {
    const dsName = dataSource.name;
    // istanbul ignore else
    if (dsName === POLISTDS) this.poList = items;
    // istanbul ignore else
    ...
    // istanbul ignore else
    if (this.dsLoadedStatus === 1 || this.dsLoadedStatus === ALL_WATCHED_DS_LOADED) {
