...
import makeSafeSave from "./utils/makeSafeSave";
...
class CalibrationPointsController extends BaseController {
  ...
  async pageResumed(page, app) {
    ...
    // Ensure all state data used by page is correctly loaded and available
    await this.loadStateData(app, page);
    ...
    this.app.state.calpointsds?.addIgnoreField('showWarningOnInputChange');
    this.app.state.calpointsds?.addIgnoreField('showWarningOnOutputChange');
    ...
    this.page.state.asfoundcallabel = this.setLabelColor(this.page.state.asfoundcalstatus);
    this.page.state.asleftcalstatus = page.params.asleftcalstatus;
    this.page.state.asleftcallabel = this.setLabelColor(this.page.state.asleftcalstatus);
  ...
  isToleranceWarningShownOnTabOut(error) {
    const isToleranceWarning = DatasheetConstants.VALIDATE_TOLERANCE_ON_TAB_OUT === this.pluscTolWarnValue && error;
  ...
  setCalPointDsWarning(input, message, whichFieldTypeChanged) {
    const field = this.getModifiedFieldName(input.plantype, whichFieldTypeChanged);;
    this.app.state.calpointsds.setWarning(
      input,
      field,
      message
    );
  ...
  async discardChanges() {
    // Reload Datasource to undo changes
    const pluscWoDs = this.app.findDatasource("pluscWoDs");

    const item = pluscWoDs.item;

    await pluscWoDs.load({ noCache: true, where: "" });
    const assetfunctionDS = await pluscWoDs.getChildDatasource(
      "pluscwodsinstr",
      item,
      DatasourceConstants.ASSET_FUNCTION_CHILD_OPTIONS
    );

    const response = await assetfunctionDS.load();
  ...
  /**
   * Load assetfunction and calibration points into global state 
   * variables based on data informed in page params.
   * 
   * @param {Application} app 
   * @param {Page} page 
   * @returns Boolean - Returns whether the state variables were loaded.
   */
  async loadStateData(app, page) {
    const { wonum, wodsnum, assetfunctionid } = page.params;
    const hasAllRequiredParams = wonum && wodsnum && assetfunctionid;

    // We alread have asset function and calibration points,
    // then make sure we are setting the correct asset function
    // as "currentItem" and skip this loading, since it is quite 
    // expensive.
    if (app.state.assetFunctionsDetailsDS && app.state.calpointsds) {
      this.setAssetFunctionCurrentItemById(assetfunctionid);
      return false;
    }

    // Must have all required params available in the url:
    //  - wonum: work order number to load woDetailCalibration
    //  - wodsnum: datasheet num to load pluscWoDs
    //  - assetfunctionid: asset function id to load pluscwodsinstr
    if (!hasAllRequiredParams) {
      return false;
    }

    // Proceed by turning on the loading spinner
    // and loading all data: work order, datasheet, assetfuncition
    // and calibration points.
    page.state.loading = true;

    const workorderDS = app.findDatasource("woDetailCalibration");
    await workorderDS.load({ qbe: { wonum } });

    const datasheetDS = app.findDatasource("pluscWoDs");
    await datasheetDS.load({ qbe: { wodsnum } });

    const assetfunctionDS = await datasheetDS.getChildDatasource("pluscwodsinstr", datasheetDS.currentItem, DatasourceConstants.ASSET_FUNCTION_CHILD_OPTIONS);
    const assetfunctions = await assetfunctionDS.load();
    const assetfunction = assetfunctions.find((item) => item.pluscwodsinstrid === Number(assetfunctionid));

    const calpointsDS = await assetfunctionDS.getChildDatasource("pluscwodspoint", assetfunction, DatasourceConstants.CALIBRATION_POINT_CHILD_OPTIONS);
    await calpointsDS.load();

    app.state.assetFunctionsDetailsDS = assetfunctionDS;
    app.state.calpointsds = calpointsDS;

    // Setting the asset function as "currentItem" in the datasource
    this.setAssetFunctionCurrentItemById(assetfunctionid);
  ...
  getDatasheetDS() {
    ...
    datasheetDS.safeSave = makeSafeSave(datasheetDS);
  ...
  /**
   * Set current item in assetfunction datasource based on the asset 
   * function id informed by url parameter. 
   * 
   * When the calibration page is opened, we receive the url param
   * `assetfunctionid`, which indicates the asset function we are 
   * currently visiting and changing. When loading state data (loadStateData),
   * we call this method to change the currentItem selected in the 
   * datasource to the one we are visiting using this id.
   * 
   * This will be important in `handleChange` and `saveUpdate`, because 
   * CalibrationPointHandler requires assetfunction datasource and
   * will work with the currentItem set there.
   * 
   * @param {Number} pluscwodsinstrid - Asset function id
   * @returns {ProxyObject} Returns current asset function.
   */
  /* istanbul ignore next */
  setAssetFunctionCurrentItemById(pluscwodsinstrid) {
    const assetFunctionDS = this.getAssetFunctionDS();
    const index = assetFunctionDS.items.findIndex(assetfunction => assetfunction.pluscwodsinstrid === Number(pluscwodsinstrid));
    const currentItem = assetFunctionDS.get(index, CalibrationPointConstants.SET_AS_CURRENT);
    return currentItem;
  }
  ...
  async calculateInitialStatus(app) {
    // Load datasources
    const pluscwodsinstr = app.state.assetFunctionsDetailsDS;

    // Map all calibration points
    // istanbul ignore next
    if (pluscwodsinstr) {
      const dsconfig = await this.loadDsConfig(pluscwodsinstr);
      const domaincalstatusds = app.findDatasource("domaincalstatusds");

      const item = pluscwodsinstr.currentItem;
      const calpointsds = this.getCalibrationPointsDS();

      // Datasheet Calculation
      const calc = new DatasheetCalculation(dsconfig.currentItem, this.getLocale());
  ...
  */
  ...
  async saveUpdate() {
    ...
    if (this.getLengthError() || this.getNumberError() || this.getInputError()) {
      this.app.toast(
    ...
    const datasheetDS = await handler._completeReadings(
      null,
      condition,
      CalibrationPointConstants.COMPLETE_READINGS_BACKGROUND
    );
    
    await datasheetDS.safeSave();
  ...
  getModifiedFieldName(plantype, whichFieldTypeChanged) {
    ...
    const fieldName = isAnalog ? `${conditionPrefix}${whichFieldTypeChanged}`: `${conditionPrefix}setpoint`;
    return fieldName;
  }
  ...
  calculateTolError(conditionPrefix, calpoint, calc, roundingOptions) {
    ...
    for (let i = 1; i < 5; i++) {
      ...
      if (!isNaN(lowervalue) && lowervalue !== null) {
        ...
        calpoint[`${conditionPrefix}error${i}`] = toDisplayableValue(tolerror, roundingOptions, locale);
      }
    }
   ...
   *
   * @param {Object} event - Object with properties "item" and new status.
   * @returns Boolean
   */
  /* istanbul ignore next */
  updateStatus(event) {
    const { item, condition, status } = event;
    this.app.state.textFieldChanged = true;
    const isValidCondition =
      CalibrationPointConstants.PREFIXES.includes(condition);

    if (isValidCondition) {

      item[`${condition}pass`] = status === DatasheetConstants.STATUS_PASS;
      item[`${condition}fail`] = status === DatasheetConstants.STATUS_FAIL;
  ...
  funcCheckToggle(args) {
    ...
    const relatedStatusId = event.id === `${condition}pass` ? `${condition}fail` : `${condition}pass`;
    item[event.id] = true;
  ...
  validateCalPointAndCalculateTolerances(context, event) {
    ...
    if (this.validateNumerical(calpoint, whichFieldTypeChanged)) {
      return;
    }

    // User typed in too long number - this checks for the max length allowed, not related to tolerance ranges
    if (this.validateLength(calpoint, whichFieldTypeChanged)) {
      return;
    };
  ...
  checkBounds(input, whichFieldTypeChanged) {
    ...
    const isIOValueOutOfRange = (ioValue, lowerValue, upperValue) => ioValue < lowerValue || ioValue > upperValue;
    ...
    const localizedCondition = conditionPrefix === CalibrationPointConstants.CONDITION_ASFOUND 
      ...
      if (rightRon1Lower && rightRon1Upper) {
        ...
        if (ioValue < rightRon1Lower || ioValue > rightRon1Upper) {
          ...
          const message = new Message(
            ...
            [localizedCondition, whichFieldTypeChanged, ioValue, ron1lower, ron1upper]
          );

          this.page.state.message = message.getLocalizedLabel();

          input[`${conditionPrefix}fail`] = true;
          this.setCalPointDsWarning(input, this.page.state.message, whichFieldTypeChanged);
          this.setInputError(input.pluscwodspointid, whichFieldTypeChanged, true);
        }
      }
      if (instr.cliplimitsin) {
        if (isIOValueOutOfRange(ioValue, inputLowerBound, inputUpperBound)) {
          ...
          const message = new Message(
            ...
            [localizedCondition, whichFieldTypeChanged, ioValue, inputLowerBound, inputUpperBound]
          );
  ...
  standardizeDecimal(calpoint, whichFieldTypeChanged) {
    const locale = this.getLocale();
    const fieldName = this.getModifiedFieldName(calpoint.plantype, whichFieldTypeChanged);
    ...
    if (ioValue) {
      ...
      const ioFormatted = toDisplayableValue(ioNumber, {
  ...
  validateNumerical(input, whichFieldTypeChanged) {
    const isAnalog = input.plantype === CalibrationPointConstants.PLANTYPE.ANALOG;
    const conditionPrefix = this.getConditionPrefix();
    const ioValue = isAnalog ? input[`${conditionPrefix}${whichFieldTypeChanged}`] : input[`${conditionPrefix}setpoint`];
    const locale = this.getLocale();
    if (!LocaleNumberUtil.isValidNumber(ioValue, locale)) {
      const message = new Message("enter_numeric_value", "Enter a numeric value");
      this.setCalPointDsWarning(input, message.getLocalizedLabel(), whichFieldTypeChanged);
  ...
  validateLength(input, whichFieldTypeChanged) {
    const isAnalog = input.plantype === CalibrationPointConstants.PLANTYPE.ANALOG;
    const conditionPrefix = this.getConditionPrefix();
    let floatInput = parseFloat(isAnalog ? input[`${conditionPrefix}${whichFieldTypeChanged}`] : input[`${conditionPrefix}setpoint`]);
    ...
    if (inputlength > CalibrationPointConstants.MAX_VALUE_LENGTH) {
      const message = new Message("entered_number_is_too_long", "Entered number is too long");
      this.setCalPointDsWarning(input, message.getLocalizedLabel(), whichFieldTypeChanged);
