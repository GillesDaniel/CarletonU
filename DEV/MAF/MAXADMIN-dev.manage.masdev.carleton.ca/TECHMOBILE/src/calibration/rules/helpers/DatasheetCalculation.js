...
class DatasheetCalculation {
  ...
  resolveToDefaultExternalSynonym(maxvalue) {
    /* 
    const synonymSet = this.calstatus.find(
      "maxvalue == $1 && defaults == $2",
      maxvalue,
      true
    );
    const synonym = synonymSet.length > 0 ? synonymSet[0] : null;
    return synonym;
    */
    return {
  ...
  /**
   * Calculates the tolerance for points that are analog of discrete.
   ...
   */
  calculateAvgAndStdDeviation(condition, assocpoints, avgpoint) {
    const locale = this.getLocale();
