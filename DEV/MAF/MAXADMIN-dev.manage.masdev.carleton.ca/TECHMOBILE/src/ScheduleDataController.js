...
class ScheduleDataController {
  ...
  computedWorkTypeStatus(item) {
    ...
    let useTimerSystemProp = this.app.state.systemProp && this.app.state.systemProp["maximo.mobile.usetimer"];
    ...
    let travelNavigation = Device.get().isMaximoMobile && this.app.state.systemProp["mxe.mobile.travel.navigation"];
    ...
    if (serlat === undefined || serlong === undefined || !travelPrompt || !this.app.geolocation.state.enabled || this.app.geolocation.state.hasPermissionError) {
      ...
      //istanbul ignore else
      if (travelPrompt && travelPrompt === '1' && this.getLabTransType(activeLabTrans) !== 'WORK' && (distance > parseFloat(travelRadius) || this.getLabTransType(activeLabTrans) === 'TRAV_ACTIVE')) {
  ...
  */
  isRejected(item) {
    ...
    if(item?.assignment?.[0]?.status?.toUpperCase() === rejectLabel) {
      return true;
    } else {
      return false;
    }
