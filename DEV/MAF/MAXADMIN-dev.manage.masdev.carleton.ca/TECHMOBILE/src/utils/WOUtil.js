...
const openEnterReadingDrawer = async (
  ...
  assetMeterDs.items.forEach((element) => {
    ...
    if (readingType === "old") {
      ...
      if(!element.computedMeterCurDate){
        element.computedMeterCurDate = app.findDatasource("woMultiAssetLocationds")?.item?.computedMeterCurDate
      }
      if(!element.computedMeterCurTime){
        element.computedMeterCurTime = app.findDatasource("woMultiAssetLocationds")?.item?.computedMeterCurTime
      }
    }
  });
  ...
  try {
    ...
    if (
      ...
      (
        ...
        assetMeterDs.name.includes("cds_tasklistMeterds.activeassetmetermobile") || 
        ...
        for (let i = 0; i < assetMeterDs.items.length; i++) {
          ...
          if (typeof udpatedMeterObj.newreading !== "undefined") {
            ...
            if (page.state.oldReading) {
              if (
                assetMeterDs.name.includes("cds_tasklistMeterds.activeassetmetermobile") || 
                assetMeterDs.name.includes("cds_tasklistMeterds.activelocationmetermobile")
              ) {
            ...
            if (udpatedMeterObj.newreading === 0 || udpatedMeterObj.newreading) {
              ...
              // istanbul ignore else
              if (isTask) {
                option.relationship = 'activeassetmetermobile';
              }
              ...
              try {
                ...
                if (updatedResponse && saveDataSuccessful) {
                  assetMeterDs.items.forEach((element) => {
                    if (element.newreading || element.newreading === 0) {
    ...
    if (
      ...
      (
        ...
        locationMeterDs.name.includes("cds_tasklistMeterds.activelocationmetermobile") ||
        locationMeterDs.name.includes("cds_woMultiAssetLocationds.activelocationmeter")
      )
    ) {
    // istanbul ignore if
        ...
        for (let j = 0; j < locationMeterDs.items.length; j++) {
          ...
          if (typeof udpatedLocMeterObj.newreading !== "undefined") {
            ...
            if (page.state.oldReading === true)
            {
              ...
              } else if (locationMeterDs.name.includes("cds_tasklistMeterds.activelocationmetermobile")) {
            ...
            if (udpatedLocMeterObj.newreading === 0 || udpatedLocMeterObj.newreading) {
              ...
              // istanbul ignore else
              if (isTask) {
                option.relationship = "activelocationmetermobile";
              }
              try {
                app.userInteractionManager.drawerBusy(true);
                saveDataSuccessful = true;
      ...
      if (changeObj.item) {
        ...
        if (self.page.state.oldReading) {
          ...
          if (
            ...
            changeObj.datasource.name.includes("cds_tasklistMeterds.activeassetmetermobile") ||
            changeObj.datasource.name.includes("cds_tasklistMeterds.activelocationmetermobile")
          ) {
  ...
  if (item) {
    ...
    if (item.newreading || item.newreading === 0 ) {
      ...
      if (newreadingdate && lastreadingdate && (newreadingdate < lastreadingdate)) {
        ...
        let date = dataFormatter.currentUserDateTime();
        item.computedMeterCurDate = date;
        item.computedMeterCurTime = date;
    }
  }
