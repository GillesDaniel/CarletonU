...
import relatedrecwo from "./test/relatedrecwo-json-data.js";
import {
  Application,
  Page,
  JSONDataAdapter,
...
it("should navigate to schedulepage", async () => {
  ...
  const reserveMaterials = new Page({ name: 'reserveMaterials'});
  page.registerDatasource(ds);
  page.registerDatasource(wodetails);
  ...
  reserveMaterials.state.reservedItemAdded = false;
  app.registerPage(reserveMaterials);
  let items = await ds.load();
...
it("should load data on container mode", async () => {
  ...
  const relatedrecwods = newDatasource(relatedrecwo, 'relatedrecwo');
  woDetailRelatedWorkOrder.controllers.push(new RelatedWoController());
  relatedrecwods.controllers.push(new RelatedWoController());
  fupPage.registerDatasource(woDetailRelatedWorkOrder);
  fupPage.registerDatasource(relatedrecwods);
  fupPage.params = {
    noCache: true,
    itemUrl: detailDS.item.href
  }
  await relatedrecwods.load();
  ...
  expect(page.state.followupCount).toBe(16);
...
it("should open DowntimeCode lookup", async () => {
  ...
  const ds = newStatusDatasource(downTimeCode, "dsnewreading");
  app.registerDatasource(ds);
...
it("should invoke validateMeterReadings using onFocusReadings", async () => {
  ...
  event.newreading = undefined;
  page.state.invalidDateTime = false;
  page.state.hasAnyReadingError = false;
  let enableDisableSaveBtnspy = sinon.spy(WOUtil,'enableDisableSaveBtn');  
  await controller.onFocusReadings(event);
  expect(validateMeterReadingsStub.called).toBe(false);
  expect(page.state.disableSave).toBe(false);
  expect(enableDisableSaveBtnspy.called).toBe(true);
...
it("saveCharactersticMeterReading for asset", async () => {
  ...
  app.registerController(controller);
  const newreadingds = newDatasourceNew(dsnewreading, "dsnewreading");
...
it("should open enter meter sliding drawer page", async () => {
  ...
  page.state.assetMeterData.items.forEach((element) => {
    ...
    expect(element.computedMeterCurDate).toBeDefined();
    expect(element.computedMeterCurTime).toBeDefined();
  });
...
it('Validate meter details', async () => {
  ...
  app.registerController(controller);
  commonUtil.meterValidate(validateEvent,'saveDiscardMeterReadingDetail',page);
});