...
it("Page initialized", async () => {
  ...
  const dsCal = newDatasource(reservationsListDS, "reservationsListDS_cal");
  dsCal.registerController(controller);
  app.registerDatasource(dsCal);
...
it("Page loading", async () => {
  ...
  page.params.reservedItemsInvUsage = [];

  await app.initialize();

  app.state.selectedInvUseDSName = "invUseDS";
  controller.pageResumed(page, app);
  ds.savedTotalCount = 10;
  ds.emit("changed", ds);
});
...
it("loadDatasource with this.app.device.isMaximoMobile = true", async () => {
  ...
  controller.onAfterLoadData(ds, reservationsListDS.member);
});
...
it("check updateSelectedItems", async () => {
  ...
  ds.items.forEach((item) => delete item._disabled);
