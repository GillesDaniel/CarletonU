...
it("Load data and execute computedDueDate functions", async () => {
  const controller = new ReservationsListDataController();
  const app = new Application();
  ...
  let computedDate = controller.computedDueDate(items[0]);
  expect(computedDate).toBeTruthy();

  computedDate = controller.computedDueDate(items[1]);
...
it("After load data", async () => {
  ...
  controller.onAfterLoadData(reservationsListDS, reservationsData.member);

  reservationsListDS.lastQuery.searchText = "abc";
  controller.onAfterLoadData(reservationsListDS, reservationsData.member);

  reservationsListDS.lastQuery.searchText = "";
  controller.onAfterLoadData(reservationsListDS, reservationsData.member);

  reservationsListDS.meta = {};
  controller.onAfterLoadData(reservationsListDS, reservationsData.member);
});
