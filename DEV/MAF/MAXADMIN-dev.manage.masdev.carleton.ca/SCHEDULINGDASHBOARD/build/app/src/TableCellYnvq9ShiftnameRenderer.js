...
const TableCellYnvq9ShiftnameRenderer = observer(props => {
    const app = props.app;
...
const jOptimizeSummaryDS = props.jOptimizeSummaryDS || app.findDatasource("jOptimizeSummaryDS");
    ...
    datacomponentoriginalid={"mbvy2"}
    datacomponenttype={"box"}
    datadraggable={"true"}
    droppable={"true"}
    
  >
<DatasourceLabel  theme={"field"} label={app.format((item?item.shiftname:undefined),'shiftname',null,datasource)} spacing={"none"} padding={"none"} id={'qy7dn[' + props.rowId + ']'} datadraggable={"true"} datacomponenttype={"label"} datacomponentoriginalid={"qy7dn"} datasource={datasource} item={item} field={"shiftname"}></DatasourceLabel>
</Box>
...
export default inject('app')(TableCellYnvq9ShiftnameRenderer);
