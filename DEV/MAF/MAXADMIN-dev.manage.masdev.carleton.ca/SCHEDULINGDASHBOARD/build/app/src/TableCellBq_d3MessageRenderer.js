...
const TableCellBq_d3MessageRenderer = observer(props => {
    const app = props.app;
...
const jOptimizeSummaryDS = props.jOptimizeSummaryDS || app.findDatasource("jOptimizeSummaryDS");
    ...
    return (
      ...
      <DatasourceLabel  theme={"field"} label={app.format((item?item.message:undefined),'message',null,datasource)} spacing={"none"} padding={"none"} id={'qad47[' + props.rowId + ']'} datadraggable={"true"} datacomponenttype={"label"} datacomponentoriginalid={"qad47"} datasource={datasource} item={item} field={"message"}></DatasourceLabel>

    
    );
  });

export default inject('app')(TableCellBq_d3MessageRenderer);
