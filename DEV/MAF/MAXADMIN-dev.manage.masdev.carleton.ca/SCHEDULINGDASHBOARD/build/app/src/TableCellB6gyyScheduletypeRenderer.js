...
const TableCellB6gyyScheduletypeRenderer = observer(props => {
    const app = props.app;
...
const jdashboardKPIds = props.jdashboardKPIds || app.findDatasource("jdashboardKPIds");
    ...
    return (
      ...
      <DatasourceLabel  label={app.format((item?item.ScheduleTypeLabel:undefined),'ScheduleTypeLabel',null,datasource)} id={'qvmae[' + props.rowId + ']'} padding={"default"} datadraggable={"true"} datacomponenttype={"label"} datacomponentoriginalid={"qvmae"} datasource={datasource} item={item} field={"ScheduleTypeLabel"}></DatasourceLabel>

    
    );
  });

export default inject('app')(TableCellB6gyyScheduletypeRenderer);
