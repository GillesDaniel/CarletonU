...
const TableCellB3gyyModifiedRenderer = observer(props => {
    const app = props.app;
...
const jresourceLevelingTaskDS = props.jresourceLevelingTaskDS || app.findDatasource("jresourceLevelingTaskDS");
    ...
    datacomponentoriginalid={"nvn37"}
    datacomponenttype={"box"}
    datadraggable={"true"}
    droppable={"true"}
    
  >
<Label  theme={"field"} label={app.format((item?item.computeChanged:undefined),'computeChanged',null,datasource) ? 'Yes' : 'No'} spacing={"none"} padding={"none"} id={'ed_kq[' + props.rowId + ']'} datadraggable={"true"} datacomponenttype={"label"} datacomponentoriginalid={"ed_kq"}></Label>
</Box>
...
export default inject('app')(TableCellB3gyyModifiedRenderer);
