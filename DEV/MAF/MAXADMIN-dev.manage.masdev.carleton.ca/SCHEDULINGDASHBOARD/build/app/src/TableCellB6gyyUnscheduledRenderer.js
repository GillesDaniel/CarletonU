...
const TableCellB6gyyUnscheduledRenderer = observer(props => {
    const app = props.app;
...
const jdashboardKPIds = props.jdashboardKPIds || app.findDatasource("jdashboardKPIds");
    ...
    hidden={(item?item.unscheduled:undefined) === 0 || (item?item.optimizationstatus:undefined) === 'PROCESSING'}
    ...
    datacomponentoriginalid={"m524p"}
    datacomponenttype={"box"}
    datadraggable={"true"}
    droppable={"true"}
    
  >
<Link label={app.format((item?item.unscheduled:undefined),'unscheduled',null,datasource)} onClick={(event)=>{eventManager.emit('change-page', {name: 'scheduleDetails', ds: datasource, item: item , params:({'selectedTab':2,'scenario':item.href,'projectname':item.name, 'projectid':item.skdprojectid, 'optimizationstatus':item.optimizationstatus, 'scheduletype':item.ScheduleTypeLabel, 'issuecount':item.unscheduled, 'startdate':item.startdateacm, 'enddate': item.enddateacm, 'nodata':item.skdscenrunkpi_allresdata ? false : true})}, event)}} theme={"small"} id={'n4wnx[' + props.rowId + ']'} spacing={"form"} datacomponenttype={"link"} datacomponentoriginalid={"n4wnx"} />
</Box>
<Box
    id={'m424p[' + props.rowId + ']'}
    ...
    datacomponentoriginalid={"m424p"}
    datacomponenttype={"box"}
    datadraggable={"true"}
    droppable={"true"}
    
  >
<DatasourceLabel  theme={"small"} label={app.format((item?item.unscheduled:undefined),'unscheduled',null,datasource)} spacing={"none"} padding={"none"} id={'nbvcp[' + props.rowId + ']'} datadraggable={"true"} datacomponenttype={"label"} datacomponentoriginalid={"nbvcp"} datasource={datasource} item={item} field={"unscheduled"}></DatasourceLabel>
</Box>
</React.Fragment>
...
export default inject('app')(TableCellB6gyyUnscheduledRenderer);
