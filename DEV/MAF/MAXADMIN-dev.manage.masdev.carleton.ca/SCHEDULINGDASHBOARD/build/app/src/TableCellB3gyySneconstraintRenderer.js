...
const TableCellB3gyySneconstraintRenderer = observer(props => {
    const app = props.app;
...
const jresourceLevelingTaskDS = props.jresourceLevelingTaskDS || app.findDatasource("jresourceLevelingTaskDS");
    ...
    datacomponentoriginalid={"znnym"}
    datacomponenttype={"box"}
    datadraggable={"true"}
    droppable={"true"}
    
  >
<SmartInput id={'bjgkvs[' + props.rowId + ']'} invalid={(item?item.sneconstraint:undefined) > (item?item.fnlconstraint:undefined)} value={(item?item.sneconstraint:undefined)} hideLabel={true} showVerbose={false} theme={"dark"} size={"none"} dateType={"date"} datadraggable={"true"} datacomponenttype={"smart-input"} datacomponentoriginalid={"bjgkvs"} datasource={datasource} item={item} field={"sneconstraint"} dataFormatter={app.formatter}/></Box>

    
    );
  });

export default inject('app')(TableCellB3gyySneconstraintRenderer);
