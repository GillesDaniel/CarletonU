...
const TableCellYnvq9LvlRenderer = observer(props => {
    const app = props.app;
...
const jOptimizeSummaryDS = props.jOptimizeSummaryDS || app.findDatasource("jOptimizeSummaryDS");
    ...
    datacomponentoriginalid={"jade2"}
    datacomponenttype={"box"}
    datadraggable={"true"}
    droppable={"true"}
    
  >
<DatasourceLabel  theme={"field"} label={app.format((item?item.lvl:undefined),'lvl',null,datasource)} spacing={"none"} padding={"none"} id={'nqegq[' + props.rowId + ']'} datadraggable={"true"} datacomponenttype={"label"} datacomponentoriginalid={"nqegq"} datasource={datasource} item={item} field={"lvl"}></DatasourceLabel>
</Box>
...
export default inject('app')(TableCellYnvq9LvlRenderer);
