...
const TableCellYnvq9AvailablehoursRenderer = observer(props => {
    const app = props.app;
...
const jOptimizeSummaryDS = props.jOptimizeSummaryDS || app.findDatasource("jOptimizeSummaryDS");
    ...
    datacomponentoriginalid={"nm2a8"}
    datacomponenttype={"box"}
    datadraggable={"true"}
    droppable={"true"}
    
  >
<DatasourceLabel  theme={"field"} label={app.format((item?item.availablehours:undefined),'availablehours',null,datasource)} spacing={"none"} padding={"none"} id={'dx6x9[' + props.rowId + ']'} datadraggable={"true"} datacomponenttype={"label"} datacomponentoriginalid={"dx6x9"} datasource={datasource} item={item} field={"availablehours"}></DatasourceLabel>
</Box>
...
export default inject('app')(TableCellYnvq9AvailablehoursRenderer);
