...
const ButtonGroupNmnpjComponent = (props) => {
  const app = props.app;
...
const jOptimizeSummaryDS = props.jOptimizeSummaryDS || app.findDatasource("jOptimizeSummaryDS");
  ...
  return (<Observer>{()=>(<><ButtonGroup id={"nmnpj"} hidden={skdodmerunForOptimizeDS.item.optimizationstatus === undefined || skdodmerunForOptimizeDS.item.optimizationstatus === 'PROCESSING'} align={"start"} padding={true} datadraggable={"true"} droppable={"true"} datacomponenttype={"button-group"} datacomponentoriginalid={"nmnpj"}>
<Button label={app.getLocalizedLabel("rp924_label", "Go to schedule")} kind={"primary"} onClick={(event)=>{eventManager.emit('goToScheduleDetailsPage', event)}} id={"rp924"} small={true} tagValueLimit={99} datadraggable={"true"} datacomponenttype={"button"} datacomponentoriginalid={"rp924"} />
</ButtonGroup>
</>)}</Observer>);
};
export default ButtonGroupNmnpjComponent;
