...
const TableCellB3gyyDurationRenderer = observer(props => {
    const app = props.app;
...
const jresourceLevelingTaskDS = props.jresourceLevelingTaskDS || app.findDatasource("jresourceLevelingTaskDS");
    ...
    datacomponentoriginalid={"ay8ja"}
    datacomponenttype={"box"}
    datadraggable={"true"}
    droppable={"true"}
    
  >
<SmartInput id={'durationInput[' + props.rowId + ']'} value={(item?item.duration:undefined)} min={0} hideLabel={true} hideStepButtons={true} theme={"dark"} size={"none"} dateType={"date"} datadraggable={"true"} datacomponenttype={"smart-input"} datacomponentoriginalid={"durationInput"} datasource={datasource} item={item} field={"duration"} dataFormatter={app.formatter}/></Box>

    
    );
  });

export default inject('app')(TableCellB3gyyDurationRenderer);
