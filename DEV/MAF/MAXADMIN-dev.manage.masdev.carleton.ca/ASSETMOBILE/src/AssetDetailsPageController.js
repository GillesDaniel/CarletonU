...
class AssetDetailsPageController {
  ...
  /**
   * Update data type for an specification item(s) based on the classstructure and set domains for it.
   * @param {object} event - The event object containing the item to check.
   * 
   */
  async checkDataType(event) {
    ...
    let assetAttributeList = assetAttributeDS.items;
    const attributeMap = {};
    assetAttributeList.forEach((element) => {
      attributeMap[element.assetattrid] = element.datatype_maxvalue;
    });

    let requiredSpecId, newlyAddedAssetSpec;
    ...
    dup.forEach((element) => {
      ...
      if (requiredSpecId) {
        ...
        newlyAddedAssetSpec = assetAttributeList.find(listElement => listElement.assetattrid === element.assetattrid);
        element.domainid = newlyAddedAssetSpec && newlyAddedAssetSpec['domainid'];
      }
  ...
  async captureLocation(param) {
    ...
    if (geolocationlat != 0 && geolocationlong != 0) {
      ...
      if (serviceAddressItems.length == 0) {
        ...
        this.app.toast(
          this.app.getLocalizedLabel(`device_location_saved`, `Device location saved`),
          "success"
        );
      } else {
      ...
      this.app.toast(
        this.app.getLocalizedLabel(`device_location_error`, `Device location not available`),
        "error"
      );
    }
  }
