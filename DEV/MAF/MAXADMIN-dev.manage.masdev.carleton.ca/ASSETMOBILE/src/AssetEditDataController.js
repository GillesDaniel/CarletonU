...
class AssetEditDataController {
  ...
  async pageResumed() {
    ...
    if (this.page.params.href) {
      ...
      this.page.state.isLinear = dsEditAsset.item.islinear;
    }
  ...
  setDefaultValidationParams() {
    ...
    this.page.state.isLinearValid = true;
  ...
  */
  ...
  displayWarningPopup(moveResponse) {
    // istanbul ignore else
    if (moveResponse && moveResponse.warnings) {
  ...
  */
  ...
  handleLinearToggleEdit() {
    let dsEditAsset = this.app.findDatasource("dsEditAsset");
    this.page.state.isLinear = !this.page.state.isLinear;
    dsEditAsset.currentItem.islinear = this.page.state.isLinear;
  
    if (!this.page.state.isLinear) {
      
      dsEditAsset.updateRequired('lrm', false, dsEditAsset.item, false)
      dsEditAsset.updateRequired('startmeasure', false, dsEditAsset.item, false)
      dsEditAsset.updateRequired('endmeasure', false, dsEditAsset.item, false)
      this.page.state.validLrm = true;
      this.page.state.validLrmStartMeasure = true;
      this.page.state.validLrmEndMeasure = true;
      this.clearWarnings("lrm");
      ...
      dsEditAsset.updateRequired('lrm', true, dsEditAsset.item, false);
      dsEditAsset.updateRequired('startmeasure', true, dsEditAsset.item, false);
      dsEditAsset.updateRequired('endmeasure', true, dsEditAsset.item, false);
      this.page.state.validLrm = false;
      this.page.state.validLrmStartMeasure = false;
      this.page.state.validLrmEndMeasure = false;

    }
    this.validateLinearFields();
  }

  async validateLinearDirectionEdit(event) {
    const assetItem = this.app.findDatasource("dsEditAsset").item;
    ...
    if ((items.length > 0) && (event.item.direction.length > 0)) {
  ...
  async validateLrmEdit(event) {
    ...
    if ((items.length > 0) && (event.item.lrm.length > 0)) {
      ...
      this.showWarnings('lrm', errorMessage);
  ...
  async openLinearDirectionLookupEdit() {
    let synonymDS = this.app.findDatasource("synonymdomainData");
    await synonymDS.initializeQbe();
    synonymDS.setQBE("domainid", "=", "LINEARDIRECTION");
    await synonymDS.searchQBE();

    this.page.showDialog("linearDirectionLookupEdit");
  }

  async openLrmLookupEdit() {
    let assetLRMEdit = this.app.findDatasource("assetLRMEdit");
    await assetLRMEdit.clearQBE();
    await assetLRMEdit.searchQBE();
    this.page.showDialog("lrmLookupEdit");
  }

  chooseLinearDirectionEdit(itemSelected) {
    let dsEditAsset = this.app.findDatasource("dsEditAsset");
    dsEditAsset.item.direction = itemSelected.value;
  ...
  async chooseLRMEdit(itemSelected) {
    let dsEditAsset = this.app.findDatasource("dsEditAsset");
    dsEditAsset.item.lrm = itemSelected.lrm;
    const LRMDialog = this.page.findDialog("lrmLookupEdit");
    //istanbul ignore else
    if (LRMDialog) {
  ...
  validateLinearFields() {
    this.page.state.isLinearValid = this.page.state.validLrm && this.page.state.validLrmStartMeasure &&
