...
it("computedStartLinearDescription returns start measure Details", async () => {
  const controller = new AssetDetailsDataController();
  const app = new Application();
  const page = new Page({ name: "assetDetails" });
  app.registerPage(page);
  ...
  let detail = controller.computedStartLinearDescription({
    startmeasure: 0,
    startdescription: 'Sanitary Pipe Segment #101'
    
  });  
  expect(detail).toBe("0 Sanitary Pipe Segment #101");  
});

it("computedEndLinearDescription returns end measure Details", async () => {
  const controller = new AssetDetailsDataController();
  const app = new Application();
  const page = new Page({ name: "assetDetails" });
  app.registerPage(page);
  ...
  let detail = controller.computedEndLinearDescription({
    endmeasure: 4,
    enddescription: 'Sanitary Pipe Segment #101'
    
  });  
  expect(detail).toBe("4 Sanitary Pipe Segment #101");  
});

it("computedServiceAdd returns service address Details", async () => {
  const controller = new AssetDetailsDataController();
  const app = new Application();
  const page = new Page({ name: "assetDetails" });
  app.registerPage(page);
  ...
  let serviceAdd = controller.computedServiceAdd({
    addressline2: 'add line 2',
    country: 'BD'
    
  });  
  expect(serviceAdd).toBe("add line 2, BD");  
